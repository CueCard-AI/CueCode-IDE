...
Push-Location -Path crates/cuecode
...
function PrepareForBundle {
    ...
    Copy-Item -Path "$env:ZED_WORKSPACE\crates\cuecode\resources\windows\*" -Destination "$innoDir" -Recurse -Force
...
function BuildCueCodeAndItsFriends {
    Write-Output "Building CueCode and its friends, for channel: $channel"
    # Build cuecode.exe, cli.exe and auto_update_helper.exe
    cargo build --release --package cuecode --package cli --package auto_update_helper --target $target
    Copy-Item -Path ".\$CargoOutDir\cuecode.exe" -Destination "$innoDir\CueCode.exe" -Force
...
function BuildRemoteServer {
    ...
    $remoteServerDst = "$env:ZED_WORKSPACE\target\cuecode-remote-server-windows-$Architecture.zip"
...
function ZipCueCodeAndItsFriendsDebug {
    $items = @(
        ".\$CargoOutDir\cuecode.pdb",
        ".\$CargoOutDir\cli.pdb",
    ...
    Compress-Archive -Path $items -DestinationPath ".\$CargoOutDir\cuecode-$env:RELEASE_VERSION-$env:ZED_RELEASE_CHANNEL.dbg.zip" -Force
}
...
function UploadToSentry {
    ...
    Write-Output "Uploading CueCode debug symbols to sentry..."
    for ($i = 1; $i -le 3; $i++) {
        try {
            sentry-cli debug-files upload --include-sources --wait -p cuecode -o zed-dev $CargoOutDir
            break
        }
        catch {
...
function SignCueCodeAndItsFriends {
    if (-not $env:CI) {
        return
    }

    $files = "$innoDir\CueCode.exe,$innoDir\cli.exe,$innoDir\auto_update_helper.exe,$innoDir\zed_explorer_command_injector.dll,$innoDir\zed_explorer_command_injector.appx"
...
function CollectFiles {
    ...
    Move-Item -Path "$innoDir\cli.exe" -Destination "$innoDir\bin\cuecode.exe" -Force
    Move-Item -Path "$innoDir\cuecode.sh" -Destination "$innoDir\bin\cuecode" -Force
...
function BuildInstaller {
    $issFilePath = "$innoDir\cuecode.iss"
    switch ($channel) {
        "stable" {
            $appId = "{{2DB0DA96-CA55-49BB-AF4F-64AF36A86712}"
            $appIconName = "app-icon"
            $appName = "CueCode"
            $appDisplayName = "CueCode"
            $appSetupName = "CueCode-$Architecture"
            # The mutex name here should match the mutex name in crates/cuecode/src/windows_only_instance.rs
            $appMutex = "CueCode-Editor-Stable-Instance-Mutex"
            $appExeName = "CueCode"
            $regValueName = "CueCode"
            $appUserId = "CueCode.CueCode"
            $appShellNameShort = "C&ueCode"
            $appAppxFullName = "CueCode.CueCode_1.0.0.0_neutral__japxn1gcva8rg"
        }
        "preview" {
            $appId = "{{F70E4811-D0E2-4D88-AC99-D63752799F95}"
            $appIconName = "app-icon-preview"
            $appName = "CueCode Preview"
            $appDisplayName = "CueCode Preview"
            $appSetupName = "CueCode-$Architecture"
            # The mutex name here should match the mutex name in crates/cuecode/src/windows_only_instance.rs
            $appMutex = "CueCode-Editor-Preview-Instance-Mutex"
            $appExeName = "CueCode"
            $regValueName = "CueCodePreview"
            $appUserId = "CueCode.CueCode.Preview"
            $appShellNameShort = "C&ueCode Preview"
            $appAppxFullName = "CueCode.CueCode.Preview_1.0.0.0_neutral__japxn1gcva8rg"
        }
        "nightly" {
            $appId = "{{1BDB21D3-14E7-433C-843C-9C97382B2FE0}"
            $appIconName = "app-icon-nightly"
            $appName = "CueCode Nightly"
            $appDisplayName = "CueCode Nightly"
            $appSetupName = "CueCode-$Architecture"
            # The mutex name here should match the mutex name in crates/cuecode/src/windows_only_instance.rs
            $appMutex = "CueCode-Editor-Nightly-Instance-Mutex"
            $appExeName = "CueCode"
            $regValueName = "CueCodeNightly"
            $appUserId = "CueCode.CueCode.Nightly"
            $appShellNameShort = "C&ueCode Nightly"
            $appAppxFullName = "CueCode.CueCode.Nightly_1.0.0.0_neutral__japxn1gcva8rg"
        }
        "dev" {
            $appId = "{{8357632E-24A4-4F32-BA97-E575B4D1FE5D}"
            $appIconName = "app-icon-dev"
            $appName = "CueCode Dev"
            $appDisplayName = "CueCode Dev"
            $appSetupName = "CueCode-$Architecture"
            # The mutex name here should match the mutex name in crates/cuecode/src/windows_only_instance.rs
            $appMutex = "CueCode-Editor-Dev-Instance-Mutex"
            $appExeName = "CueCode"
            $regValueName = "CueCodeDev"
            $appUserId = "CueCode.CueCode.Dev"
            $appShellNameShort = "C&ueCode Dev"
            $appAppxFullName = "CueCode.CueCode.Dev_1.0.0.0_neutral__japxn1gcva8rg"
        }
        default {
...
$debugArchive = "$CargoOutDir\cuecode-$env:RELEASE_VERSION-$env:ZED_RELEASE_CHANNEL.dbg.zip"
$debugStoreKey = "$env:ZED_RELEASE_CHANNEL/cuecode-$env:RELEASE_VERSION-$env:ZED_RELEASE_CHANNEL.dbg.zip"

CheckEnvironmentVariables
PrepareForBundle
GenerateLicenses
BuildCueCodeAndItsFriends
BuildRemoteServer
MakeAppx
SignCueCodeAndItsFriends
ZipCueCodeAndItsFriendsDebug
...
if ($buildSuccess) {
    ...
    if ($Install) {
        Write-Output "Installing CueCode..."
        Start-Process -FilePath "$env:ZED_WORKSPACE/target/CueCode-$Architecture-$env:RELEASE_VERSION.exe"
    }
    exit 0
}
else {
