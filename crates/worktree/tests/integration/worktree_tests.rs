...
async fn test_remote_worktree_update_entries_carry_changed_paths(cx: &mut TestAppContext) {
    ...
    // Paste a skill folder into `.cursor/skills` on the host.
    fs.insert_tree(
        path!("/root/.cursor/skills/skill-1"),
        json!({ "SKILL.md": "skill" }),
    )
    .await;
    cx.run_until_parked();
...

    {
        ...
        assert!(
            changes
                .iter()
                .any(|(path, change)| path == ".cursor/skills/skill-1/SKILL.md"
    ...
    fs.remove_dir(
        path!("/root/.cursor/skills/skill-1").as_ref(),
        RemoveOptions {
            recursive: true,
            ignore_if_not_exists: false,
        },
