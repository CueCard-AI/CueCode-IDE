...
pub use dock::Panel;
pub use layout_engine::{DraggedColumn, COLUMN_GUTTER_HIT_SIZE};
pub use multi_workspace::{
    CloseWorkspaceSidebar, FocusWorkspaceSidebar, MoveProjectToNewWindow,
...
use agent_settings::{AgentPanelSlot, LayoutBlueprint, SideColumnKind};
use dock::{Dock, DockPosition, PanelButtons, PanelHandle, PanelResizeMode, RESIZE_HANDLE_SIZE};
use fs::Fs;
use futures::{
...
use settings::{
    CenteredPaddingSettings, DefaultOpenBehavior, DockSide, Settings, SettingsLocation,
...
use cuecode_actions::{Spawn, feedback::FileBugReport, theme::ToggleMode};
...
pub(crate) const AGENT_PANEL_DOCK_KEY: &str = "agent_panel";
pub(crate) const MIN_INNER_AGENT_WIDTH: Pixels = px(300.);
...
impl Workspace {
    ...
    fn sync_dock_layout_split(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        let blueprint = self.layout_blueprint(cx);
        let split_active = self.split_layout_active(cx);
        let mut left_keys: Vec<&'static str> = Vec::new();
        let mut right_keys: Vec<&'static str> = Vec::new();
        for (side, keys) in [
            (DockSide::Left, &mut left_keys),
            (DockSide::Right, &mut right_keys),
        ] {
            match blueprint.agent_panel_slot(side) {
                AgentPanelSlot::Inner => keys.push(AGENT_PANEL_DOCK_KEY),
                AgentPanelSlot::Outer if split_active => keys.push(AGENT_PANEL_DOCK_KEY),
                _ => {}
            }
        }
        self.left_dock.update(cx, |dock, cx| {
            dock.set_display_suppressed_panel_keys(&left_keys, cx);
        });
        self.right_dock.update(cx, |dock, cx| {
            dock.set_display_suppressed_panel_keys(&right_keys, cx);
        });
        self.normalize_agent_column_sizes(window, cx);
        self.normalize_side_column_nav_sizes(window, cx);
        self.normalize_bottom_dock_panel_sizes(window, cx);
        self.sync_side_column_dock_handles(cx);
    }

    fn normalize_bottom_dock_panel_sizes(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        use crate::layout_engine::MIN_BOTTOM_DOCK_HEIGHT;

        for panel_key in ["TerminalPanel", "DebugPanel"] {
            let Some(panel) = self.bottom_dock.read(cx).panel_for_key(panel_key) else {
                continue;
            };
            let default_height = panel
                .default_size(window, cx)
                .max(MIN_BOTTOM_DOCK_HEIGHT);
            let size_state = self
                .bottom_dock
                .read(cx)
                .stored_panel_size_state(panel.as_ref())
                .unwrap_or_default();
            let current_height = size_state
                .size
                .filter(|size| *size > Pixels::ZERO)
                .or_else(|| self.dock_size(self.bottom_dock.read(cx), window, cx));
            if size_state.flex.is_none()
                && current_height.is_some_and(|height| height >= MIN_BOTTOM_DOCK_HEIGHT)
            {
                continue;
            }
            ...
            self.bottom_dock.update(cx, |dock, cx| {
                dock.resize_panel_by_key_with_mode(
                    panel_key,
                    Some(height),
                    None,
    ...
    fn fixed_column_panel_width(
        panel: &dyn PanelHandle,
        size_state: Option<dock::PanelSizeState>,
        window: &Window,
        cx: &App,
    ) -> Pixels {
        size_state
    ...
    fn normalize_agent_column_sizes(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        let split_active = self.split_layout_active(cx);
        for side in [DockSide::Left, DockSide::Right] {
            let slot = self.layout_blueprint(cx).agent_panel_slot(side);
            let uses_column = slot == AgentPanelSlot::Inner
                || (slot == AgentPanelSlot::Outer && split_active);
            if !uses_column {
                continue;
            }
            ...
            let size_state = dock
                ...
                .stored_panel_size_state(panel.as_ref())
                .unwrap_or_default();
            let needs_pixel_size = size_state.flex.is_some() || size_state.size.is_none();
            if !needs_pixel_size {
                continue;
            }
    ...
    pub(crate) fn agent_panel_dock(&self, cx: &App) -> Option<&Entity<Dock>> {
        self.all_docks().into_iter().find(|dock| {
            dock.read(cx)
                .panel_for_key(AGENT_PANEL_DOCK_KEY)
                .is_some()
        })
    }

    pub(crate) fn agent_panel_handle(&self, cx: &App) -> Option<Arc<dyn PanelHandle>> {
        self.agent_panel_dock(cx)?
            .read(cx)
            .panel_for_key(AGENT_PANEL_DOCK_KEY)
    }

    pub(crate) fn inner_agent_panel_width(
        &self,
        side: DockSide,
        window: &Window,
        cx: &App,
    ...
    fn inner_threads_strip_width(&self, side: DockSide, cx: &App) -> Pixels {
        if !self.is_inner_threads(side, cx) || !self.threads_sidebar_open(cx) {
            return Pixels::ZERO;
        }
        self.threads_column_width(side, cx)
    }

    pub(crate) fn horizontal_usage_on_side(
        &self,
        side: DockSide,
        window: &Window,
        cx: &App,
        ...
        if self.side_uses_column_row(side, cx) {
            let columns: Vec<_> = self
                ...
                .collect();
            let widths: Pixels = columns
                .iter()
                .map(|slot| self.column_width(side, slot.id, window, cx))
                .sum();
            let seams = COLUMN_GUTTER_HIT_SIZE * (columns.len() as f32 + 1.);
            return widths + seams;
        }

        let mut total = if self.is_split_outer_agent(side, cx) {
            self.agent_panel_column_width(window, cx) + COLUMN_GUTTER_HIT_SIZE * 2.
        } else {
            let mut usage = self.dock_visible_panel_width(side, window, cx);
            usage += self.inner_agent_panel_width(side, window, cx);
            usage
        };
        total += self.inner_threads_strip_width(side, cx);
        total
    }

    fn dock_visible_panel_width(
        &self,
        side: DockSide,
        window: &Window,
        cx: &App,
        ...
        dock.read(cx)
            .stored_visible_panel_size(window, cx)
            .unwrap_or(Pixels::ZERO)
    }

    fn inner_strips_width_on_side(
        &self,
        side: DockSide,
        window: &Window,
        cx: &App,
        ...
        if self.side_uses_column_row(side, cx) {
            self.resolved_side_columns(side, cx)
                .iter()
                .filter(|slot| slot.id != SideColumnKind::NavDock)
                .map(|slot| self.column_width(side, slot.id, window, cx))
                .sum()
        } else {
            self.inner_agent_panel_width(side, window, cx)
                + self.inner_threads_strip_width(side, cx)
        }
    }

    fn max_dock_width_on_side(
        &self,
        side: DockSide,
        window: &Window,
        cx: &App,
        ...
        split_column_max_dock_width(
            self.bounds.size.width,
            self.horizontal_usage_on_side(opposite_side, window, cx),
            self.inner_strips_width_on_side(side, window, cx),
            LayoutBlueprint::MIN_EDITOR_WIDTH,
        )
    }

    fn max_inner_agent_width_on_side(
        &self,
        side: DockSide,
        window: &Window,
        cx: &App,
        ...
        split_column_max_inner_agent_width(
            self.bounds.size.width,
            self.horizontal_usage_on_side(opposite_side, window, cx),
            self.dock_visible_panel_width(side, window, cx),
            self.inner_threads_strip_width(side, cx),
            LayoutBlueprint::MIN_EDITOR_WIDTH,
            MIN_INNER_AGENT_WIDTH,
        )
    }

    fn clamp_split_column_layout(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        for side in [DockSide::Left, DockSide::Right] {
            if self.side_uses_column_row(side, cx) {
                for slot in self.resolved_side_columns(side, cx) {
                    let current = self.column_width(side, slot.id, window, cx);
                    let max = match slot.id {
                        SideColumnKind::NavDock => self.max_dock_width_on_side(side, window, cx),
                        SideColumnKind::AgentPanel => {
                            self.max_inner_agent_width_on_side(side, window, cx)
                        }
                        SideColumnKind::ThreadsSidebar => continue,
                    };
                    if current > max {
                        self.apply_column_width(side, slot.id, max, window, cx);
                    }
                }
                continue;
            }

            let max_dock = self.max_dock_width_on_side(side, window, cx);
            let current_dock = self.dock_visible_panel_width(side, window, cx);
            if current_dock > max_dock {
                match side {
                    DockSide::Left => self.resize_left_dock(max_dock, window, cx),
                    DockSide::Right => self.resize_right_dock(max_dock, window, cx),
                }
            }

            if self.is_inner_agent(side, cx) {
                let max_agent = self.max_inner_agent_width_on_side(side, window, cx);
                let current_agent = self.inner_agent_panel_width(side, window, cx);
                if current_agent > max_agent {
                    self.resize_inner_agent_panel(side, max_agent, window, cx);
                }
            }
        }

        let workspace_width = self.bounds.size.width;
        let workspace_height = self.bounds.size.height;
        self.left_dock.update(cx, |dock, cx| {
            dock.clamp_panel_size(workspace_width, window, cx);
        });
        self.right_dock.update(cx, |dock, cx| {
            dock.clamp_panel_size(workspace_width, window, cx);
        });
        self.bottom_dock.update(cx, |dock, cx| {
            dock.clamp_panel_size(workspace_height, window, cx);
        });
    }

    fn resize_inner_agent_panel(
        &mut self,
        side: DockSide,
        new_width: Pixels,
        window: &mut Window,
    ...
    pub fn toggle_panel_focus<T: Panel>(
        ...
        // If the panel's dock is closed, toggling must always open + focus it.
        // Window focus can still linger on the panel's focus handle after its
        // dock closes, which would otherwise turn this toggle into a no-op.
        let dock_open = self
            .all_docks()
            .iter()
            .find_map(|dock| {
                let dock = dock.read(cx);
                dock.panel_index_for_type::<T>().map(|_| dock.is_open())
            })
            .unwrap_or(false);
        ...
        self.focus_or_unfocus_panel::<T>(window, cx, &mut |panel, window, cx| {
            did_focus_panel =
                !dock_open || !panel.panel_focus_handle(cx).contains_focused(window, cx);
            did_focus_panel
        });
    ...
    fn render_center_editor(
        &self,
        padding_sizes: (Option<f32>, Option<f32>),
        ...
        let render_padding = move |size: f32| {
            div()
                .h_full()
                .w(relative(size))
        ...
        h_flex()
            ...
            .overflow_hidden()
            .when_some(
                padding_sizes.0.filter(|&size| size > 0.0),
                |this, size| this.child(render_padding(size).border_r_1()),
            )
            .child(
                div()
                    .flex_1()
            ...
            .when_some(
                padding_sizes.1.filter(|&size| size > 0.0),
                |this, size| this.child(render_padding(size).border_l_1()),
            )
    }

    /// Side columns (nav / threads / agent) flanking the center stack: `[left…] | center | [right…]`.
    fn render_side_columns_and_center(
        &self,
        center: Div,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) -> Div {
        self.render_side_layout(
            DockSide::Right,
            self.render_side_layout(
                DockSide::Left,
                div()
            ...
            .child(center.flex_1().min_w_0().min_h_0().overflow_hidden().occlude()),
            window,
            cx,
        )
        .flex_1()
    ...
    fn render_dock(
        ...
        // Keep the dock mounted even when closed so its focus handle stays in the
        // element tree — status-bar panel buttons focus the dock handle before
        // dispatching toggle actions, and an unmounted handle breaks that dispatch
        // path. A closed dock renders empty and receives no sizing, so it
        // collapses to zero height/width.
        let dock_read = dock.read(cx);

        let leader_border = dock_read.active_panel().and_then(|panel| {
        ...
        let side_uses_column_row = dock_side
            .is_some_and(|side| self.side_uses_column_row(side, cx));
        let uses_editor_gutter = dock_side
            .is_some_and(|side| self.classic_dock_uses_editor_gutter(side, cx));
        let resize_mode = self.dock_panel_resize_mode(position, cx);
        let use_fixed_pixels =
            side_uses_column_row || uses_editor_gutter || resize_mode == PanelResizeMode::FixedPixels;

        let mut container = match position {
            DockPosition::Bottom => div().overflow_hidden().w_full().flex_shrink_0(),
            _ => div()
                .when(use_fixed_pixels, |this| this.relative().flex_shrink_0())
                .flex()
                .overflow_hidden()
                .flex_none(),
        }
        .child(
            // Dock renders `size_full` (sizing lives on this wrapper), so the
            // wrapper must always have a resolved size — an unsized wrapper
            // collapses the dock content to zero width in the classic flex path.
            div().size_full().overflow_hidden().child(dock.clone()),
        )
        .children(leader_border);

        let dock = dock.read(cx);
        let sizing_panel = dock
            .visible_panel()
            .map(|panel| panel.clone())
            .or_else(|| {
                dock.is_open()
                    .then(|| dock.active_panel().cloned())
                    .flatten()
            })
            .or_else(|| {
                dock.is_open()
                    .then(|| dock.panel_for_key("TerminalPanel"))
                    .flatten()
            });
        if let Some(panel) = sizing_panel.as_ref() {
            ...
            if position.axis() == Axis::Horizontal {
                ...
                let size = size_state
                    ...
                    .filter(|size| *size > Pixels::ZERO)
                    .unwrap_or_else(|| panel.default_size(window, cx))
                    .max(crate::layout_engine::MIN_BOTTOM_DOCK_HEIGHT);
                container = container.h(size).flex_shrink_0().w_full();
            }
        } else if position == DockPosition::Bottom && dock.is_open() {
            container = container
                .h(crate::layout_engine::MIN_BOTTOM_DOCK_HEIGHT.max(px(320.)))
                .flex_shrink_0()
                .w_full();
        }
    ...
    fn resize_left_dock(&mut self, new_size: Pixels, window: &mut Window, cx: &mut App) {
        let dock = self.left_dock.read(cx);
        let min_size = dock
            .visible_panel()
            .and_then(|panel| panel.min_size(window, cx))
            .unwrap_or(RESIZE_HANDLE_SIZE);
        let max_size = if self.split_layout_active(cx) {
            self.max_dock_width_on_side(DockSide::Left, window, cx)
        } else {
            self.bounds.size.width - RESIZE_HANDLE_SIZE
        };
        let size = new_size
            .min(max_size)
            .max(min_size)
            .min(self.bounds.size.width - RESIZE_HANDLE_SIZE);
        let mode = self.dock_panel_resize_mode(DockPosition::Left, cx);
        let flex_grow = if mode == PanelResizeMode::Dock {
            self.dock_flex_for_size(DockPosition::Left, size, window, cx)
        } else {
            None
        };
        self.left_dock.update(cx, |left_dock, cx| {
                ...
                .contains(&DockPosition::Left)
            {
                ...
                left_dock.resize_displayed_panel_with_mode(
                    Some(size),
                    flex_grow,
                    mode,
                    window,
    ...
    fn resize_right_dock(&mut self, new_size: Pixels, window: &mut Window, cx: &mut App) {
        if self.is_split_outer_agent(DockSide::Right, cx) {
            let size = new_size
                .min(self.max_inner_agent_width_on_side(DockSide::Right, window, cx))
                .max(MIN_INNER_AGENT_WIDTH);
            if let Some(dock) = self.agent_panel_dock(cx).cloned() {
                dock.update(cx, |dock, cx| {
                    dock.resize_panel_by_key_with_mode(
                        AGENT_PANEL_DOCK_KEY,
                        Some(size),
        ...
        let min_size = dock
            ...
            .unwrap_or(RESIZE_HANDLE_SIZE);
        let max_size = if self.split_layout_active(cx) {
            self.max_dock_width_on_side(DockSide::Right, window, cx)
        } else {
            self.bounds.size.width - RESIZE_HANDLE_SIZE
        };
        let size = new_size
            ...
            .max(min_size)
            .min(self.bounds.size.width - RESIZE_HANDLE_SIZE);
        let mode = self.dock_panel_resize_mode(DockPosition::Right, cx);
        let flex_grow = if mode == PanelResizeMode::Dock {
            self.dock_flex_for_size(DockPosition::Right, size, window, cx)
        } else {
            None
        };
        self.right_dock.update(cx, |right_dock, cx| {
                ...
                .contains(&DockPosition::Right)
            {
                ...
                right_dock.resize_displayed_panel_with_mode(
                    Some(size),
                    flex_grow,
                    mode,
                    window,
    ...
    fn resize_bottom_dock(&mut self, new_size: Pixels, window: &mut Window, cx: &mut App) {
        let size = new_size
            .max(crate::layout_engine::MIN_BOTTOM_DOCK_HEIGHT)
            .min(self.bounds.bottom() - RESIZE_HANDLE_SIZE - self.bounds.top());
...
impl Render for Workspace {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        self.sync_dock_layout_split(window, cx);
                ...
                div()
                    ...
                    .child(
                        div()
                                ...
                                canvas(
                                    move |bounds, window, cx| {
                                        this.update(cx, |this, cx| {
                                            ...
                                            if bounds_changed {
                                                this.clamp_split_column_layout(window, cx);
                            ...
                            .child({
                                let mut center_stack = |with_bottom_dock: bool| {
                                    let mut stack = div()
                                        .flex()
                                        .flex_col()
                                        .flex_1()
                                        ...
                                        .child(
                                            div()
                                                ...
                                                .h_full()
                                                .w_full()
                                                .child(self.render_center_editor(
                                                    padding_sizes,
                                                    &pane_render_context,
                                                    window,
                                                    cx,
                                    ...
                                    if with_bottom_dock {
                                        stack = stack.children(self.render_dock(
                                            DockPosition::Bottom,
                                            &self.bottom_dock,
                                            window,
                                            cx,
                                    ...
                                    self.render_side_columns_and_center(stack, window, cx)
                                };
                                ...
                                match bottom_dock_layout {
                                    BottomDockLayout::Full => div()
                                        ...
                                        .child(
                                            div()
                                                ...
                                                .min_w_0()
                                                .overflow_hidden()
                                                .child(center_stack(false)),
                                        )
                                        .child(
                                            div().w_full().flex_shrink_0().children(
                                                self.render_dock(
                                                    DockPosition::Bottom,
                                                    &self.bottom_dock,
                                                    window,
                                    ...
                                    BottomDockLayout::LeftAligned | BottomDockLayout::RightAligned => {
                                        div()
                                            .flex()
                                            .flex_row()
                                            .h_full()
...
actions!(
    ...
    [
        ...
        ///
        /// If you want to open a specific channel, use `cuecode::OpenAppUrl` with a channel notes URL -
...
actions!(
    ...
    [
        ...
        /// Opens the CueCode log file.
        OpenLog,
        #[action(deprecated_aliases = ["zed::RevealLogInFileManager"])]
        /// Reveals the CueCode log file in the system file manager.
                    ...
                    .update(cx, |_, window, cx| {
                        let detail: SharedString = match err.error_code() {
                            ...
                            ErrorCode::UpgradeRequired => concat!(
                                "Your are running an unsupported version of CueCode. ",
...
fn split_column_max_dock_width(
    workspace_width: Pixels,
    opposite_side_usage: Pixels,
    same_side_inner_strips: Pixels,
    min_editor_width: Pixels,
) -> Pixels {
    (workspace_width - opposite_side_usage - same_side_inner_strips - min_editor_width)
        .max(RESIZE_HANDLE_SIZE)
}

fn split_column_max_inner_agent_width(
    workspace_width: Pixels,
    opposite_side_usage: Pixels,
    same_side_dock_width: Pixels,
    same_side_threads_width: Pixels,
    min_editor_width: Pixels,
    min_agent_width: Pixels,
) -> Pixels {
    (workspace_width
        - opposite_side_usage
        - same_side_dock_width
        - same_side_threads_width
        - min_editor_width)
        .max(min_agent_width)
}
...
mod tests {
    ...
    #[test]
    fn split_column_max_dock_width_reserves_inner_strips_and_editor() {
        let max = split_column_max_dock_width(
            px(1200.),
            px(0.),
            px(400.),
            LayoutBlueprint::MIN_EDITOR_WIDTH,
        );
        assert_eq!(max, px(400.));
    }

    #[test]
    fn split_column_max_dock_width_with_opposite_side_usage() {
        let max = split_column_max_dock_width(
            px(1200.),
            px(300.),
            px(400.),
        ...
        assert_eq!(max, px(100.));
    }

    #[test]
    fn split_column_max_inner_agent_width_reserves_nav_and_editor() {
        let max = split_column_max_inner_agent_width(
            px(1200.),
            px(0.),
            px(240.),
            px(0.),
    ...
    async fn test_tracking_active_path(cx: &mut TestAppContext) {
        ...
        assert_eq!(cx.window_title().as_deref(), Some("CueCode — root1 — one.txt"));

        // Add a second item to a non-empty pane
        workspace.update_in(cx, |workspace, window, cx| {
            workspace.add_item_to_active_pane(Box::new(item2), None, true, window, cx)
        });
        assert_eq!(cx.window_title().as_deref(), Some("CueCode — root1 — two.txt"));
        project.update(cx, |project, cx| {
            assert_eq!(
                project.active_entry(),
                project
        ...
        project
            ...
            .await
            .unwrap();
        assert_eq!(cx.window_title().as_deref(), Some("CueCode — root1, root2 — one.txt"));

        // Remove a project folder
        project.update(cx, |project, cx| project.remove_worktree(worktree_id, cx));
        assert_eq!(cx.window_title().as_deref(), Some("CueCode — root2 — one.txt"));
    }
    ...
    async fn test_window_title_follows_active_workspace(cx: &mut TestAppContext) {
        ...
        let cx = &mut VisualTestContext::from_window(multi_workspace_handle.into(), cx);
        cx.run_until_parked();
        assert_eq!(cx.window_title().as_deref(), Some("CueCode — root1"));
        ...
        multi_workspace_handle
            ...
            .unwrap();
        cx.run_until_parked();
        assert_eq!(cx.window_title().as_deref(), Some("CueCode — root2"));
    ...
    async fn test_toggle_theme_mode_persists_and_updates_active_theme(cx: &mut TestAppContext) {
        ...
        use cuecode_actions::theme::ToggleMode;
