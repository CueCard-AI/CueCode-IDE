...
impl SidebarStatus {
    fn query(multi_workspace: &Option<WeakEntity<MultiWorkspace>>, cx: &App) -> Self {
        multi_workspace
            ...
            .map(|mw| {
                ...
                let state = mw.sidebar_render_state(cx);
                Self {
                    open: state.open,
                    side: state.side,
                    outer_tiled: state.outer_tiled,
    ...
    fn outer_tiled_on(&self, side: SidebarSide) -> bool {
                ...
                Decorations::Client { tiling, .. } => el
                    .when(
                        !(tiling.bottom || tiling.right)
                            && !sidebar.outer_tiled_on(SidebarSide::Right),
                        |el| el.rounded_br(CLIENT_SIDE_DECORATION_ROUNDING),
                    )
                    .when(
                        !(tiling.bottom || tiling.left)
                            && !sidebar.outer_tiled_on(SidebarSide::Left),
