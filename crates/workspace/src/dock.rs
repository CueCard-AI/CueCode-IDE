...
pub struct Dock {
    ...
    display_suppressed_panel_keys: Vec<&'static str>,
    hide_builtin_resize_handle: bool,
...
/// How [`resize_panel_entry`] interprets a resize request.
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq)]
pub enum PanelResizeMode {
    /// Respect [`PanelHandle::has_flexible_size`] (standard dock resize).
    #[default]
    Dock,
    /// Inner / split column: always write pixel width and clear flex.
    FixedPixels,
}
...
fn resize_panel_entry(
    ...
    match mode {
        ...
        PanelResizeMode::Dock if uses_flexible_width => {
...
impl Dock {
    pub fn new(
        ...
        let dock = cx.new(|cx| {
            ...
            Self {
                ...
                modal_layer,
                display_suppressed_panel_keys: Vec::new(),
                hide_builtin_resize_handle: false,
            }
        });
    ...
    fn visible_entry(&self) -> Option<&PanelEntry> {
        ...
        if let Some(active) = self.active_panel_entry() {
            if !self.is_panel_suppressed(active) {
                return Some(active);
            }
        }
        self.panel_entries
            .iter()
            .find(|entry| !self.is_panel_suppressed(entry))
    }

    fn is_panel_suppressed(&self, entry: &PanelEntry) -> bool {
        self.display_suppressed_panel_keys
            .contains(&entry.panel.panel_key())
    }

    pub fn set_display_suppressed_panel_keys(
        &mut self,
        keys: &[&'static str],
        cx: &mut Context<Self>,
    ) {
        if self.display_suppressed_panel_keys.as_slice() == keys {
            return;
        }
        self.display_suppressed_panel_keys.clear();
        self.display_suppressed_panel_keys.extend_from_slice(keys);
        cx.notify();
    }

    pub fn set_hide_builtin_resize_handle(
        &mut self,
        hide: bool,
        cx: &mut Context<Self>,
    ) {
        if self.hide_builtin_resize_handle == hide {
            return;
        }
        self.hide_builtin_resize_handle = hide;
        cx.notify();
    }

    pub fn panel_for_key(&self, panel_key: &str) -> Option<Arc<dyn PanelHandle>> {
        self.panel_entries
            .iter()
    ...
    pub fn resize_panel_by_key(
        ...
        self.resize_panel_by_key_with_mode(
            panel_key,
            size,
            flex,
            PanelResizeMode::Dock,
    ...
    pub fn resize_panel_by_key_with_mode(
        &mut self,
        panel_key: &'static str,
        size: Option<Pixels>,
        flex: Option<f32>,
        ...
        let (panel_key, size_state) =
            resize_panel_entry(self.position, entry, size, flex, mode, window, cx);
    ...
    pub fn stored_visible_panel_size(&self, window: &Window, cx: &App) -> Option<Pixels> {
        let panel = self.visible_panel()?;
        Some(
            self.stored_panel_size(panel.as_ref(), window, cx)
                .unwrap_or_else(|| panel.default_size(window, cx)),
        )
    }

    pub fn resize_visible_panel(
        &mut self,
        size: Option<Pixels>,
        flex: Option<f32>,
        window: &mut Window,
        ...
        self.resize_panel_by_key(panel_key, size, flex, window, cx);
    }

    /// Resizes the panel the user sees — visible when agent is suppressed, else active.
    pub fn resize_displayed_panel(
        &mut self,
        size: Option<Pixels>,
        flex: Option<f32>,
        window: &mut Window,
        ...
        self.resize_displayed_panel_with_mode(
            size,
            flex,
            PanelResizeMode::Dock,
            window,
    ...
    pub fn resize_displayed_panel_with_mode(
        &mut self,
        size: Option<Pixels>,
        flex: Option<f32>,
        mode: PanelResizeMode,
        ...
        let panel_key = if self.display_suppressed_panel_keys.is_empty() {
        ...
        self.resize_panel_by_key_with_mode(panel_key, size, flex, mode, window, cx);
    }
        ...
        if let Some(entry) = self.visible_entry() {
            ...
            div()
                ...
                .map(|this| match self.position() {
                    ...
                    // A pronounced divider between the editor and the bottom
                    // dock; border tokens are near-invisible in some themes.
                    DockPosition::Bottom => this
                        .border_t_2()
                        .border_color(cx.theme().colors().text_muted.opacity(0.4)),
                })
                .child(
                    div()
                        .overflow_hidden()
                ...
                .when(self.resizable(cx) && !self.hide_builtin_resize_handle, |this| {
