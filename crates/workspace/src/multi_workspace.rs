...
use gpui::{
    AnyView, App, Context, Entity, EntityId, EventEmitter, FocusHandle, Focusable,
...
use cuecode_actions::agents_sidebar::ToggleThreadSwitcher;

use agent_settings::{AgentSettings, LayoutBlueprint, ThreadsSidebarSlot};
use settings::{DockSide, SettingsStore, SidebarDockPosition};
use ui::{ContextMenu, right_click_menu};

use crate::open_remote_project_with_existing_connection;
use crate::{
    CloseIntent, CloseWindow, DockPosition, Event as WorkspaceEvent, Item,
...
pub struct SidebarRenderState {
    ...
    /// True when the threads sidebar is outer-tiled at the window edge (Layout Studio outer slot).
    pub outer_tiled: bool,
}

impl SidebarRenderState {
    pub fn outer_tiled_on(&self, side: SidebarSide) -> bool {
        self.open && self.outer_tiled && self.side == side
    }
}
...
impl MultiWorkspace {
    ...
    pub fn sidebar_render_state(&self, cx: &App) -> SidebarRenderState {
        let open = self.sidebar_open() && self.multi_workspace_enabled(cx);
        SidebarRenderState {
            open,
            side: self.sidebar_side(cx),
            outer_tiled: open && self.threads_sidebar_outer_tiled(cx),
        }
    }

    fn threads_sidebar_outer_tiled(&self, cx: &App) -> bool {
        let blueprint = LayoutBlueprint::from_merged_settings(
            cx.global::<SettingsStore>().merged_settings(),
        );
        let slot = match self.sidebar_side(cx) {
            SidebarSide::Left => blueprint.threads_sidebar_slot(DockSide::Left),
            SidebarSide::Right => blueprint.threads_sidebar_slot(DockSide::Right),
        };
        matches!(slot, ThreadsSidebarSlot::Outer)
    }
    ...
    pub(crate) fn threads_sidebar_width(&self, cx: &App) -> Pixels {
        self.sidebar
            .as_ref()
    ...
    pub(crate) fn render_outer_threads_column(
        ...
        if !self.multi_workspace_enabled(cx) || !self.sidebar_open() {
            return None;
        }

        let sidebar = self.sidebar.as_ref()?;
        // This runs during MultiWorkspace render (entity leased), so it must
        // not call Workspace helpers like `is_outer_threads` that read this
        // entity back — that double-lease panics. Read the blueprint directly.
        if !self.threads_sidebar_outer_tiled(cx) {
            return None;
        }

        let width = sidebar.width(cx);
        let content = div()
            .id("outer-threads-column")
            .size_full()
            .overflow_hidden()
            .child(sidebar.to_any())
            .into_any_element();

        Some(layout_engine::render_outer_threads_column(
            side,
            width,
            content,
            weak_workspace,
    ...
    fn apply_open_sidebar(&mut self, cx: &mut Context<Self>) {
        ...
        self.sync_workspace_layout_after_sidebar_toggle(cx);
        self.serialize(cx);
        cx.notify();
    }

    /// Inner threads columns are rendered by the workspace itself, and toggling
    /// the sidebar can flip a side between column-row and classic dock. The
    /// layout resolver reads this MultiWorkspace, so the sync must run deferred
    /// — outside the current entity lease — to avoid a double-lease panic.
    fn sync_workspace_layout_after_sidebar_toggle(&self, cx: &mut Context<Self>) {
        let workspaces = self.retained_workspaces.clone();
        cx.defer(move |cx| {
            for workspace in workspaces {
                workspace.update(cx, |workspace, cx| {
                    workspace.sync_side_column_dock_handles(cx);
                    cx.notify();
                });
            }
        });
...
impl Render for MultiWorkspace {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let sidebar: Option<AnyElement> = if multi_workspace_enabled && self.sidebar_open() {
            let side = if sidebar_on_right {
                DockSide::Right
            } else {
                DockSide::Left
            };
            self.render_outer_threads_column(side, self.workspace().downgrade(), cx)
        } else {
            None
        };

        let (left_sidebar, right_sidebar) = match if sidebar_on_right {
                blueprint.threads_sidebar_slot(DockSide::Right)
            } else {
                blueprint.threads_sidebar_slot(DockSide::Left)
            } {
                ThreadsSidebarSlot::Absent => (None, None),
                ThreadsSidebarSlot::Outer if sidebar_on_right => (None, sidebar),
                ThreadsSidebarSlot::Outer => (sidebar, None),
                ThreadsSidebarSlot::Inner => (None, None),
            };

        let tile_left = left_sidebar.is_some();
        let tile_right = right_sidebar.is_some();
