...
#[cfg(target_os = "macos")]
fn main() {
    // Set CUECODE_STATELESS early to prevent file system access to real config directories
    // This must be done before any code accesses cuecode_env_vars::CUECODE_STATELESS
    // SAFETY: We're at the start of main(), before any threads are spawned
    unsafe {
        std::env::set_var("CUECODE_STATELESS", "1");
    }
...
use {
    ...
    cuecode_actions::OpenSettingsAt,
};
...
mod constants {
    ...
    pub const BASELINE_DIR: &str = "crates/cuecode/test_fixtures/visual_tests";
...
fn run_tool_permissions_visual_tests(
    ...
    use cuecode_actions::OpenSettingsAt;
