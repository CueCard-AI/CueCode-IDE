...
use client::{Client, ProxySettings, RefreshLlmTokenListener, UserStore, app_urls, parse_zed_link};
...
fn build_application() -> Application {
    ...
    if env_var::read_bool_env("CUECODE_EXPERIMENTAL_A11Y", "ZED_EXPERIMENTAL_A11Y") {
...
fn files_not_created_on_launch(errors: HashMap<io::ErrorKind, Vec<&Path>>) {
    let message = "CueCode failed to launch";
...
fn fail_to_open_window(e: anyhow::Error, _cx: &mut App) {
    eprintln!(
        "CueCode failed to open a window: {e:?}."
    );
    ...
    #[cfg(any(target_os = "linux", target_os = "freebsd"))]
    {
        ...
        _cx.spawn(async move |_cx| {
            ...
            proxy
                .add_notification(
                    notification_id,
                    Notification::new("CueCode failed to launch")
                        .body(Some(
                            format!(
                                "{e:?}. See https://cuecode.dev/docs/linux for troubleshooting steps."
                            )
                            .as_str(),
                        ))
...
fn main() {
    ...
    // `cuecode --askpass` Makes cuecode operate in nc/netcat mode for use with askpass
    ...
    // `cuecode --crash-handler` Makes cuecode operate in minidump crash handler mode
    ...
    // `cuecode --printenv` Outputs environment variables as JSON to stdout
    if args.printenv {
    ...
    match util::get_zed_cli_path() {
        ...
        Err(err) => {
            eprintln!("Error: {}", err);
            if paths::compile_time_bundle_is_set() {
                process::exit(1);
            }
        }
    }
    ...
    let app_commit_sha = option_env!("CUECODE_COMMIT_SHA")
    ...
    let failed_single_instance_check = if *cuecode_env_vars::CUECODE_STATELESS
    ...
    let should_install_crash_handler = matches!(
        env_var::read_env_var("CUECODE_GENERATE_MINIDUMPS", "ZED_GENERATE_MINIDUMPS")
            .value
            .as_deref(),
        Some("true" | "1")
    ) || *release_channel::RELEASE_CHANNEL
    ...
    let git_binary_path =
        if cfg!(target_os = "macos") && paths::compile_time_bundle_is_true() {
                    ...
                    multi_workspace.update(cx, |multi_workspace, window, cx| {
                        multi_workspace.workspace().update(cx, |workspace, cx| {
                            if let Some(panel) = workspace.focus_panel::<AgentPanel>(window, cx) {
                                ...
                                log::warn!(
                                    "cuecode://agent received but the AgentPanel is not registered \
                ...
                workspace::with_active_or_new_workspace(cx, |_workspace, window, cx| {
                    cx.spawn_in(window, async move |workspace, cx| {
                        let res = async move {
                            ...
                            let uri = format!("cuecode://schemas/{}", schema_path);
            ...
            OpenRequestKind::Setting { setting_path } => {
                ...
                cx.spawn(async move |cx| {
                    ...
                    workspace.update(cx, |_, window, cx| match setting_path {
                        None => window.dispatch_action(Box::new(cuecode_actions::OpenSettings), cx),
...
async fn authenticate(_client: Arc<Client>, _cx: &AsyncApp) -> Result<()> {
    // CueCode v1: no automatic zed.dev sign-in on launch.
    Ok(())
}
...
#[derive(Parser, Debug)]
#[command(name = "cuecode", disable_version_flag = true, max_term_width = 100)]
struct Args {
    ...
    ///
    /// URLs can use `file://`, `cuecode://`, or legacy `zed://` scheme.
    ...
    /// On macOS, the default is `~/Library/Application Support/CueCode`.
    /// On Linux/FreeBSD, the default is `$XDG_DATA_HOME/cuecode`.
    /// On Windows, the default is `%LOCALAPPDATA%\CueCode`.
    #[arg(long, value_name = "DIR", verbatim_doc_comment)]
    user_data_dir: Option<String>,

    /// The username and WSL distribution to use when opening paths. If not specified,
    /// CueCode will attempt to open the paths directly.
    ///
    ...
    /// Instructs CueCode to run as a dev server on this machine. (not implemented)
    #[arg(long)]
    ...
    /// prevents CueCode from starting, so you can't run `cuecode: copy system specs to
    /// clipboard`
    #[arg(long)]
    system_specs: bool,

    /// Used for recording minidumps on crashes by having CueCode run a separate
    /// process communicating over a socket.
    #[arg(long, hide = true)]
    crash_handler: Option<PathBuf>,

    /// Run CueCode in the foreground, only used on Windows, to match the behavior on macOS.
    #[arg(long)]
    #[cfg(target_os = "windows")]
    #[arg(hide = true)]
    foreground: bool,
