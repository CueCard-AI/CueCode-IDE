...
use cuecode_actions::{
    ...
    OpenStatusPage, OpenAppUrl, Quit, ReloadWindow,
};

const STATUS_URL: &str = "https://status.cuecode.dev";
...
pub fn init(cx: &mut App) {
    ...
    cx.on_action(|_: &cuecode_actions::OpenLicenses, cx| {
        with_active_or_new_workspace(cx, |workspace, window, cx| {
            open_bundled_file(
                workspace,
    ...
    .on_action(|&cuecode_actions::OpenKeymapFile, cx| {
    ...
    .on_action(|_: &OpenAccountSettings, _cx| {
        // CueCode v1: no cloud account portal.
    })
    ...
    .on_action(|_: &cuecode_actions::OpenDefaultKeymap, cx| {
        with_active_or_new_workspace(cx, |workspace, window, cx| {
            open_bundled_file(
                workspace,
...
pub fn build_window_options(display_uuid: Option<Uuid>, cx: &mut App) -> WindowOptions {
    ...
    let window_decorations = match env_var::read_env_var("CUECODE_WINDOW_DECORATIONS", "ZED_WINDOW_DECORATIONS").value.as_deref() {
...
fn initialize_file_watcher(window: &mut Window, cx: &mut Context<Workspace>) {
    if let Err(e) = fs::fs_watcher::global(|_| {}) {
        let message = format!(
            ...
            This may be due to system-wide limits on inotify instances. For troubleshooting see: {docs}
            "#},
            e,
            docs = release_channel::CUECODE_DOCS_URL.to_owned() + "/linux",
        );
            ...
            if prompt.await == Ok(0) {
                cx.update(|cx| {
                    cx.open_url(&format!(
                        ...
                        release_channel::CUECODE_DOCS_URL
                    ));
                    cx.quit();
                });
            }
...
fn initialize_file_watcher(window: &mut Window, cx: &mut Context<Workspace>) {
    if let Err(e) = fs::fs_watcher::global(|_| {}) {
        let message = format!(
            ...
            This may occur on network filesystems and WSL paths. For troubleshooting see: {docs}
            "#},
            docs = format!("{}/windows", release_channel::CUECODE_DOCS_URL)
            e
        );
        let prompt = window.prompt(
            PromptLevel::Critical,
        ...
        cx.spawn(async move |_, cx| {
            if prompt.await == Ok(0) {
                cx.update(|cx| {
                    cx.open_url(&format!("{}/windows", release_channel::CUECODE_DOCS_URL));
                    cx.quit()
                });
            }
        })
...
fn show_software_emulation_warning_if_needed(
    ...
    if specs.is_software_emulated && std::env::var("ZED_ALLOW_EMULATED_GPU").is_err() {
        let docs_base = release_channel::CUECODE_DOCS_URL;
        let (graphics_api, docs_url, open_url) = if cfg!(target_os = "windows") {
            (
                "DirectX",
                format!("{docs_base}/windows"),
                format!("{docs_base}/windows"),
            )
        } else {
            (
                "Vulkan",
                format!("{docs_base}/linux"),
                format!("{docs_base}/linux#zed-fails-to-open-windows"),
            )
        };
        let message = format!(
            db::indoc! {r#"
            CueCode uses {} for rendering and requires a compatible GPU.
...
fn register_actions(
    ...
    workspace
        .register_action(|_, _: &OpenDocs, _, cx| {
            cx.open_url(&release_channel::ReleaseChannel::global(cx).docs_url(""))
        })
        ...
        .register_action(|workspace, action: &cuecode_actions::OpenRemote, window, cx| {
        ...
        .register_action({
            ...
            move |_, action: &cuecode_actions::IncreaseUiFontSize, _window, cx| {
                if action.persist {
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &cuecode_actions::DecreaseUiFontSize, _window, cx| {
                if action.persist {
                    update_settings_file(fs.clone(), cx, move |settings, cx| {
                        let ui_font_size = ThemeSettings::get_global(cx).ui_font_size(cx) - px(1.0);
                        let _ = settings
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &cuecode_actions::ResetUiFontSize, _window, cx| {
                if action.persist {
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &cuecode_actions::IncreaseBufferFontSize, _window, cx| {
                if action.persist {
                    update_settings_file(fs.clone(), cx, move |settings, cx| {
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &cuecode_actions::DecreaseBufferFontSize, _window, cx| {
                if action.persist {
                    update_settings_file(fs.clone(), cx, move |settings, cx| {
                        let buffer_font_size =
                            ThemeSettings::get_global(cx).buffer_font_size(cx) - px(1.0);
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &cuecode_actions::ResetBufferFontSize, _window, cx| {
                if action.persist {
                    update_settings_file(fs.clone(), cx, move |settings, _| {
        ...
        .register_action({
            let fs = app_state.fs.clone();
            move |_, action: &cuecode_actions::ResetAllZoom, _window, cx| {
                if action.persist {
                    update_settings_file(fs.clone(), cx, move |settings, _| {
                        settings.theme.ui_font_size = None;
                        settings.theme.buffer_font_size = None;
        ...
        .register_action(|_, _: &install_cli::RegisterAppUrlSchemes, window, cx| {
            cx.spawn_in(window, async move |workspace, cx| {
                install_cli::register_zed_scheme(cx).await?;
                workspace.update_in(cx, |workspace, _, cx| {
                    struct RegisterAppUrlSchemes;

                    workspace.show_toast(
                        Toast::new(
                            NotificationId::unique::<RegisterAppUrlSchemes>(),
                            format!(
                                "cuecode:// links (and legacy zed://) will now open in {}.",
            ...
            .detach_and_prompt_err(
                "Error registering cuecode:// scheme",
                window,
                cx,
                |_, _, _| None,
            );
        ...
        .register_action(
            |workspace: &mut Workspace,
             _: &cuecode_actions::project_panel::ToggleFocus,
             window: &mut Window,
             cx: &mut Context<Workspace>| {
...
fn open_about_window(cx: &mut App) {
                ...
                v_flex()
                    ...
                    .child(
                        v_flex()
                            ...
                            .child(
                                Label::new("Local-first agent sandbox — no zed.dev account required")
                                    .color(Color::Muted)
                                    .size(LabelSize::XSmall),
                            ),
                    )
    ...
    cx.open_window(
        WindowOptions {
            titlebar: Some(TitlebarOptions {
            title: Some(format!("About {}", paths::APP_NAME).into()),
                appears_transparent: true,
...
fn reload_window(_: &ReloadWindow, cx: &mut App) {
    cx.restart();
}
                        ...
                        MessageNotification::new(format!("Invalid user settings file\n{error}"), cx)
                            ...
                            .primary_on_click(|window, cx| {
                                window.dispatch_action(
                                    cuecode_actions::OpenSettingsFile.boxed_clone(),
                                    cx,
                                );
            ...
            if !showed_parse_error {
                show_app_notification(id, cx, move |cx| {
                    cx.new(|cx| {
                        ...
                        .primary_icon(IconName::Settings)
                        .primary_on_click(|window, cx| {
                            window.dispatch_action(cuecode_actions::OpenSettingsFile.boxed_clone(), cx);
                            cx.emit(DismissEvent);
                        })
                    })
                });
...
/// Starts watching `~/.config/cuecode/AGENTS.md` (or the platform equivalent) and
    ...
    show_app_notification(notification_id, cx, move |cx| {
        cx.new(|cx| {
            MessageNotification::new(message.clone(), cx)
                ...
                .primary_icon(IconName::Settings)
                .primary_on_click(|window, cx| {
                    window.dispatch_action(cuecode_actions::OpenKeymapFile.boxed_clone(), cx);
                    cx.emit(DismissEvent);
                })
        })
    });
...
fn open_project_debug_tasks_file(
    workspace: &mut Workspace,
    _: &cuecode_actions::OpenProjectDebugTasks,
    window: &mut Window,
    cx: &mut Context<Workspace>,
) {
    open_local_file(
