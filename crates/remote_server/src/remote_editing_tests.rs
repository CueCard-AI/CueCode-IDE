...
async fn test_adding_remote_skill(cx: &mut TestAppContext, server_cx: &mut TestAppContext) {
    ...
    let expected = format!(
        ...
        path!("/project/.cursor/skills/test-skill"),
    );
    assert_eq!(output, SkillToolOutput::Found { rendered: expected });

    fs.create_dir(Path::new(path!("/project/.cursor/skills/test-2")))
        .await
        .unwrap();
    fs.insert_file(
        path!("/project/.cursor/skills/test-2/SKILL.md"),
        "---\nname: test-2\ndescription: test description\n---\ntest body"
            .as_bytes()
            .into(),
    )
    ...
    let expected2 = format!(
        ...
        path!("/project/.cursor/skills/test-2"),
    );
    assert_eq!(
        output,
