...
mod tests {
    ...
    /// `load_bundled_themes` parses with strict `serde_json`, so every theme
    /// family shipped in `assets/themes` must deserialize without errors or
    /// it will be silently skipped at startup.
    #[test]
    fn bundled_theme_families_parse() {
        let themes_dir =
            std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../../assets/themes");
        let mut families = Vec::new();
        for entry in std::fs::read_dir(&themes_dir).expect("assets/themes must exist") {
            ...
            for file in std::fs::read_dir(&dir).unwrap() {
                ...
                if path.extension().is_some_and(|ext| ext == "json") {
                    ...
                    assert!(
                        !family.themes.is_empty(),
                        "theme family {path:?} has no themes"
                    );
                    families.push(family.name.clone());
                }
            }
        }
        for expected in ["One", "Ayu", "Gruvbox", "Dracula"] {
            assert!(
                families.iter().any(|name| name == expected),
                "bundled theme family {expected:?} missing (found: {families:?})"
            );
        }
    }
