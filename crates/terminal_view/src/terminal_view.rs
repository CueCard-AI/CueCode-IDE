...
use cuecode_actions::{agent::AddSelectionToThread, assistant::InlineAssist};
...
impl TerminalView {
    ...
    /// Cursor-style floating "Add to Chat" pill anchored above the terminal
    /// selection. Clicking it (or the shown keybinding) sends the selection to
    /// the agent message editor via `agent::AddSelectionToThread`.
    fn render_add_selection_pill(&self, cx: &mut Context<Self>) -> Option<AnyElement> {
        if matches!(self.mode, TerminalMode::Embedded { .. }) {
            return None;
        }

        let terminal = self.terminal.read(cx);
        // Wait until the mouse is released so the pill doesn't chase the drag.
        ...
        // Anchor above the first selected line. A selection that starts above
        // the viewport pins to the top edge; one entirely below it shows no pill.
        let viewport_line =
            viewport_line_for_point(selection.start, content.display_offset).unwrap_or(0);
        if viewport_line >= bounds.num_lines() {
            return None;
        }
        let position = gpui::point(
            bounds.bounds.origin.x + selection.start.column as f32 * bounds.cell_width,
            bounds.bounds.origin.y + viewport_line as f32 * bounds.line_height,
        );
        ...
        Some(
            deferred(
                anchored()
                    ...
                    .child(
                        div().occlude().pb_0p5().child(
                            Button::new("terminal-add-selection-to-chat", "Add to Chat")
                                ...
                                .on_click(cx.listener(|_, _, window, cx| {
                                    window
                                        .dispatch_action(AddSelectionToThread.boxed_clone(), cx);
                                })),
                        ),
                    ),
            )
...
fn terminal_rerun_override(task: &TaskId) -> cuecode_actions::Rerun {
    cuecode_actions::Rerun {
...
impl Render for TerminalView {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        div()
            ...
            .children(self.render_add_selection_pill(cx))
