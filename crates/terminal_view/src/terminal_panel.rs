...
use cuecode_actions::assistant::InlineAssist;
...
pub struct TerminalPanel {
    ...
    active: bool,
    /// VS Code-style terminal list: tabs hidden, terminals listed in a side column.
...
impl TerminalPanel {
    ...
    fn set_list_view(&mut self, list_view: bool, cx: &mut Context<Self>) {
        if self.list_view == list_view {
            return;
        }
    ...
    pub(crate) fn apply_tab_bar_buttons(
        ...
        let terminal_panel = cx.entity().downgrade();
            ...
            pane.set_render_tab_bar_buttons(cx, move |pane, window, cx| {
                ...
                let right_children = h_flex()
                            ...
                            .menu(move |window, cx| {
                                ...
                                let menu = ContextMenu::build(window, cx, |menu, _, _| {
                                    menu.context(focus_handle.clone())
                                        ...
                                        .action(
                                            "Spawn Task",
                                            cuecode_actions::Spawn::modal().boxed_clone(),
                                        )
                                });
                    ...
                    .child({
                        ...
                        IconButton::new("terminal-list-view", IconName::ListTree)
                            .icon_size(IconSize::Small)
                            .tooltip(Tooltip::text("Show Terminal List"))
                            .on_click(move |_, _, cx| {
                                if let Some(terminal_panel) = terminal_panel.upgrade() {
                                    terminal_panel.update(cx, |terminal_panel, cx| {
                                        terminal_panel.set_list_view(true, cx);
                                    });
                                }
                            })
                    })
    ...
    fn serialize(&mut self, cx: &mut Context<Self>) {
        ...
        self.pending_serialization = cx.spawn(async move |terminal_panel, cx| {
            ...
            let (items, list_view) = terminal_panel.update(cx, |terminal_panel, cx| {
                (
...
pub fn new_terminal_pane(
    ...
    let pane = cx.new(|cx| {
        ...
        let tab_bar_terminal_panel = terminal_panel.downgrade();
        pane.set_should_display_tab_bar(move |_, cx| {
            // Tabs are replaced by the side list while list view is active.
            tab_bar_terminal_panel
                .read_with(cx, |terminal_panel, _| !terminal_panel.list_view)
                .unwrap_or(true)
        });
        ...
        let popover_menu = PopoverMenu::new("settings-popover")
            ...
            .menu(move |window, cx| {
                Some(ContextMenu::build(window, cx, |context_menu, _, _| {
                    context_menu
                        .action("Open Settings", cuecode_actions::OpenSettings.boxed_clone())
                        .action(
                            "Edit settings.json",
                    ...
                    .child(SplitButton::new(
                        ButtonLike::new("open-settings-ui")
                            ...
                            .on_click(|_, window, cx| {
                                window.dispatch_action(cuecode_actions::OpenSettings.boxed_clone(), cx);
                            }),
...
impl TerminalPanel {
    /// VS Code-style side column listing all terminals in the panel.
    fn render_terminal_list(&self, cx: &mut Context<Self>) -> impl IntoElement {
        let colors = cx.theme().colors().clone();
        let mut rows: Vec<AnyElement> = Vec::new();
        for pane in self.center.panes() {
            let pane = pane.clone();
            let pane_read = pane.read(cx);
            let active_ix = pane_read.active_item_index();
            let pane_is_active = pane == self.active_pane;
            for (ix, item) in pane_read.items().enumerate() {
                let title = item.tab_content_text(0, cx);
                let item_id = item.item_id();
                let is_active = pane_is_active && ix == active_ix;
                let activate_pane = pane.clone();
                let close_pane = pane.clone();
                rows.push(
                    h_flex()
                        .id(("terminal-list-item", item_id.as_u64() as usize))
                        .group("terminal-list-row")
                        .w_full()
                        .gap_1()
                        .px_1p5()
                        .py_0p5()
                        ...
                        .child(
                            IconButton::new(
                                ("terminal-list-close", item_id.as_u64() as usize),
                                IconName::Close,
                            )
                            .icon_size(IconSize::XSmall)
                            .visible_on_hover("terminal-list-row")
                        ...
                        .on_click(cx.listener(move |_, _, window, cx| {
                            activate_pane.update(cx, |pane, cx| {
                                let ix = pane
                                    .items()
                ...
                h_flex()
                    ...
                    .child(
                        h_flex()
                            .gap_0p5()
                            .child(
                                IconButton::new("terminal-list-new", IconName::Plus)
                                    .icon_size(IconSize::Small)
                            ...
                            .child(
                                IconButton::new("terminal-list-view-off", IconName::ListTree)
                                    .icon_size(IconSize::Small)
                                    .toggle_state(true)
                                    .tooltip(Tooltip::text("Show Terminal Tabs"))
                                    .on_click(cx.listener(|this, _, _, cx| {
                                        this.set_list_view(false, cx);
                                    })),
                            ),
                    ),
            )
...
impl Render for TerminalPanel {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        let registrar = cx
            ...
            .unwrap_or_else(div);
        let terminal_list = self.list_view.then(|| {
            // Seam-style gutter (matching workspace column seams) so the list
            // column reads as separate chrome instead of a hairline border.
            let colors = cx.theme().colors();
            // `text_muted` always contrasts with the background, unlike border
            // tokens which are near-invisible in some themes (e.g. Dracula).
            let seam = div()
                .w(px(6.))
                .h_full()
                .flex_shrink_0()
                ...
                .flex()
                .justify_center()
                .child(div().w(px(2.)).h_full().bg(colors.text_muted.opacity(0.4)))
                .into_any_element();
            (seam, self.render_terminal_list(cx).into_any_element())
        });
        self.workspace
            .update(cx, |workspace, cx| {
                registrar
                    ...
                    .children(terminal_list.into_iter().flat_map(|(seam, list)| [seam, list]))
            })
            .ok()
            .map(|div| {
                div.on_action({
