...
impl MentionUri {
    pub fn parse(input: &str, path_style: PathStyle) -> Result<Self> {
        ...
        match url.scheme() {
            ...
            "zed" => {
                if let Some(thread_id) = path.strip_prefix("/agent/thread/") {
                ...
                } else if path.starts_with("/agent/spec") {
                    let mut title = None;
                    ...
                    for (key, value) in url.query_pairs() {
                        match key.as_ref() {
                            "title" => {
                                if title.replace(value.to_string()).is_some() {
                                    bail!("duplicate spec title query parameter");
                                }
                            }
                            "path" => {
                                if relative_path
                                    ...
                                    .is_some()
                                {
                                    bail!("duplicate spec path query parameter");
                                }
                            }
                            _ => bail!("invalid query parameter"),
                        }
                    ...
                    Ok(Self::Spec {
                        title: title.context("missing spec title")?,
                        relative_path: relative_path.context("missing spec path")?,
                    })
                } else {
    ...
    pub fn name(&self) -> String {
        match self {
            ...
            MentionUri::Spec { title, .. } => title.clone(),
        }
    }
    ...
    pub fn tooltip_text(&self) -> Option<SharedString> {
        match self {
            ...
            MentionUri::Spec { relative_path, .. } => {
                Some(relative_path.to_string_lossy().into_owned().into())
            }
            _ => None,
        }
    }
    ...
    pub fn icon_path(&self, cx: &mut App) -> SharedString {
        match self {
            ...
            MentionUri::Spec { .. } => IconName::Reader.path().into(),
        }
    }
    ...
    pub fn to_uri(&self) -> Url {
        match self {
            ...
            MentionUri::Spec {
                ...
                let mut url = Url::parse("zed:///").unwrap();
                url.set_path("/agent/spec");
                url.query_pairs_mut()
                    .append_pair("title", title)
                    .append_pair("path", &relative_path.to_string_lossy());
                url
            }
        }
    }
