//! Paths to locations used by CueCode.
...
/// On macOS, this is `~/Library/Application Support/CueCode`.
/// On Linux/FreeBSD, this is `$XDG_DATA_HOME/cuecode`.
/// On Windows, this is `%LOCALAPPDATA%\CueCode`.
static CURRENT_DATA_DIR: OnceLock<PathBuf> = OnceLock::new();

/// The resolved config directory, combining custom override or platform defaults.
/// This is set once and cached for subsequent calls.
/// On macOS, this is `~/.config/cuecode`.
/// On Linux/FreeBSD, this is `$XDG_CONFIG_HOME/cuecode`.
/// On Windows, this is `%APPDATA%\CueCode`.
...
/// Returns the path to the configuration directory used by CueCode.
...
/// Returns the path to the data directory used by CueCode.
...
/// Returns the path to the primary log file (`CueCode.log`).
pub fn log_file() -> &'static PathBuf {
    static LOG_FILE: OnceLock<PathBuf> = OnceLock::new();
    LOG_FILE.get_or_init(|| logs_dir().join(format!("{}.log", APP_NAME)))
}

/// Returns the path to the rotated log file (`CueCode.log.old`).
...
/// Returns the legacy Zed project settings folder name (read-only compatibility).
pub fn legacy_local_settings_folder_name() -> &'static str {
    ".zed"
}

/// Returns the CueCode project settings folder name used for new project files.
...
/// Returns the legacy `.zed/settings.json` path (read-only compatibility).
...
/// Returns the relative path to a project `settings.json` file.
pub fn local_settings_file_relative_path() -> &'static RelPath {
    static CACHED: LazyLock<&'static RelPath> =
        LazyLock::new(|| RelPath::unix(".cuecode/settings.json").unwrap());
    *CACHED
}

/// Returns the legacy `.zed/tasks.json` path (read-only compatibility).
...
/// Returns the relative path to a project `tasks.json` file.
pub fn local_tasks_file_relative_path() -> &'static RelPath {
    static CACHED: LazyLock<&'static RelPath> =
        LazyLock::new(|| RelPath::unix(".cuecode/tasks.json").unwrap());
    *CACHED
}
...
/// Returns the legacy `.zed/debug.json` path (read-only compatibility).
...
/// Returns the relative path to a project `debug.json` file.
pub fn local_debug_file_relative_path() -> &'static RelPath {
    static CACHED: LazyLock<&'static RelPath> =
        LazyLock::new(|| RelPath::unix(".cuecode/debug.json").unwrap());
    *CACHED
}

/// Returns true when `name` is a project local-settings folder (`.cuecode` or legacy `.zed`).
pub fn is_local_settings_folder_name(name: &str) -> bool {
    name.eq_ignore_ascii_case(local_settings_folder_name())
        || name.eq_ignore_ascii_case(legacy_local_settings_folder_name())
}

/// Returns true when `path` points at a project settings file in either `.cuecode` or legacy `.zed`.
pub fn is_local_settings_file(path: &RelPath) -> bool {
    path.ends_with(local_settings_file_relative_path())
        || path.ends_with(legacy_local_settings_file_relative_path())
}

/// Returns true when `path` points at a project tasks file in either `.cuecode` or legacy `.zed`.
pub fn is_local_tasks_file(path: &RelPath) -> bool {
    path.ends_with(local_tasks_file_relative_path())
        || path.ends_with(legacy_local_tasks_file_relative_path())
}

/// Returns true when `path` points at a project debug file in either `.cuecode` or legacy `.zed`.
pub fn is_local_debug_file(path: &RelPath) -> bool {
    path.ends_with(local_debug_file_relative_path())
        || path.ends_with(legacy_local_debug_file_relative_path())
}

/// Returns the project settings directory ancestor for a settings/tasks/debug file path.
pub fn local_settings_directory_for_file(path: &RelPath) -> Option<Arc<RelPath>> {
    ...
    if path.ends_with(legacy_local_settings_file_relative_path()) {
        return path
            .ancestors()
            .nth(legacy_local_settings_file_relative_path().components().count())
            .map(Into::into);
    }
    if path.ends_with(local_tasks_file_relative_path()) || path.ends_with(legacy_local_tasks_file_relative_path()) {
        let components = if path.ends_with(local_tasks_file_relative_path()) {
            local_tasks_file_relative_path().components().count()
        } else {
            legacy_local_tasks_file_relative_path().components().count()
        };
        return path
            .ancestors()
            .nth(components.saturating_sub(1))
            .map(Into::into);
    }
    if path.ends_with(local_debug_file_relative_path()) || path.ends_with(legacy_local_debug_file_relative_path()) {
        let components = if path.ends_with(local_debug_file_relative_path()) {
            local_debug_file_relative_path().components().count()
        } else {
            legacy_local_debug_file_relative_path().components().count()
        };
        return path
            .ancestors()
            .nth(components.saturating_sub(1))
...
/// Returns true when this build was produced as a bundled app release.
///
/// Prefer `CUECODE_BUNDLE`; `ZED_BUNDLE` is accepted for backward compatibility.
pub fn compile_time_bundle_is_set() -> bool {
    option_env!("CUECODE_BUNDLE").is_some() || option_env!("ZED_BUNDLE").is_some()
}

/// Returns true when bundled as a macOS `.app` with auxiliary executables.
pub fn compile_time_bundle_is_true() -> bool {
    matches!(option_env!("CUECODE_BUNDLE"), Some("true"))
        || matches!(option_env!("ZED_BUNDLE"), Some("true"))
}
