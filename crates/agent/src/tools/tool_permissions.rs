...
/// either inside a `.zed/` local-settings directory, inside `.cursor/skills/`
/// (or legacy `.agents/skills/`), or inside
...
pub async fn sensitive_settings_kind(
    ...
    let legacy_local_settings_folder = paths::legacy_local_settings_folder_name();
    ...
    // `.cuecode/` or legacy `.zed/` or project skills (`.cursor/skills/`, legacy `.agents/skills/`).
    if path.components().any(|component| {
        let name = component.as_os_str();
        component_matches_ignore_ascii_case(name, local_settings_folder)
            || component_matches_ignore_ascii_case(name, legacy_local_settings_folder)
    }) {
