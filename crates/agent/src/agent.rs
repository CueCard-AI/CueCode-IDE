                    ...
                    match err {
                        ...
                        _ => {
                            ...
                            match provider_id.0.as_ref() {
                                "lmstudio" | "ollama" | "openai-subscribed" => {
                                    // LM Studio and Ollama both make fetch requests to the local APIs to determine if they are "authenticated".
                                    // OpenAI Subscription requires ChatGPT sign-in, which CueCode does not enable by default.
                                    //
...
impl NativeAgent {
    ...
    fn register_session(
        ...
        let active_spec_path = thread.active_spec_path().map(PathBuf::from);
        let active_ticket_id = thread.active_ticket_id().map(str::to_owned);
        ...
        let acp_thread = cx.new(|cx| {
            ...
            acp_thread.set_active_spec_path(active_spec_path, cx);
            acp_thread.set_active_ticket_id(active_ticket_id, cx);
        ...
        let acp_thread_for_observer = acp_thread.clone();
        let subscriptions = vec![
            cx.subscribe(&thread_handle, Self::handle_thread_title_updated),
            cx.subscribe(&thread_handle, Self::handle_thread_token_usage_updated),
            cx.observe(&thread_handle, move |this, thread, cx| {
                let active_spec = thread.read(cx).active_spec_path().map(PathBuf::from);
                let active_ticket = thread.read(cx).active_ticket_id().map(str::to_owned);
                acp_thread_for_observer.update(cx, |acp_thread, cx| {
                    acp_thread.set_active_spec_path(active_spec, cx);
                    acp_thread.set_active_ticket_id(active_ticket, cx);
                });
    ...
    fn register_project_with_initial_context(
        ...
        Self::ensure_spec_indices(&project, cx);
    ...
    fn ensure_spec_indices(project: &Entity<Project>, cx: &mut App) {
        let worktrees: Vec<_> = project
            .read(cx)
            .visible_worktrees(cx)
            .map(|worktree| {
                let snapshot = worktree.read(cx);
                (snapshot.abs_path(), snapshot.id())
            })
            .collect();
        for (root, worktree_id) in worktrees {
    ...
    fn build_project_context(
        ...
        // trusted. Skills in `.cursor/skills/` ship with the project; a
    ...
    fn thread_save_payload(
        ...
        let active_spec_path = session
            .acp_thread
            .read(cx)
            .active_spec_path()
            .map(PathBuf::from);
        let active_ticket_id = session
            .acp_thread
            .read(cx)
            .active_ticket_id()
            .map(str::to_owned);
        ...
        let db_thread = session.thread.update(cx, |thread, cx| {
            ...
            thread.set_active_spec_path(active_spec_path);
            thread.set_active_ticket_id(active_ticket_id);
            thread.to_db(cx)
        });
...
pub static ZED_AGENT_ID: LazyLock<AgentId> =
    LazyLock::new(|| AgentId::new(format!("{} Agent", paths::APP_NAME)));
...
mod internal_tests {
    ...
    fn make_global_skill(name: &str, description: &str) -> Skill {
        Skill {
            ...
            directory_path: PathBuf::from(format!("/home/user/.cursor/skills/{name}")),
            skill_file_path: PathBuf::from(format!("/home/user/.cursor/skills/{name}/SKILL.md")),
    ...
    fn make_project_skill(name: &str, description: &str, worktree: &str) -> Skill {
        Skill {
            ...
            directory_path: PathBuf::from(format!("/{worktree}/.cursor/skills/{name}")),
            skill_file_path: PathBuf::from(format!("/{worktree}/.cursor/skills/{name}/SKILL.md")),
            load_warnings: Vec::new(),
            disable_model_invocation: false,
            embedded_body: None,
        }
    ...
    async fn test_project_skill_body_resolves_through_buffer(cx: &mut TestAppContext) {
        ...
        let relative_path: Arc<RelPath> = rel_path(".cursor/skills/my-skill/SKILL.md").into();
        let buffer = project
            .update(cx, |project, cx| {
                project.open_buffer((worktree_id, relative_path), cx)
            })
