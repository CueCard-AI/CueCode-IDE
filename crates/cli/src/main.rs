...
#[command(
    ...
    before_help = "The CueCode CLI binary.
This CLI is a separate binary that invokes CueCode.

Examples:
    `cuecode`
          Simply opens CueCode
    `cuecode --foreground`
          Runs in foreground (shows all logs)
    `cuecode path-to-your-project`
          Open your project in CueCode
    `cuecode -n path-to-file `
          Open file/folder in a new window",
    after_help = "To read from stdin, append '-', e.g. 'ps axf | cuecode -'"
)]
struct Args {
    ...
    reuse: bool,
    /// Open in existing CueCode window
    ...
    #[cfg_attr(target_os = "macos", doc = "`~/Library/Application Support/CueCode`.")]
    #[cfg_attr(target_os = "windows", doc = "`%LOCALAPPDATA%\\CueCode`.")]
    #[cfg_attr(
        not(any(target_os = "windows", target_os = "macos")),
        doc = "`$XDG_DATA_HOME/cuecode`."
    )]
    #[arg(long, value_name = "DIR")]
    user_data_dir: Option<String>,
    /// The paths to open in CueCode (space-separated).
    ///
    /// Use `path:line:column` syntax to open a file at the given line and column.
    paths_with_position: Vec<String>,
    /// Print CueCode's version and the app path.
    #[arg(short, long)]
    version: bool,
    /// Run CueCode in the foreground (useful for debugging)
    #[arg(long)]
    foreground: bool,
    /// Custom path to CueCode.app or the cuecode binary
    #[arg(long)]
    zed: Option<PathBuf>,
    /// Run CueCode in dev-server mode
    #[arg(long)]
    ...
    wsl: Option<String>,
    /// Not supported in CueCode CLI, only supported on CueCode binary
    ...
    diff: Vec<String>,
    /// Uninstall CueCode from user system
    #[cfg(all(
    ...
    /// by having CueCode act like netcat communicating over a Unix socket.
    #[arg(long, hide = true)]
...
fn run() -> Result<()> {
    ...
    if args.system_specs {
        let path = app.path();
        let msg = [
            "The `--system-specs` argument is not supported in the CueCode CLI, only on the CueCode binary.",
...
fn prompt_open_behavior() -> Option<cli::CliBehaviorSetting> {
    ...
    let items = [
        format!(
            "Add to existing CueCode window ({})",
            blue.apply_to("cuecode --existing")
        ),
        format!("Open a new window ({})", blue.apply_to("cuecode --classic")),
    ];

    let prompt = format!(
        "Configure default behavior for {}\n{}",
        blue.apply_to("cuecode <path>"),
        console::style("You can change this later in CueCode settings"),
    );
        ...
        pub fn detect(path: Option<&Path>) -> anyhow::Result<impl InstalledApp> {
            let path = if let Some(path) = path {
                ...
                let possible_locations = [
                    "../libexec/cuecode-editor",
                    "../lib/zed/cuecode-editor",
                    "./cuecode",
                    "../libexec/zed-editor",
                    "../lib/zed/zed-editor",
                    "./zed",
...
mod flatpak {
    ...
    pub fn try_restart_to_host() {
        if let Some(flatpak_dir) = get_flatpak_dir() {
            ...
            args.push("--env=ZED_UPDATE_EXPLANATION=Please use flatpak to update CueCode".into());
            args.push(
                format!(
            ...
            args.push(flatpak_dir.join("bin").join("cuecode").into());
            ...
            if !is_app_location_set {
                args.push("--zed".into());
                args.push(flatpak_dir.join("libexec").join("cuecode-editor").into());
            }
    ...
    pub fn set_bin_if_no_escape(mut args: super::Args) -> super::Args {
        if env::var(NO_ESCAPE_ENV_NAME).is_ok()
            && env::var("FLATPAK_ID").is_ok_and(|id| id.starts_with("dev.cuecode.CueCode"))
            && args.zed.is_none()
        {
            args.zed = Some("/app/libexec/cuecode-editor".into());
            unsafe { env::set_var("ZED_UPDATE_EXPLANATION", "Please use flatpak to update CueCode") };
        }
        args
    }
    ...
    fn get_flatpak_dir() -> Option<PathBuf> {
        ...
        if let Ok(flatpak_id) = env::var("FLATPAK_ID") {
            if !flatpak_id.starts_with("dev.cuecode.CueCode") {
                return None;
            }
...
mod mac_os {
    ...
    impl InstalledApp for Bundle {
        fn zed_version_string(&self) -> String {
            format!("CueCode {} – {}", self.version(), self.path().display(),)
        }
        ...
        fn run_foreground(
            ...
            let path = match self {
                Bundle::App { app_bundle, .. } => app_bundle.join("Contents/MacOS/cuecode"),
