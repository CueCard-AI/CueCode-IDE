            ...
            match ManagedNodeRuntime::install_if_needed(&state.http).await {
                ...
                Err(err) => {
                    ...
                    Box::new(UnavailableNodeRuntime {
                        error_message: format!(
                            "failure while downloading and/or installing {} managed Node.js, \
                            restart {} to retry: {}",
                            paths::APP_NAME,
                            paths::APP_NAME,
                            err
            ...
            match result {
                Ok(output) => {
                    if output.status.success() {
                        ...
                        log::warn!(
                            "{} managed Node.js binary at {} failed check with output: {:?}",
                            paths::APP_NAME,
                            node_binary.display(),
                            output
                        );
                ...
                Err(err) => {
                    log::warn!(
                        "{} managed Node.js binary at {} failed check, so re-downloading it. \
                        Error: {}",
                        paths::APP_NAME,
                        node_binary.display(),
                        err
