...
pub(crate) static JOBS: LazyLock<[Job; 22]> = LazyLock::new(|| {
    ...
    [
        ...
        Job::mkdir(p("old")),
        Job::move_file(p("CueCode.exe"), p("old\\CueCode.exe")),
        Job::mkdir(p("old\\bin")),
        Job::move_file(p("bin\\CueCode.exe"), p("old\\bin\\CueCode.exe")),
        Job::move_if_exists(p("bin\\cuecode"), p("old\\bin\\cuecode")),
        ...
        // Copy new files
        Job::move_file(p("install\\CueCode.exe"), p("CueCode.exe")),
        Job::move_file(p("install\\bin\\CueCode.exe"), p("bin\\CueCode.exe")),
        Job::move_file(p("install\\bin\\cuecode"), p("bin\\cuecode")),
...
fn release_file_handles(app_dir: &Path) -> Result<()> {
    ...
    let files_to_release = [
        app_dir.join("CueCode.exe"),
        app_dir.join("bin\\CueCode.exe"),
        app_dir.join("bin\\cuecode"),
...
pub(crate) fn perform_update(app_dir: &Path, hwnd: Option<isize>, launch: bool) -> Result<()> {
    ...
    if launch {
        ...
        let _ = std::process::Command::new(app_dir.join("CueCode.exe")).spawn();
    }
