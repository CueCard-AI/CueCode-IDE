...
pub(crate) async fn send_custom_server_request(
    ...
    match provider {
        ...
        _ => {
            ...
            // Accept either a full completions endpoint or a provider base URL
            // (e.g. one shared with the agent's chat provider).
            let api_url = settings.api_url.as_ref();
            let uri = if api_url.ends_with("/completions") {
                api_url.to_string()
            } else {
                format!("{}/completions", api_url.trim_end_matches('/'))
            };
