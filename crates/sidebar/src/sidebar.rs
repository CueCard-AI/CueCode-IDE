...
use cuecode_actions::OpenRecent;
use cuecode_actions::editor::{MoveDown, MoveUp};

use cuecode_actions::agents_sidebar::{FocusSidebarFilter, ToggleThreadSwitcher};
...
impl Sidebar {
    ...
    fn rebuild_contents(&mut self, cx: &App) {
        ...
        let notified_terminals: HashSet<TerminalId> = HashSet::new();
        ...
        let current_terminal_ids: HashSet<TerminalId> = HashSet::new();
        ...
        for group in &groups {
            ...
            // Agent terminal threads are retired from the sidebar: the workspace
            // terminal panel is the primary terminal surface, and listing agent
            // terminals here would open them in the agent panel. Stored metadata
            // rows are ignored rather than deleted.
            let terminals: Vec<TerminalEntry> = Vec::new();
    ...
    fn is_thread_visible_in_agent_panel(
        thread_id: &ThreadId,
        workspace: &Entity<Workspace>,
        cx: &App,
    ) -> bool {
        workspace
            .read(cx)
            .panel::<AgentPanel>(cx)
            .is_some_and(|panel| panel.read(cx).active_thread_id(cx) == Some(*thread_id))
    }
    ...
    fn activate_thread_locally(
        ...
        if self.is_thread_active_in_workspace(&metadata.thread_id, workspace, cx)
            && Self::is_thread_visible_in_agent_panel(&metadata.thread_id, workspace, cx)
            && !workspace.read(cx).panel::<AgentPanel>(cx).is_some_and(|panel| {
                panel
                    .read(cx)
                    .should_force_reload_thread(metadata.thread_id, cx)
            })
        {
            workspace.update(cx, |workspace, cx| {
                if let Some(panel) = workspace.panel::<AgentPanel>(cx) {
                    panel.update(cx, |panel, cx| {
                        panel.sync_active_thread_list_state(window, cx);
                    });
                }
                workspace.focus_panel::<AgentPanel>(window, cx);
            });
    ...
    fn create_new_entry(
        ...
        // Always create an agent thread. Agent terminal threads are retired from
        // the sidebar, so "+" must not repeat a last-created terminal entry.
        self.create_new_thread(workspace, window, cx);
    }
    ...
    fn threads_sidebar_outer_tiled(side: SidebarSide, cx: &App) -> bool {
    ...
    fn render_sidebar_header(
        ...
        let outer_tiled = Self::threads_sidebar_outer_tiled(self.side(cx), cx);
        let traffic_lights =
            cfg!(target_os = "macos") && not_fullscreen && sidebar_on_left && outer_tiled;
        let left_window_controls =
            !cfg!(target_os = "macos") && not_fullscreen && sidebar_on_left && outer_tiled;
        let right_window_controls =
            !cfg!(target_os = "macos") && not_fullscreen && sidebar_on_right && outer_tiled;
    ...
    fn render_acp_import_onboarding(
        ...
        render_import_onboarding_banner(
            ...
            "Import threads from agents like Claude Agent, Codex, and more, whether started in CueCode or another client.",
            if verbose_labels {
