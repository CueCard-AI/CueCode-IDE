...
impl settings::Settings for AllLanguageSettings {
    fn from_settings(content: &settings::SettingsContent) -> Self {
        ...
        let ep_openai_compatible = edit_predictions.open_ai_compatible_api.unwrap();
        let ep_max_output_tokens = ep_openai_compatible.max_output_tokens.unwrap();
        let ep_prompt_format = ep_openai_compatible.prompt_format.unwrap();
        let openai_compatible_settings = ep_openai_compatible
            .model
            .filter(|model| !model.is_empty())
            .zip(
                ep_openai_compatible
                    .api_url
                    .filter(|api_url| !api_url.is_empty()),
            )
            .map(|(model, api_url)| OpenAiCompatibleEditPredictionSettings {
                model,
                max_output_tokens: ep_max_output_tokens,
                api_url: api_url.into(),
                prompt_format: ep_prompt_format.into(),
            })
            .or_else(|| {
                // No explicit edit prediction endpoint configured: fall back to
                // the first OpenAI-compatible provider configured for the agent
                // (`language_models.openai_compatible`), so a single BYOK setup
                // powers both chat and inline predictions. Credentials are
                // shared automatically because API keys are stored per URL.
                let providers = content.language_models.as_ref()?.openai_compatible.as_ref()?;
                let (_, provider) = providers
                    .iter()
                    .filter(|(_, provider)| !provider.api_url.is_empty())
                    .filter(|(_, provider)| !provider.available_models.is_empty())
                    .min_by_key(|(id, _)| (*id).clone())?;
                let model = provider.available_models.first()?.name.clone();
                Some(OpenAiCompatibleEditPredictionSettings {
                    model,
                    max_output_tokens: ep_max_output_tokens,
                    api_url: provider.api_url.clone().into(),
                    prompt_format: ep_prompt_format.into(),
                })
            });
