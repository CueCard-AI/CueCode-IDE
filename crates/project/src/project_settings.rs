...
use paths::{
    EDITORCONFIG_NAME, is_local_debug_file, is_local_settings_file, is_local_tasks_file,
    local_settings_directory_for_file, local_settings_file_relative_path,
...
/// In ssh mode it also monitors ~/.config/cuecode/{settings, task}.json and sends the content
/// upstream.
impl SettingsObserver {
    ...
    fn update_local_worktree_settings(
        ...
        for (path, _, change) in changes.iter() {
            let (settings_dir, kind) = if is_local_settings_file(path) {
                let settings_dir = local_settings_directory_for_file(path).unwrap();
                (settings_dir, LocalSettingsKind::Settings)
            } else if is_local_tasks_file(path) {
                let settings_dir = local_settings_directory_for_file(path).unwrap();
                ...
                let settings_dir = path
                    ...
                    .into();
                (settings_dir, LocalSettingsKind::Tasks)
            } else if is_local_debug_file(path) {
                let settings_dir = local_settings_directory_for_file(path).unwrap();
