        ...
        let sub1 = cx.observe_keystrokes(|this, e, _, cx| {
            ...
            this.last_possibilities = possibilities
                ...
                .map(|binding| {
                    ...
                    if name == "cuecode::NoAction" || name == "zed::NoAction" {
                        name = "(null)"
                    }
...
impl KeyContextView {
    ...
    fn action_matches(&self, a: &Option<Box<dyn Action>>, b: &dyn Action) -> bool {
        if let Some(last_action) = a {
            ...
            b.name() == "cuecode::NoAction" || b.name() == "zed::NoAction"
        }
    }
}
...
impl Render for KeyContextView {
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl ui::IntoElement {
        ...
        v_flex()
            ...
            .child(Label::new("This view lets you determine the current context stack for creating custom key bindings in CueCode. When a keyboard shortcut is triggered, it also shows all the possible contexts it could have triggered in, and which one matched."))
            .child(
                h_flex()
                    .mt_4()
                    .gap_4()
                    ...
                    .child(
                        Button::new("view_default_keymap", "View Default Keymap")
                            ...
                            .key_binding(ui::KeyBinding::for_action(
                                &cuecode_actions::OpenDefaultKeymap,
                                cx
                            ))
                            .on_click(|_, window, cx| {
                                window.dispatch_action(cuecode_actions::OpenDefaultKeymap.boxed_clone(), cx);
                            }),
                    )
                    .child(
                        Button::new("edit_your_keymap", "Edit Keymap File")
                            .style(ButtonStyle::Filled)
                            .key_binding(ui::KeyBinding::for_action(&cuecode_actions::OpenKeymapFile, cx))
                            .on_click(|_, window, cx| {
