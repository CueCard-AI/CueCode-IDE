...
impl GitExcludeOverride {
    ...
    fn remove_auto_generated_block(content: &str) -> String {
        ...
        // Older versions did not have end-of-block markers,
...
pub async fn get_git_committer(cx: &AsyncApp) -> GitCommitter {
    ...
    let git_binary_path =
        if cfg!(target_os = "macos")
            && (option_env!("CUECODE_BUNDLE").as_deref() == Some("true")
...
fn checkpoint_author_envs() -> HashMap<String, String> {
    HashMap::from_iter([
        ("GIT_AUTHOR_NAME".to_string(), "CueCode".to_string()),
        ("GIT_AUTHOR_EMAIL".to_string(), "hi@zed.dev".to_string()),
        ("GIT_COMMITTER_NAME".to_string(), "CueCode".to_string()),
            ...
            vec![
                Branch {
                    ...
                    most_recent_commit: Some(CommitSummary {
                        ...
                        author_name: SharedString::new_static("CueCode"),
                        has_parent: true,
                    })
                },
                Branch {
