...
impl CopilotCodeVerification {
    ...
    fn render_prompting_modal(
        ...
        v_flex()
            ...
            .items_center()
            .text_center()
            .child(Headline::new(format!("Use GitHub Copilot in {}", paths::APP_NAME)).size(HeadlineSize::Large))
            .child(
...
impl ConfigurationView {
    ...
    fn render_for_chat(&self) -> impl IntoElement {
        let start_label = format!(
            "To use {}'s agent with GitHub Copilot, you need to be logged in to GitHub. Note that your GitHub account must have an active Copilot Chat subscription.",
            paths::APP_NAME
        );
