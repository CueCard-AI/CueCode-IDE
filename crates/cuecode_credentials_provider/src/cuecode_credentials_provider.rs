...
///
/// By default, running CueCode in development uses the development credentials
...
///
/// Prefer `CUECODE_DEVELOPMENT_USE_KEYCHAIN`; `ZED_DEVELOPMENT_USE_KEYCHAIN` is
/// accepted for backward compatibility.
static CUECODE_DEVELOPMENT_USE_KEYCHAIN: LazyLock<bool> = LazyLock::new(|| {
    read_bool_env("CUECODE_DEVELOPMENT_USE_KEYCHAIN", "ZED_DEVELOPMENT_USE_KEYCHAIN")
});

pub struct CueCodeCredentialsProvider(pub Arc<dyn CredentialsProvider>);

impl Global for CueCodeCredentialsProvider {}

/// Deprecated alias for [`CueCodeCredentialsProvider`].
pub use CueCodeCredentialsProvider as ZedCredentialsProvider;
...
pub fn init_global(cx: &mut App) {
    ...
    let provider = new(cx);
    cx.set_global(CueCodeCredentialsProvider(provider));
}

pub fn global(cx: &App) -> Arc<dyn CredentialsProvider> {
    cx.try_global::<CueCodeCredentialsProvider>()
...
fn new(cx: &App) -> Arc<dyn CredentialsProvider> {
    let use_development_provider = match ReleaseChannel::try_global(cx) {
        Some(ReleaseChannel::Dev) => {
            ...
            //
            // However, if the `CUECODE_DEVELOPMENT_USE_KEYCHAIN` environment
            // variable is set, we will use the actual keychain.
            !*CUECODE_DEVELOPMENT_USE_KEYCHAIN
        }
...
/// re-allow access to the system keychain when developing CueCode.
