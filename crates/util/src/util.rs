...
pub fn prevent_root_execution() {
    ...
    if is_root && !allow_root {
        eprintln!(
            "\
Error: Running CueCode as root or via sudo is unsupported.
       Doing so (even once) may subtly break things for all subsequent non-root usage of CueCode.
...
pub fn get_shell_safe_zed_path(shell_kind: shell::ShellKind) -> anyhow::Result<String> {
    ...
    zed_path
        ...
        .context("Failed to shell-escape CueCode executable path.")
}
...
pub fn get_zed_cli_path() -> Result<PathBuf> {
    ...
    let possible_locations: &[&str] = if cfg!(target_os = "macos") {
        // On macOS, the cuecode executable and cli are inside the app bundle,
        // so here ./cli is for both installed and development builds.
        &["./cli"]
    } else if cfg!(target_os = "windows") {
        // bin/cuecode.exe is for installed builds, ./cli.exe is for development builds.
        &["bin/cuecode.exe", "bin/zed.exe", "./cli.exe"]
        ...
        &["../bin/cuecode", "../bin/zed", "./cli"]
    } else {
        anyhow::bail!("unsupported platform for determining cuecode-cli path");
    };
    ...
    possible_locations
        ...
        .with_context(|| {
            format!(
                "could not find cuecode-cli from any of: {}",
