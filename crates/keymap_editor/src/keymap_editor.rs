...
use cuecode_actions::{ChangeKeybinding, OpenKeymap};
                                ...
                                h_flex()
                                    ...
                                    .child(
                                        Button::new("edit-in-json", "Edit in JSON")
                                            ...
                                            .key_binding(
                                                ui::KeyBinding::for_action_in(&cuecode_actions::OpenKeymapFile, &focus_handle, cx)
                                                    .map(|kb| kb.size(rems_from_px(10.))),
                                            )
                                            .on_click(|_, window, cx| {
                                                window.dispatch_action(
                                                    cuecode_actions::OpenKeymapFile.boxed_clone(),
                                                    cx,
                                                );
                                            })
                                    )
...
async fn load_keybind_context_language(
    ...
    let language_task = workspace
        .read_with(cx, |workspace, cx| {
            workspace
                ...
                .read(cx)
                .languages()
                .language_for_name("CueCode Keybind Context")
        })
        .context("Failed to load CueCode Keybind Context language")
        .log_err();
    ...
    language.unwrap_or_else(|| {
        Arc::new(Language::new(
            LanguageConfig {
                name: "CueCode Keybind Context".into(),
                ..Default::default()
            },
...
mod tests {
    ...
    fn binding_is_unbound_by_unbind_respects_precedence() {
        let binding = gpui::KeyBinding::new("tab", cuecode_actions::OpenKeymap, None);
        let unbind =
