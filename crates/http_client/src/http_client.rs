...
impl HttpClientWithUrl {
    ...
    pub fn build_zed_api_url(&self, path: &str, query: &[(&str, &str)]) -> Result<Url> {
        ...
        let base_api_url = zed_api_base_url(&base_url);
    ...
    /// Builds a URL for the extension marketplace API.
    ///
    /// CueCode rebrand: `server_url` may be empty to disable Zed cloud, but extension
    /// browsing and installs still use the Zed marketplace registry in compat mode.
    pub fn build_extensions_api_url(&self, path: &str, query: &[(&str, &str)]) -> Result<Url> {
        let base_url = self.base_url();
        let base_api_url = extensions_api_base_url(&base_url);
...
fn zed_api_base_url(server_url: &str) -> &str {
    match server_url {
...
/// Extension registry base URL for compat mode (see `03-fork-and-rebrand.md` §extensions).
fn extensions_api_base_url(server_url: &str) -> &str {
    match server_url {
        "" => "https://api.zed.dev",
        "https://zed.dev" => "https://api.zed.dev",
        "https://staging.zed.dev" => "https://api-staging.zed.dev",
        "http://localhost:3000" => "http://localhost:8080",
        // CueCode v1: no forked marketplace yet — keep Zed registry without enabling cloud.
        _ => "https://api.zed.dev",
    }
}

#[cfg(test)]
mod extensions_api_url_tests {
    use super::*;

    #[test]
    fn extensions_api_base_url_defaults_when_server_url_empty() {
        assert_eq!(extensions_api_base_url(""), "https://api.zed.dev");
    }

    #[test]
    fn extensions_api_base_url_maps_zed_dev_hosts() {
        assert_eq!(
            extensions_api_base_url("https://zed.dev"),
            "https://api.zed.dev"
        );
        assert_eq!(
            extensions_api_base_url("https://staging.zed.dev"),
            "https://api-staging.zed.dev"
        );
        assert_eq!(
            extensions_api_base_url("http://localhost:3000"),
            "http://localhost:8080"
        );
    }

    #[test]
    fn extensions_api_base_url_falls_back_for_custom_server_url() {
        assert_eq!(
            extensions_api_base_url("https://cuecode.dev"),
            "https://api.zed.dev"
        );
    }
}
