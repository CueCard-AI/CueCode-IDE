...
/// Returns value of `CUECODE_BUNDLE_TYPE` set at compile time or else at runtime.
///
/// The compiletime value is used by flatpak since it doesn't seem to have a way to provide a
/// runtime environment variable.
///
/// The runtime value is used by snap since release binaries cannot have this baked in.
///
/// Prefer `CUECODE_BUNDLE_TYPE`; `ZED_BUNDLE_TYPE` is accepted for backward compatibility.
fn bundle_type() -> Option<String> {
    option_env!("CUECODE_BUNDLE_TYPE")
        .map(|bundle_type| bundle_type.to_string())
        .or_else(|| option_env!("ZED_BUNDLE_TYPE").map(|bundle_type| bundle_type.to_string()))
        .or_else(|| {
            env_var::read_env_var("CUECODE_BUNDLE_TYPE", "ZED_BUNDLE_TYPE")
                .value
        })
}
