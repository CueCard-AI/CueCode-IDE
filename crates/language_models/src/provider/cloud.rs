...
impl LanguageModelProvider for CloudLanguageModelProvider {
    ...
    fn authentication_error_message(&self) -> SharedString {
        format!("Failed to sign in with your {} account (401).", paths::APP_NAME).into()
    }

    fn missing_credentials_error_message(&self) -> SharedString {
        format!(
            "You are not signed in to your {} account. Sign in to continue.",
            paths::APP_NAME
        )
        .into()
    }
    ...
    fn fast_mode_confirmation(&self, _cx: &App) -> Option<FastModeConfirmation> {
        Some(FastModeConfirmation {
            title: format!("Enable Fast Mode for {}?", paths::APP_NAME).into(),
            message: format!(
...
impl RenderOnce for ZedAiConfiguration {
    fn render(self, _window: &mut Window, _cx: &mut App) -> impl IntoElement {
        ...
        v_flex()
            ...
            .child(
                Label::new(format!(
                    "{} cloud models are not enabled in this build.",
                    paths::APP_NAME
                ))
                .color(Color::Muted),
            )
            .child(
                List::new()
                    .child(ListBulletItem::new("Ollama (local)"))
                    .child(ListBulletItem::new("OpenAI-compatible endpoint"))
                    .child(ListBulletItem::new("Anthropic or other API keys")),
            )
            .child(
                Label::new("Configure providers in Settings → AI.")
...
impl Component for ZedAiConfiguration {
    ...
    fn description() -> &'static str {
        "The configuration surface for CueCode's hosted AI models, \
        showing the user's connection status, current plan, trial eligibility, \
        and entry points for enabling the CueCode model provider."
    }
    ...
    fn preview(_window: &mut Window, _cx: &mut App) -> AnyElement {
        ...
        v_flex()
            ...
            .children(vec![
                ...
                single_example(
                    format!("{} Pro Trial Plan", paths::APP_NAME),
                    configuration(PreviewConfiguration {
                ...
                single_example(
                    format!("{} Pro Plan", paths::APP_NAME),
                    configuration(PreviewConfiguration {
                ...
                single_example(
                    "Business Plan - CueCode models enabled",
                    configuration(PreviewConfiguration {
                ...
                single_example(
                    "Business Plan - CueCode models disabled",
                    configuration(PreviewConfiguration {
                        plan: Some(Plan::ZedBusiness),
                        is_connected: true,
