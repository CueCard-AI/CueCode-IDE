...
use gpui::{AnyView, App, AppContext, AsyncApp, Entity, SharedString, Task, Window};
...
use crate::provider::api_compatible::{
    ApiCompatibleProviderSettings, ApiCompatibleProviderState,
};
...
struct CompatibleProviderMetadata {
    api_key_url: SharedString,
    api_key_placeholder: &'static str,
}

fn compatible_provider_metadata(provider_id: &str, api_url: &str) -> CompatibleProviderMetadata {
    match provider_id {
        "zai" | "z-ai" | "z.ai" => CompatibleProviderMetadata {
            api_key_url: SharedString::new("https://z.ai/"),
            api_key_placeholder: "your-z-ai-api-key",
        },
        _ => CompatibleProviderMetadata {
            api_key_url: SharedString::new(api_url),
            api_key_placeholder: "api-key",
        },
    }
}

fn compatible_provider_display_name(id: &SharedString) -> LanguageModelProviderName {
    match id.as_ref() {
        "zai" | "z-ai" | "z.ai" => LanguageModelProviderName::new("Z.ai"),
        _ => LanguageModelProviderName(id.clone()),
    }
}
...
impl OpenAiCompatibleLanguageModelProvider {
    ...
    fn display_name(&self) -> LanguageModelProviderName {
        compatible_provider_display_name(&self.id.0)
    }

    fn metadata(&self, cx: &App) -> CompatibleProviderMetadata {
        let api_url = self.state.read(cx).settings.api_url.as_str();
        compatible_provider_metadata(self.id.0.as_ref(), api_url)
    }
    ...
    fn api_key_editor(
        ...
        cx.new(|cx| {
            crate::ApiKeyEditor::new(
                ...
                |state, cx| state.authenticate(cx),
                window,
                cx,
            )
        })
...
impl LanguageModelProvider for OpenAiCompatibleLanguageModelProvider {
    ...
    fn configuration_view(
        ...
        self.api_key_editor(window, cx).into()
    }
    ...
    fn configuration_view_v2(
        ...
        ProviderConfigurationView::Inline(self.api_key_editor(window, cx).into())
    }
