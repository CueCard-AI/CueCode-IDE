...
pub enum EditPredictionProvider {
    ...
    #[default]
    Copilot,
    #[serde(rename = "cuecode", alias = "zed")]
    CueCode,
    Codestral,
    Ollama,
    OpenAiCompatibleApi,
...
impl EditPredictionProvider {
    pub fn is_cuecode(&self) -> bool {
        match self {
            EditPredictionProvider::CueCode => true,
    ...
    /// Deprecated alias for [`Self::is_cuecode`].
    ...
    pub fn display_name(&self) -> Option<&'static str> {
        match self {
            EditPredictionProvider::CueCode => Some("CueCode AI"),
...
pub struct EditPredictionSettingsContent {
    ...
    /// Controls whether CueCode may collect training data when using CueCode's Edit Predictions.
...
/// Controls whether CueCode collects training data when using CueCode's Edit Predictions.
#[derive(
    Copy,
    Clone,
    Debug,
...
pub enum EditPredictionDataCollectionChoice {
    ...
    #[default]
    Default,
    /// Allow CueCode to collect training data from open-source projects.
    Yes,
...
pub struct LanguageSettingsContent {
    ...
    /// CueCode's Prettier integration settings.
    ...
    /// when you type '(', CueCode will automatically add a closing ')' at the correct position.
    ///
    /// Default: true
    pub use_autoclose: Option<bool>,
    /// Whether to automatically surround text with characters for you. For example,
    /// when you select text and type '(', CueCode will automatically surround text with ().
    ///
    /// Default: true
...
#[serde(rename_all = "snake_case")]
pub enum Formatter {
    /// Format files using CueCode's Prettier integration (if applicable),
    /// or falling back to formatting via language server.
    #[default]
    Auto,
    /// Do not format code.
    None,
    /// Format code using CueCode's Prettier integration.
    Prettier,
...
pub struct LanguageTaskSettingsContent {
    ...
    pub enabled: Option<bool>,
    /// Use LSP tasks over CueCode language extension ones.
    /// If no LSP tasks are returned due to error/timeout or regular execution,
    /// CueCode language extension tasks will be used instead.
    ///
    /// Other CueCode tasks will still be shown:
    /// * CueCode task from either of the task config file
    /// * CueCode task from history (e.g. one-off task was spawned before)
