...
/// Persisted Plan host for Layout Studio (1.4g).
#[derive(
    Clone,
    Copy,
    Debug,
...
#[serde(rename_all = "snake_case")]
pub enum LayoutPlanHostSetting {
    #[default]
    TabInAgent,
    ColumnLeft,
    ColumnRight,
...
/// Persisted outer-strip order for Layout Studio (1.4g).
#[derive(
    Clone,
    Copy,
    Debug,
...
#[serde(rename_all = "snake_case")]
pub enum LayoutOuterStripOrderSetting {
    #[default]
    ThreadsFirst,
    NavFirst,
}
...
pub struct AgentSettingsContent {
    ...
    /// Layout Studio: where Plan lives (tab, column, or detached window).
    pub layout_plan_host: Option<LayoutPlanHostSetting>,
    /// Layout Studio: Plan column width when pinned as a column.
    #[serde(serialize_with = "crate::serialize_optional_f32_with_two_decimal_places")]
    pub layout_plan_column_width: Option<f32>,
    /// Layout Studio: Threads vs Nav order on the left outer strip.
    pub layout_outer_left_order: Option<LayoutOuterStripOrderSetting>,
    /// Layout Studio: Threads vs Nav order on the right outer strip.
    pub layout_outer_right_order: Option<LayoutOuterStripOrderSetting>,
    /// Layout Studio: threads sidebar hidden.
    pub layout_threads_hidden: Option<bool>,
    /// Layout Studio: project/outline nav hidden.
    pub layout_nav_hidden: Option<bool>,
    /// Layout Studio: implement-mode plan strip only.
    pub layout_plan_strip_only: Option<bool>,
    ...
    /// external agent's permission system is only used when CueCode would allow
...
pub struct SandboxPermissionsContent {
    ...
    /// to without prompting. Paths written by CueCode are absolute.
    /// Default: []
