...
pub struct AllLanguageModelSettingsContent {
    ...
    #[serde(rename = "cuecode.dev", alias = "zed.dev")]
...
pub struct AnthropicCompatibleAvailableModel {
    ...
    pub name: String,
    /// The model's name in CueCode's UI, such as in the model selector dropdown menu in the assistant panel.
...
pub struct AnthropicAvailableModel {
    ...
    pub name: String,
    /// The model's name in CueCode's UI, such as in the model selector dropdown menu in the agent panel.
    pub display_name: Option<String>,
    /// The model's context window size.
    pub max_tokens: u64,
    /// A model `name` to substitute when calling tools, in case the primary model doesn't support tool calling.
