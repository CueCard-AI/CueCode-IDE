...
const ICON_DIR: &str = concat!(env!("CARGO_MANIFEST_DIR"), "/../cuecode/resources/windows");
...
pub fn compile(manifest: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (icon_filename, product_name) = match channel {
        "stable" => ("app-icon.ico", "CueCode"),
        "preview" => ("app-icon-preview.ico", "CueCode Preview"),
        "nightly" => ("app-icon-nightly.ico", "CueCode Nightly"),
        _ => ("app-icon-dev.ico", "CueCode Dev"),
    };
...
BEGIN
    ...
    BEGIN
        ...
        BEGIN
            ...
            VALUE "CompanyName", "CueCode\0"
            VALUE "LegalCopyright", "Copyright 2022 - 2025 CueCode contributors\0"
        END
    END
    BLOCK "VarFileInfo"
    BEGIN
...
"#
    ...
    let rc_path = out_dir.join("cuecode_resources.rc");
