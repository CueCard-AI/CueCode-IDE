    ...
    v_flex()
        ...
        .child(
            SwitchField::new(
                ...
                None::<&str>,
                Some("Help improve CueCode by sending anonymous usage data".into()),
        ...
        .child(
            SwitchField::new(
                ...
                None::<&str>,
                Some(
                    "Help fix CueCode by sending crash reports so we can fix critical issues fast"
                        .into(),
                ),
...
fn render_worktree_auto_trust_switch(tab_index: &mut isize, cx: &mut App) -> impl IntoElement {
    ...
    let tooltip_description = format!(
        "{app_name} can only allow services like language servers, project settings, and MCP servers to run after you mark a new project as trusted.",
    ...
    SwitchField::new(
        ...
        Some(format!(
            "Automatically mark all new projects as trusted to unlock all {app_name}'s features",
            app_name = paths::APP_NAME
        ).into()),
        toggle_state,
        {
...
fn render_zed_agent_button(_user_store: &Entity<UserStore>, _cx: &mut App) -> impl IntoElement {
    AgentSetupButton::new("cuecode-agent-onboarding")
        .icon(
        ...
        .on_click(move |_, window, cx| {
            window.dispatch_action(cuecode_actions::agent::OpenSettings.boxed_clone(), cx);
        })
}
