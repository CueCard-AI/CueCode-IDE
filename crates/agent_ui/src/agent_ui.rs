...
mod layout_studio;
mod layout_studio_apply;
mod layout_studio_chrome;
mod layout_studio_editor;
mod layout_studio_preview;
mod detached_agent_chat;
mod planning_hub;
mod plan_session;
mod plan_store;
mod spec_pin_chip;
...
use cuecode_actions;
pub use cuecode_actions::{CreateWorktree, NewWorktreeBranchTarget, SwitchWorktree};
...
impl Agent {
    ...
    pub fn label(&self) -> SharedString {
        match self {
            Self::NativeAgent => format!("{} Agent", paths::APP_NAME).into(),
...
pub fn init(
    ...
    cx.observe_new(crate::planning_hub::PlanningHubModal::register).detach();
    cx.observe_new(crate::layout_studio::LayoutStudioModal::register).detach();
    cx.observe_new(|workspace: &mut Workspace, _window, _cx| {
        workspace.register_action(
            |workspace: &mut Workspace,
...
fn update_command_palette_filter(cx: &mut App) {
    ...
    CommandPaletteFilter::update_global(cx, |filter, _| {
        ...
        let manage_skills_action = [TypeId::of::<cuecode_actions::assistant::ManageSkills>()];
        let skill_creator_actions = [
            TypeId::of::<cuecode_actions::assistant::OpenSkillCreator>(),
            TypeId::of::<cuecode_actions::assistant::CreateSkillFromUrl>(),
        ];
        ...
        if disable_ai {
            ...
            filter.hide_namespace("edit_prediction_onboarding");
            filter.hide_namespace("edit_prediction");

            filter.hide_action_types(&edit_prediction_actions);
            filter.hide_action_types(&[TypeId::of::<cuecode_actions::OpenEditPredictionOnboarding>()]);
        } else {
            if agent_enabled {
            ...
            match edit_prediction_provider {
                ...
                EditPredictionProvider::CueCode
            ...
            filter.show_namespace("edit_prediction_onboarding");
            filter.show_action_types(&[TypeId::of::<cuecode_actions::OpenEditPredictionOnboarding>()]);
...
mod tests {
    ...
    fn test_agent_command_palette_visibility(cx: &mut TestAppContext) {
        ...
        cx.update(|cx| {
            ...
            assert!(
                !filter.is_hidden(&cuecode_actions::assistant::OpenSkillCreator),
                "OpenSkillCreator should be visible by default"
            );
            assert!(
                !filter.is_hidden(&cuecode_actions::assistant::CreateSkillFromUrl),
                "CreateSkillFromUrl should be visible by default"
            );
            assert!(
                !filter.is_hidden(&cuecode_actions::assistant::OpenGlobalAgentsMdRules),
                "OpenGlobalAgentsMdRules should be visible by default"
            );
            assert!(
                !filter.is_hidden(&cuecode_actions::assistant::OpenProjectAgentsMdRules),
