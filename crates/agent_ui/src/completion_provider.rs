...
impl<T: PromptCompletionProviderDelegate> PromptCompletionProvider<T> {
    ...
    fn completion_for_spec(
        ...
        let crease_text: SharedString = spec.title.as_ref().into();
        let source_highlight_id = cx
            .theme()
            .syntax()
            .highlight_id("variable")
            .map(HighlightId::new);
        let label = build_slash_item_label(&spec.title, None, source_highlight_id);
        let documentation = spec.summary.map(CompletionDocumentation::MultiLinePlainText);
        Completion {
            replace_range: source_range.clone(),
            new_text,
            label,
    ...
    fn search_mentions(
        ...
        match mode {
            ...
            Some(PromptContextType::Spec) => {
                let specs = available_specs_for_workspace(&workspace, cx);
                let search_specs_task = search_specs(query, cancellation_flag, specs, cx);
                cx.background_spawn(async move {
                    search_specs_task
                        .await
                        .into_iter()
    ...
    fn available_context_picker_entries(
        ...
        if self.source.supports_context(PromptContextType::Spec, cx)
            && !available_specs_for_workspace(workspace, cx).is_empty()
        {
            entries.push(PromptContextEntry::Mode(PromptContextType::Spec));
        }
                        ...
                        matches
                            ...
                            .filter_map(|mat| {
                                ...
                                let mut completion = match mat {
                                    ...
                                    Match::Spec(spec) => Some(Self::completion_for_spec(
                                        spec,
                                        source_range.clone(),
                                        source.clone(),
                                        editor.clone(),
...
fn available_specs_for_workspace(
    workspace: &Entity<Workspace>,
    cx: &App,
) -> Vec<AvailableSpec> {
    let workspace = workspace.read(cx);
...
pub(crate) fn search_specs(
    query: String,
    cancellation_flag: Arc<AtomicBool>,
    specs: Vec<AvailableSpec>,
    cx: &mut App,
) -> Task<Vec<AvailableSpec>> {
    if specs.is_empty() {
        return Task::ready(Vec::new());
    }
    let executor = cx.background_executor().clone();
    cx.background_spawn(async move {
        let candidates = specs
            ...
            .map(|(id, spec)| {
                let search_text = format!(
                    "{} {}",
                    spec.title,
                    spec.relative_path.to_string_lossy()
                );
                StringMatchCandidate::new(id, search_text.as_str())
            })
            .collect::<Vec<_>>();
        let matches = fuzzy::match_strings(
            &candidates,
        ...
        matches
            .into_iter()
            .map(|mat| specs[mat.candidate_id].clone())
            .collect()
    })
}
