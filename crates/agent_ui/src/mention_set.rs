...
impl MentionSet {
    ...
    pub fn confirm_mention_for_uri(
        ...
        match mention_uri {
            ...
            MentionUri::Spec {
                relative_path, title, ..
            } => self.confirm_mention_for_spec(relative_path, &title, cx),
    ...
    fn confirm_mention_for_spec(
        ...
        let worktree_root = project.read(cx).visible_worktrees(cx).find_map(|worktree| {
            let snapshot = worktree.read(cx);
            let root = snapshot.abs_path().to_path_buf();
            root.join(&relative_path).is_file().then_some(root)
        });
        ...
        cx.background_spawn(async move {
            let doc = cuecode_specs::read_spec_document(&worktree_root, &relative_path)?;
            let content = cuecode_specs::render_spec_envelope(&doc.path, &title, &doc.body);
