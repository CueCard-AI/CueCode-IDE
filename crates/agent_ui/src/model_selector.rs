...
use util::ResultExt;
use cuecode_actions::open_llm_providers_settings;
...
impl PickerDelegate for ModelPickerDelegate {
    ...
    fn render_footer(
        ...
        Some(
            ModelSelectorFooter::new(open_llm_providers_settings().boxed_clone(), focus_handle)
                .into_any_element(),
        )
    }
}
...
mod tests {
    ...
    fn test_favorites_are_not_duplicated_when_repeated_in_other_sections(_cx: &mut TestAppContext) {
        let models = create_model_list(vec![
            ...
            ("CueCode", vec!["zed/claude", "zed/gpt-5"]),
