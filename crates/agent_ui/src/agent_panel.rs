...
use crate::detached_agent_chat::{
    DetachedAgentChatView, activate_detached_threads_window, close_detached_threads_window,
};
...
use crate::{
    ...
    planning_hub::PlanningHubView,
    plan_store::plan_store_for,
    spec_planning::ensure_spec_indices_loaded,
...
use agent_settings::{AgentSettings, PanelLayout};
...
use feature_flags::{
    CreateThreadToolFeatureFlag, FeatureFlagAppExt as _,
};
...
use gpui::{
    ...
    PlatformDisplay, Subscription, Task, TaskExt, WeakEntity, WindowHandle, WindowOptions,
...
use ui::{
    Button, ButtonStyle, ContextMenu, ContextMenuEntry, GradientFade, IconButton, IconName,
    IconSize, KeyBinding, LabelSize, PopoverMenu, PopoverMenuHandle, ProjectEmptyState, Tab,
...
const MIN_PANEL_HEIGHT: Pixels = px(240.);
            ...
            workspace
                ...
                .register_action(
                    |workspace: &mut Workspace, _: &AddSelectionToThread, window, cx| {
                        ...
                        {
                            // Cursor parity: "add to chat" with nothing selected
                            // just focuses the agent panel.
                            workspace.focus_panel::<AgentPanel>(window, cx);
                            return;
                        }
...
enum BaseView {
    ...
    Plan {
        plan_view: Entity<PlanningHubView>,
    },
    Terminal {
        terminal_id: TerminalId,
    },
...
impl AgentPanel {
                ...
                panel.update(cx, |panel, cx| {
                    ...
                    if let Some(metadata) = terminal_to_restore {
                        ...
                        panel.load_agent_thread(
                            ...
                            info.session_id
                                .clone()
                                .map(|session_id| acp::SessionId::new(session_id)),
                            false,
                            AgentThreadSource::AgentPanel,
                            window,
                            cx,
    ...
    pub fn focus_plan_from_workspace(
        workspace: &mut Workspace,
        ...
        cx.defer(move |cx| {
            window_handle
                .update(cx, |_, window, cx| {
                    panel.update(cx, |panel, cx| {
                        panel.focus_plan(focus_spec_path, window, cx);
                    });
                })
                .ok();
        });
    ...
    pub fn apply_implement_layout_from_workspace(
        workspace: &mut Workspace,
        window: &mut Window,
        cx: &mut Context<Workspace>,
    ) {
        ...
        let panel = panel.clone();
        panel.update(cx, |panel, cx| {
            let on_plan = matches!(panel.base_view, BaseView::Plan { .. });
            if let Some(ticket_id) = panel.active_implement_ticket_id(cx) {
                let (task_progress, ref_count) = panel
                    .plan_store(cx)
                    .map(|store| {
                        let store = store.read(cx);
                        (
                            store
                                .collapsed_task_progress
                                .clone()
                                .unwrap_or_else(|| "—".to_string()),
                            store.collapsed_ref_count,
                        )
                    })
                ...
                panel.show_threads_surface(window, cx);
            }
        });
    }

    pub fn focus_agent_chat_from_workspace(
        workspace: &mut Workspace,
        window: &mut Window,
        cx: &mut Context<Workspace>,
    ) {
        if let Some(workspace_id) = workspace.database_id()
            && activate_detached_threads_window(workspace_id, cx)
        {
            return;
        }
    ...
    pub fn focus_terminal_from_workspace(
        workspace: &mut Workspace,
        window: &mut Window,
        cx: &mut Context<Workspace>,
    ) {
        ...
        cx.defer(move |cx| {
            window_handle
                .update(cx, |_, window, cx| {
                    panel.update(cx, |panel, cx| {
                        panel.show_terminal_surface(window, cx);
                    });
                })
                .ok();
        });
    ...
    pub fn dock_threads_from_detached(
        workspace: Entity<Workspace>,
        workspace_id: WorkspaceId,
        cx: &mut App,
    ) {
        close_detached_threads_window(workspace_id, cx);

        let Some(main_window) = crate::planning_hub::multi_workspace_window_for(&workspace, cx)
        else {
            return;
        };
    ...
    fn is_threads_detached(&self, cx: &mut App) -> bool {
        self.workspace_id
            .map(|id| {
                let store = plan_store_for(id, cx);
                store.read(cx).is_threads_detached()
            })
            .unwrap_or(false)
    }

    pub fn open_detached_threads_window(
        &mut self,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
        ...
        if let Some(handle) = self
            .plan_store(cx)
            .map(|store| store.read(cx).detached_threads_window)
            .flatten()
        {
            if handle
            ...
            self.plan_store(cx).map(|store| {
                store.update(cx, |store, _| store.detached_threads_window = None);
            });
        }

        if !matches!(self.base_view, BaseView::AgentThread { .. }) {
            self.show_threads_surface(window, cx);
        }

        let workspace = self
            .workspace
            .upgrade()
            .expect("agent panel workspace");
        let plan_store = self
            .plan_store(cx)
            .expect("plan store for open project");

        let detached = cx
            .open_window(WindowOptions::default(), move |_window, cx| {
                cx.new(|cx| DetachedAgentChatView::new(workspace.clone(), workspace_id, cx))
            })
            .log_err();

        if let Some(handle) = detached {
            plan_store.update(cx, |store, _| {
                store.detached_threads_window = Some(handle.into());
            });
            cx.notify();
        }
    }

    fn render_threads_detached_placeholder(
        &self,
        cx: &mut Context<Self>,
    ) -> impl IntoElement {
        v_flex()
            ...
            .p_4()
            .child(
                Label::new("Agent chat is open in a separate window")
                    .size(LabelSize::Small)
                    .color(Color::Muted),
            )
            .child(
                Button::new("dock-threads-inline", "Dock chat to panel")
                    .style(ButtonStyle::Outlined)
                    .on_click(cx.listener(|this, _, _window, cx| {
                        if let Some(workspace_id) = this.workspace_id {
                            let workspace = this.workspace.upgrade().expect("workspace");
                            Self::dock_threads_from_detached(workspace, workspace_id, cx);
                        }
                        cx.notify();
                    })),
            )
    ...
    fn ensure_plan_indices(&mut self, cx: &mut Context<Self>) {
    ...
    pub fn focus_plan(
        ...
        let (plan_view, created) = if let BaseView::Plan { plan_view } = &self.base_view {
            (plan_view.clone(), false)
        } else {
            let focus = focus_spec_path.clone();
            (
                cx.new(|cx| {
                    PlanningHubView::new_for_panel(
                        workspace.clone(),
                        workspace_id,
                        focus,
                        window,
        ...
        if !created && focus_spec_path.is_some() {
            let path = focus_spec_path;
            plan_view.update(cx, |view, cx| {
                view.apply_focus_spec_path(path, window, cx);
            });
        }

        self.set_base_view(BaseView::Plan { plan_view }, true, window, cx);

        if created {
            crate::layout_studio::prompt_layout_studio_for_plan(&workspace, window, cx);
        }
    }

    pub fn collapse_plan_after_implement(
        &mut self,
        ticket_id: &str,
        ...
        store.update(cx, |store, _| {
            store.collapse_for_ticket(ticket_id, task_progress, ref_count);
        });
        self.ensure_message_editor_expanded(window, cx);
        cx.notify();
    }

    fn clear_plan_collapsed(&mut self, cx: &mut Context<Self>) {
        let Some(workspace_id) = self.workspace_id else {
            return;
        };
        plan_store_for(workspace_id, cx).update(cx, |store, _| {
            store.clear_collapsed();
        });
        cx.notify();
    }

    fn plan_store(&self, cx: &mut App) -> Option<Entity<crate::plan_store::PlanStore>> {
        self.workspace_id.map(|id| plan_store_for(id, cx))
    }

    fn active_implement_ticket_id(&self, cx: &App) -> Option<String> {
        let conversation_view = self.active_conversation_view()?;
    ...
    fn plan_strip_ticket_id(&self, cx: &mut App) -> Option<String> {
        self.active_implement_ticket_id(cx).or_else(|| {
            self.plan_store(cx)
                .and_then(|store| store.read(cx).collapsed_ticket_id.clone())
        })
    }

    fn should_show_plan_strip(&self, cx: &mut App) -> bool {
        if !matches!(self.base_view, BaseView::AgentThread { .. }) {
            return false;
        }
        let Some(store) = self.plan_store(cx) else {
            return false;
        };
        if !store.read(cx).plan_collapsed {
            return false;
        }
        self.active_implement_ticket_id(cx).is_some()
    }

    fn expand_plan_from_strip(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        let ticket_id = self.plan_strip_ticket_id(cx);
        self.clear_plan_collapsed(cx);
        self.ensure_plan_indices(cx);
        self.focus_plan(ticket_id, window, cx);
    }

    fn ensure_message_editor_expanded(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        active_thread.update(cx, |active_thread, cx| {
            // Only auto-expand the composer for threads with no history (the
            // draft "big editor" state). Threads with messages must keep the
            // compact composer — the expanded editor takes 80% of the window
            // and buries the message list when switching back from Plan. A
            // user-initiated expand (cmd-option-esc) is left untouched.
            let has_messages = !active_thread.thread.read(cx).entries().is_empty();
            if !has_messages && !active_thread.editor_expanded {
                active_thread.set_editor_is_expanded(true, cx);
            }
            active_thread.focus_handle(cx).focus(window, cx);
        });
    }

    pub fn show_threads_surface(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        if matches!(self.base_view, BaseView::AgentThread { .. }) {
        ...
        if let Some(thread_id) = self.preferred_thread_for_threads_surface(cx) {
    ...
    fn conversation_active_ticket_id(
        conversation_view: &Entity<ConversationView>,
        cx: &App,
    ) -> Option<String> {
        conversation_view
            .read(cx)
            .as_native_thread(cx)
            .and_then(|thread| thread.read(cx).active_ticket_id().map(str::to_string))
            .or_else(|| {
                conversation_view
                    .read(cx)
                    .root_thread(cx)
    ...
    fn preferred_thread_for_threads_surface(&self, cx: &mut App) -> Option<ThreadId> {
        let implement_threads = self
            .retained_threads
            .iter()
            .filter(|(_, view)| Self::conversation_active_ticket_id(view, cx).is_some())
            .map(|(id, _)| *id)
            .collect::<Vec<_>>();

        if let Some(store) = self.plan_store(cx) {
            let store = store.read(cx);
            if store.plan_collapsed {
                if let Some(ticket_id) = store.collapsed_ticket_id.as_deref() {
                    for (id, view) in &self.retained_threads {
                        if Self::conversation_active_ticket_id(view, cx).as_deref()
                            == Some(ticket_id)
                        {
                            return Some(*id);
                        }
                ...
                if let Some(id) = implement_threads.first().copied() {
                    return Some(id);
                }
            }
        }

        if implement_threads.len() == 1 {
            return Some(implement_threads[0]);
        }
            ...
            self.retained_threads
                ...
                .filter_map(|id| {
                    store
                        .read(cx)
                        .entry(*id)
                        .map(|metadata| (*id, metadata.interacted_at))
                })
                .max_by_key(|(_, interacted_at)| *interacted_at)
                .map(|(id, _)| id)
        })
    }

    fn show_plan_surface(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        let ticket_id = self.plan_strip_ticket_id(cx);
        self.clear_plan_collapsed(cx);
        self.ensure_plan_indices(cx);
        self.focus_plan(ticket_id, window, cx);
    ...
    pub fn show_terminal_surface(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        if let BaseView::Terminal { .. } = self.base_view {
            self.focus_handle(cx).focus(window, cx);
            return;
        }
    ...
    fn manage_skills(
        ...
        window.dispatch_action(
            Box::new(cuecode_actions::OpenSettingsAt {
                path: cuecode_actions::AGENT_SKILLS_SETTINGS_PATH.to_string(),
                target: None,
            }),
            cx,
        );
    ...
    pub(crate) fn open_configuration(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        window.dispatch_action(Box::new(cuecode_actions::OpenSettings), cx);
    }
    ...
    pub fn sync_active_thread_list_state(&self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        thread_view.update(cx, |thread_view, cx| {
            thread_view.ensure_list_state_matches_entries(window, cx);
            thread_view.sync_editor_mode_for_empty_state(cx);
        });
    }

    pub fn should_force_reload_thread(&self, thread_id: ThreadId, cx: &App) -> bool {
        Self::thread_metadata_expects_history(thread_id, cx)
            && self.in_memory_thread_has_empty_history(thread_id, cx)
    }

    fn thread_metadata_expects_history(thread_id: ThreadId, cx: &App) -> bool {
        ThreadMetadataStore::try_global(cx)
            .and_then(|store| store.read(cx).entry(thread_id).cloned())
            .is_some_and(|metadata| !metadata.is_draft() && metadata.session_id.is_some())
    }

    fn conversation_has_empty_history(
        conversation_view: &Entity<ConversationView>,
        cx: &App,
    ) -> bool {
        let conversation = conversation_view.read(cx);
        ...
        conversation
            .root_thread_view()
            .is_none_or(|thread_view| thread_view.read(cx).thread.read(cx).entries().is_empty())
    }

    pub fn in_memory_thread_has_empty_history(&self, thread_id: ThreadId, cx: &App) -> bool {
        if let BaseView::AgentThread { conversation_view } = &self.base_view {
            if conversation_view.read(cx).thread_id == thread_id
                && Self::conversation_has_empty_history(conversation_view, cx)
            {
                return true;
            }
        }
    ...
    fn evict_in_memory_thread(&mut self, thread_id: ThreadId, cx: &mut Context<Self>) {
        self.retained_threads.remove(&thread_id);
        if self
            .draft_thread
            .as_ref()
    ...
    fn set_base_view(
        ...
        let refresh_for_plan_threads_switch = matches!(
            (&old_view, &self.base_view),
            (BaseView::Plan { .. }, BaseView::AgentThread { .. })
                | (BaseView::AgentThread { .. }, BaseView::Plan { .. })
        );
        self.retain_running_thread(old_view, cx);
        ...
        cx.emit(AgentPanelEvent::ActiveViewChanged);
        if refresh_for_plan_threads_switch {
            window.refresh();
        }
        self.sync_active_thread_list_state(window, cx);
        cx.notify();
    }
        ...
        self._base_view_observation = match &self.base_view {
            BaseView::AgentThread { conversation_view } => {
                ...
                Some(cx.observe_in(&cv, window, |this, server_view, window, cx| {
                    ...
                    this.sync_active_thread_list_state(window, cx);
                    cx.emit(AgentPanelEvent::ActiveViewChanged);
                    this.serialize(cx);
                    cx.notify();
                }))
            ...
            BaseView::Plan { plan_view } => {
                self._thread_view_subscription = None;
                let focus_handle = plan_view.focus_handle(cx);
                self._active_thread_focus_subscription =
                    Some(cx.on_focus_in(&focus_handle, window, |_this, _window, cx| {
                        cx.emit(AgentPanelEvent::ActiveViewFocused);
                        cx.notify();
    ...
    fn visible_surface(&self) -> VisibleSurface<'_> {
        match &self.base_view {
            ...
            BaseView::Plan { plan_view } => VisibleSurface::Plan(plan_view),
    ...
    pub fn load_agent_thread(
        ...
        let must_reload_from_storage = Self::thread_metadata_expects_history(thread_id, cx)
            && self.in_memory_thread_has_empty_history(thread_id, cx);

        if must_reload_from_storage {
            self.evict_in_memory_thread(thread_id, cx);
        } else {
        ...
        // Not in memory (or stale in-memory view was evicted). Build a fresh
        // ConversationView. For drafts we also seed the message editor with
        // any prompt text the user had typed before closing the window.
        ...
        cx.spawn(async move |cx| {
            ...
            let target_panel = if request.use_new_worktree {
                ...
                let branch_target = match request.base_ref.as_ref() {
                    Some(ref_name) => cuecode_actions::NewWorktreeBranchTarget::ExistingBranch {
                        name: ref_name.clone(),
                    },
                    None => cuecode_actions::NewWorktreeBranchTarget::CurrentBranch,
                };
                let action = cuecode_actions::CreateWorktree {
...
impl Focusable for AgentPanel {
    fn focus_handle(&self, cx: &App) -> FocusHandle {
        match self.visible_surface() {
            ...
            VisibleSurface::Plan(plan_view) => plan_view.focus_handle(cx),
...
impl Panel for AgentPanel {
    ...
    fn set_position(&mut self, position: DockPosition, _: &mut Window, cx: &mut Context<Self>) {
        ...
        let merged_layout = PanelLayout::from_merged_settings(
            cx.global::<SettingsStore>().merged_settings(),
        );
        let agent_dock: settings::DockPosition = position.into();
        settings::update_settings_file(self.fs.clone(), cx, move |settings, _| {
            settings
                .agent
                .get_or_insert_default()
                .set_dock(agent_dock);
            PanelLayout::reconcile_side_panels_for_agent_dock(
                agent_dock,
                &merged_layout,
                settings,
            );
    ...
    fn min_size(&self, window: &Window, cx: &App) -> Option<Pixels> {
        match self.position(window, cx) {
            ...
            DockPosition::Bottom => Some(MIN_PANEL_HEIGHT),
        }
    }
...
impl AgentPanel {
    ...
    fn destination_has_meaningful_state(&self, cx: &App) -> bool {
        ...
        match &self.base_view {
            ...
            BaseView::Plan { .. } | BaseView::Terminal { .. } => true,
            BaseView::AgentThread { conversation_view } => {
    ...
    fn render_spec_pin_chip(&self, cx: &Context<Self>) -> Option<AnyElement> {
        let BaseView::AgentThread { conversation_view } = &self.base_view else {
            return None;
        };
        conversation_view
            .read(cx)
            .root_thread_view()
            .and_then(|view| view.read(cx).spec_pin_chip.clone())
            .map(|chip| chip.into_any_element())
    }
    ...
    fn render_title_view(&self, window: &mut Window, cx: &Context<Self>) -> AnyElement {
        let content = match self.visible_surface() {
            ...
            VisibleSurface::Plan(_) => Label::new("Plan")
                .color(Color::Muted)
                .truncate()
                .into_any_element(),
        };
    ...
    fn render_panel_options_menu(
        ...
        let workspace = self.workspace.clone();
        let current_dock = agent_panel_dock_position(cx);
                    ...
                    Some(ContextMenu::build(window, cx, |mut menu, _window, _| {
                        ...
                        if !showing_terminal {
                            menu = menu
                                ...
                                .action(
                                    "Install New Servers…",
                                    Box::new(cuecode_actions::Extensions {
                                        category_filter: Some(
                                            cuecode_actions::ExtensionCategoryFilter::ContextServers,
                                        ),
                                        id: None,
                                    }),
                                )
                        ...
                        let current_dock = current_dock;
                        let workspace_for_dock = workspace.clone();
                        menu = menu
                            .separator()
                            .action(
                                "Arrange Workspace…",
                                Box::new(cuecode_actions::ArrangeWorkspace),
                            )
                            .separator()
                            .header("Panel Position (Advanced)")
                            .toggleable_entry(
                                "Dock Left",
                                current_dock == DockPosition::Left,
                                IconPosition::Start,
                                None,
                                {
                                    let workspace = workspace_for_dock.clone();
                                    move |window, cx| {
                                        workspace
                                            .update(cx, |workspace, cx| {
                                                if let Some(panel) =
                            ...
                            .toggleable_entry(
                                "Dock Right",
                                current_dock == DockPosition::Right,
                                IconPosition::Start,
                                None,
                                {
                                    let workspace = workspace_for_dock.clone();
                            ...
                            .toggleable_entry(
                                "Dock Bottom",
                                current_dock == DockPosition::Bottom,
                                IconPosition::Start,
                                None,
                                {
                                    let workspace = workspace_for_dock.clone();
    ...
    fn render_toolbar(&self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let showing_terminal = matches!(self.visible_surface(), VisibleSurface::Terminal(_));
        let showing_plan = matches!(self.visible_surface(), VisibleSurface::Plan(_));

        let (selected_agent_custom_icon, selected_agent_label) = if showing_terminal {
            (None, SharedString::from("Terminal"))
        } else if showing_plan {
            (None, SharedString::from("Plan"))
        ...
        let active_thread = match &self.base_view {
            ...
            BaseView::Terminal { .. } | BaseView::Uninitialized | BaseView::Plan { .. } => None,
        };
            ...
            Rc::new(move |window, cx| {
                ...
                Some(ContextMenu::build(window, cx, |menu, _window, cx| {
                    menu.context(focus_handle.clone())
                        ...
                        .item(
                            ContextMenuEntry::new(format!("{} Agent", paths::APP_NAME))
                                .when(
                        ...
                        .item(
                            ContextMenuEntry::new("Add More Agents")
                                ...
                                .handler({
                                    move |window, cx| {
                                        window
                                            .dispatch_action(Box::new(cuecode_actions::AcpRegistry), cx)
                                    }
                                }),
                        )
                }))
        ...
        } else if matches!(self.base_view, BaseView::Plan { .. }) {
            ToolbarMode::Plan
        ...
        let show_detach_threads = matches!(
            mode,
            ToolbarMode::EmptyThread | ToolbarMode::ActiveThread
        ) && !self.is_threads_detached(cx);
        let detach_threads_button = IconButton::new("detach-threads-window", IconName::ArrowUpRight)
            .icon_size(IconSize::Small)
            .tooltip(Tooltip::text("Open Agent Chat in New Window"))
            .on_click(cx.listener(|this, _, window, cx| {
                this.open_detached_threads_window(window, cx);
            }));
            ...
            base_container
                .child(
                    h_flex()
                        ...
                        .when_some(self.render_spec_pin_chip(cx), |this, chip| {
                            this.child(chip).child(
                                div()
                                    .h(rems(1.25))
                                    .w(px(1.))
                                    .flex_none()
                ...
                .child(
                    h_flex()
                        ...
                        .when(show_detach_threads, |this| this.child(detach_threads_button))
    ...
    fn render_agent_panel_chrome(
        ...
        v_flex()
            ...
            .when(can_create_entries && !self.is_overlay_open(), |this| {
                this.child(self.render_surface_tabs(window, cx))
            })
            .when(self.should_show_plan_strip(cx), |this| {
                this.child(self.render_plan_strip(cx))
            })
    }

    fn render_plan_strip(&self, cx: &mut Context<Self>) -> impl IntoElement {
        let store = self
            .plan_store(cx)
            .expect("render_plan_strip called when should_show_plan_strip is true");
        let store = store.read(cx);
        let ticket_id = store
            .collapsed_ticket_id
            .clone()
            .or_else(|| self.active_implement_ticket_id(cx))
            .unwrap_or_else(|| "phase".to_string());
        let progress = store.collapsed_task_progress.as_deref().unwrap_or("—");
        let ref_count = store.collapsed_ref_count;
        let specs = if ref_count == 1 {
            "1 spec".to_string()
        } else {
            format!("{ref_count} specs")
        };
        let label = format!("📎 {ticket_id} · {progress} · {specs}");

        h_flex()
            .id("agent-panel-plan-strip")
            .h(Tab::container_height(cx))
            .flex_shrink_0()
            .w_full()
            .items_center()
            ...
            .bg(cx.theme().colors().tab_bar_background)
            .child(
                Button::new("plan-strip-summary", label)
                    .style(ButtonStyle::Subtle)
                    .label_size(LabelSize::Small)
                    .tooltip(Tooltip::text("Open Plan for this phase"))
                    .on_click(cx.listener(|this, _, window, cx| {
                        this.expand_plan_from_strip(window, cx);
                    })),
            )
            .child(
                IconButton::new("plan-strip-expand", IconName::ArrowUpRight)
                    .icon_size(IconSize::Small)
    ...
    fn render_surface_tabs(
        ...
        let focus_handle = self.focus_handle(cx);
        let threads_focus_handle = focus_handle.clone();
        let plan_focus_handle = focus_handle;

        // Terminal surfaces (agent terminal threads) are still reachable from the
        // thread switcher, but don't get a tab: the workspace terminal panel is
        // the primary terminal surface. An out-of-range index leaves no tab
        // highlighted while a terminal thread is visible.
        let selected_index = match self.visible_surface() {
            VisibleSurface::AgentThread(_) | VisibleSurface::Uninitialized => 0,
            VisibleSurface::Plan(_) => 1,
            VisibleSurface::Terminal(_) => 2,
        };

        h_flex()
            .id("agent-panel-surface-tabs")
            .h(Tab::container_height(cx))
            .flex_shrink_0()
            .w_full()
            .items_center()
            ...
            .child(
                ToggleButtonGroup::single_row(
                    "agent-panel-surfaces",
                    [
                        ToggleButtonSimple::new(
                            "Threads",
                            cx.listener(|this, _, window, cx| {
                                this.show_threads_surface(window, cx);
                            }),
                        )
                        .tooltip(move |_, cx| {
                            Tooltip::for_action_in(
                                "Threads",
                                &cuecode_actions::FocusAgentChat,
                                &threads_focus_handle,
                                cx,
                            )
                        }),
                        ToggleButtonSimple::new(
                            "Plan",
                            cx.listener(|this, _, window, cx| {
                                this.show_plan_surface(window, cx);
                            }),
                        )
                        .tooltip(move |_, cx| {
                            Tooltip::for_action_in(
                                "Plan",
                                &cuecode_actions::FocusPlan::default(),
                                &plan_focus_handle,
                                cx,
                            )
                        }),
    ...
    fn should_render_new_user_onboarding(&mut self, cx: &mut Context<Self>) -> bool {
        ...
        match &self.base_view {
            BaseView::Uninitialized | BaseView::Terminal { .. } | BaseView::Plan { .. } => false,
            BaseView::AgentThread { conversation_view } => {
    ...
    fn handle_drop(
        ...
        match &self.base_view {
            ...
            BaseView::Uninitialized | BaseView::Plan { .. } => {}
        }
    }
...
impl Render for AgentPanel {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let content = v_flex()
            ...
            .child(self.render_agent_panel_chrome(window, cx))
            ...
            .child({
                ...
                match self.visible_surface() {
                    VisibleSurface::Uninitialized if !self.has_open_project(cx) => {
                        body = body.child(self.render_no_project_state(cx));
                    }
                    VisibleSurface::Uninitialized => {}
                    VisibleSurface::AgentThread(_) if self.is_threads_detached(cx) => {
                        body = body.child(self.render_threads_detached_placeholder(cx));
                    }
                    VisibleSurface::AgentThread(conversation_view) => {
                        body = body.child(
                            div()
                                .id("agent-surface-threads")
                                .flex_1()
                                .min_h_0()
                                .min_w_0()
                                .size_full()
                    ...
                    VisibleSurface::Plan(plan_view) => {
                        body = body.child(
                            div()
                                .id("agent-surface-plan")
                                .flex_1()
                                .min_h_0()
                                .min_w_0()
                                .size_full()
                                .overflow_hidden()
                                .child(plan_view.clone()),
                        );
                    }
                ...
                body.child(self.render_drag_target(cx))
            })
...
mod tests {
    ...
    async fn test_new_thread_dismisses_settings_overlay(cx: &mut TestAppContext) {
        ...
        panel.update_in(&mut cx, |panel, window, cx| {
            panel.set_test_overlay(true, true, window, cx);
        });
        cx.run_until_parked();
