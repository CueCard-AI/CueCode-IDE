...
pub struct RootPlan {
    ...
    /// recorded when CueCode created the worktree. [`remove_root`] re-stats the
    /// directory and refuses to delete anything if the time has changed,
    /// which means the worktree was recreated outside CueCode.
...
/// Resolves the CueCode-managed worktrees base directory for a given repo.
///
...
pub fn build_root_plan(
    ...
    // Only archive worktrees that live inside the CueCode-managed worktrees
    ...
    // Only archive worktrees that CueCode explicitly created. The directory
...
pub async fn remove_root(root: RootPlan, cx: &mut AsyncApp) -> Result<()> {
    ...
    // user later creates a new worktree at the same path outside CueCode, a
...
/// Confirms that the worktree on disk is still the one CueCode created, by
/// comparing the creation time of its git metadata directory against the
/// time recorded when CueCode created it.
///
/// Outcomes:
...
async fn verify_created_by_zed(root: &RootPlan, cx: &mut AsyncApp) -> Result<()> {
    ...
    let created_at = receiver
        ...
        .with_context(|| {
            format!(
                "refusing to delete worktree at {}: failed to verify that CueCode created it",
    ...
    match created_at {
        ...
        Some(_) => {
            ...
            .log_err();
            Err(anyhow!(
                "refusing to delete worktree at {}: it is not the worktree CueCode created \
                 (it was likely removed and recreated outside CueCode)",
                root.root_path.display()
            ))
        }
    }
...
mod tests {
        ...
        workspace.read_with(cx, |_workspace, cx| {
            ...
            assert!(
                ...
                "build_root_plan should return None for a linked worktree \
                 outside the CueCode-managed worktrees directory",
            );
        });
    }
    ...
    async fn test_build_root_plan_returns_none_for_unrecorded_linked_worktree_in_managed_directory(
        ...
        workspace.read_with(cx, |_workspace, cx| {
            ...
            assert!(
                plan.is_none(),
                "build_root_plan should return None for a linked worktree CueCode didn't create, \
                 even when it lives inside the CueCode-managed worktrees directory",
            );
        });
    }
    ...
    async fn test_remove_root_refuses_when_worktree_recreated_outside_zed(cx: &mut TestAppContext) {
        ...
        // simulating a worktree that was removed and recreated outside CueCode
        ...
        assert!(
            error.to_string().contains("not the worktree CueCode created"),
