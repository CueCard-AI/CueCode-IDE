...
use cuecode_actions::agent::{Chat, ToggleModelSelector};
...
use crate::spec_pin_chip::SpecPinChip;
...
impl ConversationView {
    ...
    fn new_thread_view(
        ...
        let native_thread = connection
            ...
            .downcast::<agent::NativeAgentConnection>()
            .and_then(|native_connection| native_connection.thread(&session_id, cx));
        let spec_pin_chip = cx.new(|cx| {
            SpecPinChip::new(
                thread.clone(),
                native_thread.map(|t| t.downgrade()),
                self.workspace.clone(),
                self.focus_handle(cx),
                cx,
            )
    ...
    fn spawn_external_agent_login(
        ...
        window.spawn(cx, async move |cx| {
            ...
            if let Some(cmd) = &task.command {
                // Have "node" command use CueCode's managed Node runtime by default
                if cmd == "node" {
    ...
    fn render_unsupported(
        ...
        let (heading_label, description_label) = (
            format!("Upgrade {} to work with CueCode", self.agent.agent_id()),
            if version.is_empty() {
                format!(
...
pub(crate) mod tests {
    ...
    async fn test_move_up_in_empty_editor_restores_last_queued_message(cx: &mut TestAppContext) {
        ...
        editor.update_in(cx, |_editor, window, cx| {
            window.dispatch_action(Box::new(cuecode_actions::editor::MoveUp), cx);
        });
        cx.run_until_parked();
