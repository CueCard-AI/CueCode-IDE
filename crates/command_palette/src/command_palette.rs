...
use client::{parse_zed_link, app_urls::is_app_deep_link};
...
use cuecode_actions::{OpenAppUrl, command_palette::Toggle};
        ...
        let commands = window
            ...
            .filter_map(|action| {
                ...
                Some(Command {
                    ...
                    search_aliases: cursor_palette_aliases::aliases_for_action(action.name())
                        .iter()
...
impl PickerDelegate for CommandPaletteDelegate {
    ...
    fn update_matches(
        ...
        let is_app_link =
            is_app_deep_link(query_str) || parse_zed_link(query_str, cx).is_some();
        ...
        let task = cx.background_spawn({
            ...
            async move {
                ...
                let candidates = commands
                    ...
                    .enumerate()
                    .flat_map(|(ix, command)| {
                        std::iter::once(StringMatchCandidate::new(ix, &command.name)).chain(
                            command
                                .search_aliases
                                .iter()
                                .map(move |alias| StringMatchCandidate::new(ix, alias)),
                        )
                    })
                    .collect::<Vec<_>>();
                ...
                let intercept_result = if is_app_link {
    ...
    fn confirm(&mut self, secondary: bool, window: &mut Window, cx: &mut Context<Picker<Self>>) {
        if secondary {
            ...
            let open_keymap = Box::new(cuecode_actions::ChangeKeybinding {
...
mod tests {
    ...
    #[test]
    fn test_cursor_palette_aliases() {
        let aliases = cursor_palette_aliases::aliases_for_action("cuecode::ReloadWindow");
        assert!(aliases.iter().any(|alias| alias.contains("Developer")));
    }
