use agent_skills::{
    MAX_SKILL_DESCRIPTION_LEN, MAX_SKILL_FILE_SIZE, SKILL_FILE_NAME, SkillMetadata,
    SkillsUpdatedHook, global_skills_dir, parse_skill_file_content, project_skills_relative_path,
...
impl ScopeChoice {
    /// Absolute path of the project-local skills directory this scope writes to.
    fn skills_dir(&self) -> PathBuf {
        match self {
            ScopeChoice::Global => global_skills_dir(),
            ScopeChoice::Project { abs_path, .. } => abs_path.join(project_skills_relative_path()),
        }
    }
}
            ...
            .child(match &self.url_import_status {
                UrlImportStatus::Idle => Label::new(
                    "Paste a GitHub .md URL to fetch it and fill out the form. \
                     For private files, CueCode retries using GITHUB_TOKEN, if set.",
                )
