...
pub fn init(cx: &mut App) {
    ...
    cx.on_action(|_: &cuecode_actions::assistant::OpenSkillCreator, cx| {
        open_skill_creator(pages::SkillCreatorOpenMode::Form, None, cx);
    });
    cx.on_action(|_: &cuecode_actions::assistant::CreateSkillFromUrl, cx| {
    ...
    cx.observe_new(|workspace: &mut workspace::Workspace, _, _| {
        workspace
            ...
            .register_action(
                |_, _: &cuecode_actions::assistant::OpenSkillCreator, window, cx| {
                    let window_handle = window.window_handle().downcast::<MultiWorkspace>();
                    open_skill_creator(pages::SkillCreatorOpenMode::Form, window_handle, cx);
                },
            )
            .register_action(
                |_, _: &cuecode_actions::assistant::CreateSkillFromUrl, window, cx| {
                    let window_handle = window.window_handle().downcast::<MultiWorkspace>();
                    let initial_url = pages::skill_url_from_clipboard(cx);
                    open_skill_creator(
...
fn open_settings_editor_at_target(
    ...
    fn open_path(
        ...
        settings_window.reset_to_unfiltered(window, cx);

        if settings_window.navigate_to_sub_page_in_nav(path, window, cx) {
            cx.notify();
            return;
        }
    ...
    open_settings_editor_with(workspace_handle, cx, move |settings_window, window, cx| {
        ...
        if let Some(path) = path {
            ...
            settings_window.reset_to_unfiltered(window, cx);
            settings_window.open_first_nav_page();
            cx.notify();
        }
    });
}
        ...
        cx.open_window(
            WindowOptions {
                titlebar: Some(TitlebarOptions {
                    title: Some(format!("{} — Settings", APP_NAME).into()),
            ...
            IconButton::new((id.into(), "copy-link-btn"), link_icon)
                ...
                .when_some(json_path, |this, path| {
                    this.on_click(cx.listener(move |this, _, _, cx| {
                        let link = format!("cuecode://settings/{}", path);
...
fn all_language_names(cx: &App) -> Vec<SharedString> {
    ...
    state
        ...
        .language_names()
        .into_iter()
        .filter(|name| name.as_ref() != "CueCode Keybind Context")
        .map(Into::into)
        .collect()
}
...
impl SettingsWindow {
    ...
    fn reset_to_unfiltered(&mut self, window: &mut Window, cx: &mut Context<SettingsWindow>) {
        self.sub_page_stack.clear();
        self.search_task.take();
    ...
    fn navigate_to_sub_page_in_nav(
        &mut self,
        json_path: &str,
        window: &mut Window,
        cx: &mut Context<SettingsWindow>,
            ...
            page.items.iter().any(|item| {
                matches!(
                    ...
                    SettingsPageItem::SubPageLink(link)
                        if link.json_path == Some(json_path)
                )
            })
        }) else {
            return false;
        ...
        if let Some(navbar_entry_index) = self.navbar_entries.iter().position(|entry| {
            entry.page_index == page_index && entry.is_root
        }) {
            self.open_navbar_entry_page(navbar_entry_index);
        }
...
pub mod test {
    ...
    async fn test_skills_page_scope_switch_updates_displayed_skills(cx: &mut gpui::TestAppContext) {
        ...
        let project_skills: Vec<Skill> = load_skills_from_directory(
            &fs,
            Path::new("/project/.cursor/skills"),
    ...
    async fn test_open_skill_creator_action_opens_settings_window_at_sub_page(
        ...
        multi_workspace.update_in(cx, |_multi_workspace, window, cx| {
            window.dispatch_action(Box::new(cuecode_actions::assistant::OpenSkillCreator), cx);
        });
