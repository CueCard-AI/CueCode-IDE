use anyhow::Result;
use client::{Client, ClientSettings, UserStore, zed_urls};
...
use cuecode_actions::{OpenBrowser, OpenSettingsAt};
...
impl Render for EditPredictionButton {
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        match language_settings.edit_predictions.provider {
            ...
            provider @ (EditPredictionProvider::CueCode | EditPredictionProvider::Mercury) => {
                let enabled = self.editor_enabled.unwrap_or(true);
                let file = self.file.clone();
                let language = self.language.clone();
                let project = self.project.clone();
                let provider_name: &'static str = match provider {
                    EditPredictionProvider::CueCode => "cuecode",
                    _ => "unknown",
                };
                let icons = self
                    ...
                    .map(|p| p.icons(cx))
                    .unwrap_or_else(|| {
                        edit_prediction_types::EditPredictionIconSet::new(IconName::EditPredict)
                    });
                        ...
                        IconButton::new("zed-predict-pending-button", ep_icon)
                            ...
                            .on_click(cx.listener(move |_, _, window, cx| {
                                ...
                                window.dispatch_action(
                                    cuecode_actions::OpenEditPredictionOnboarding.boxed_clone(),
                                    cx,
                                );
                            })),
                    );
                ...
                let zed_cloud_needs_sign_in =
                    matches!(provider, EditPredictionProvider::CueCode) && user.is_none();
...
impl EditPredictionButton {
        ...
        if !providers.is_empty() {
            ...
            for provider in providers {
                ...
                let is_disabled_zed_provider =
                    provider == EditPredictionProvider::CueCode && is_zed_provider_disabled;
    ...
    pub fn build_language_settings_menu(
        ...
        if matches!(provider, EditPredictionProvider::CueCode) {
                                ...
                                v_flex()
                                    ...
                                    .child(
                                        Label::new(indoc!{
                                            ...
                                            CueCode must detect a license file in your repo for this setting to take effect. \
        ...
        menu = menu.item(
            ContextMenuEntry::new("Configure Excluded Files")
                ...
                .documentation_aside(DocumentationSide::Left, |_| {
                    Label::new(indoc!{"
                        Open your settings to add sensitive paths for which CueCode will never predict edits."}).into_any_element()
                })
                .handler(move |window, cx| {
                    telemetry::event!(
                        "Edit Prediction Menu Action",
    ...
    fn build_edit_prediction_context_menu(
        ...
        ContextMenu::build(window, cx, |mut menu, window, cx| {
            let user = self.user_store.read(cx).current_user();
            let cloud_enabled = !ClientSettings::get_global(cx).server_url.is_empty();

            let needs_sign_in = user.is_none()
                && matches!(
                    provider,
                    EditPredictionProvider::None | EditPredictionProvider::CueCode
                );

            if needs_sign_in && cloud_enabled {
                menu = menu
...
fn render_zeta_tab_animation(cx: &App) -> impl IntoElement {
    ...
    h_flex()
        ...
        .child(Icon::new(IconName::EditPredict))
