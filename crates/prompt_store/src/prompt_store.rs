...
/// a shared future to a global. Idempotent: a no-op if already initialized
/// (opening the same LMDB env twice in one process fails with heed's
/// "environment already open" error, which breaks the prompt store for the
/// whole process — e.g. the headless eval path calls init from multiple sites).
pub fn init(cx: &mut App) {
    if cx.has_global::<GlobalPromptStore>() {
        return;
    }
