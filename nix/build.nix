...
let
  ...
  commonArgs =
    let
      zedCargoLock = builtins.fromTOML (builtins.readFile ../crates/cuecode/Cargo.toml);
      stdenv' = stdenv;
    in
    rec {
      pname = "zed-editor";
      ...
      cargoExtraArgs = "-p cuecode -p cli --locked --features=gpui_platform/runtime_shaders";
...
craneLib.buildPackage (
  lib.recursiveUpdate commonArgs {
    ...
    preBuild = ''
      ...
      echo nightly > crates/cuecode/RELEASE_CHANNEL
    '';
    ...
    installPhase =
      if stdenv.hostPlatform.isDarwin then
        ...
        ''
          ...
          install -D "crates/cuecode/resources/app-icon-nightly@2x.png" \
            "$out/share/icons/hicolor/1024x1024@2x/apps/zed.png"
          install -D crates/cuecode/resources/app-icon-nightly.png \
          ...
          (
            ...
            ${lib.getExe envsubst} < "crates/cuecode/resources/zed.desktop.in" > "$out/share/applications/dev.zed.Zed-Nightly.desktop"
