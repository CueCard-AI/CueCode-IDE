...
Welcome to CueCode's documentation.

This is built on push to `main` and published automatically to [https://cuecode.dev/docs](https://cuecode.dev/docs).
...
To add images or videos to the docs, upload them to cuecode.dev/docs/assets, GitHub's asset storage) and then link out to them from the docs.

Putting binary assets such as images in the Git repository will bloat the repository size over time.

## Internal notes:

- We have a Cloudflare router called `docs-proxy` that intercepts requests to `cuecode.dev/docs` and forwards them to the "docs" Cloudflare Pages project.
...
{#kb scope::Action} - e.g., {#kb cuecode::OpenSettings}.
...
{#action scope::Action} - e.g., {#action cuecode::OpenSettings}.
