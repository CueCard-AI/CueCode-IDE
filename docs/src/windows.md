...
You can also build CueCode from source; see [these docs](https://cuecode.dev/docs/development/windows) for instructions.

### Package managers

Install via winget when a CueCode package is published for your channel. Until then, use the download page above.

## Uninstall

- Installed via installer: Use `Settings` → `Apps` → `Installed apps`, search for CueCode, and click Uninstall.
...
CueCode supports remote development on Windows through both SSH and WSL. You can connect to remote servers via SSH or work with files inside WSL distributions directly from CueCode.
...
### CueCode fails to start or shows a blank window

- Check that your hardware and operating system version are compatible with CueCode. See our [installation guide](./installation.md) for more information.
...
- Try launching CueCode with no extensions or custom settings to isolate conflicts.
...
#### CueCode fails to open / degraded performance

CueCode requires a DirectX 11 compatible GPU to run. If CueCode fails to open, your GPU may not meet the minimum requirements.
...
If you're running CueCode inside a virtual machine, it will use the emulated adapter provided by your VM. While CueCode will work in this environment, performance may be degraded.
