---
title: Telemetry
description: "What data CueCode collects and how to control telemetry settings."
---

# Telemetry in CueCode

CueCode collects anonymous telemetry to understand usage patterns and diagnose issues.
...
You have full control over what data is sent out by CueCode.
To enable or disable some or all telemetry types, open Settings ({#kb cuecode::OpenSettings}) and search for "telemetry", or add the following to your settings file:
...
Crash reports consist of a [minidump](https://learn.microsoft.com/en-us/windows/win32/debug/minidump-files) and debug metadata. Reports are sent on the next launch after a crash, allowing CueCode to identify and fix issues without requiring you to file a bug report.
...
Usage data is tied to a random telemetry ID. If you've authenticated, this ID may be linked to your email so CueCode can analyze patterns over time and reach out for feedback.

To audit what CueCode has reported, run {#action cuecode::OpenTelemetryLog} from the command palette or click `Help > View Telemetry Log`.
...
When using CueCode's hosted services, we collect metadata for rate limiting and billing (e.g., token usage). CueCode does not store your prompts or code unless you explicitly share feedback or opt into Edit Prediction training data collection.
...
Administrators on CueCode Business can enforce a no-sharing policy org-wide; members can't opt into [Edit Prediction training data sharing](./ai/ai-improvement.md#edit-predictions) or [AI feedback ratings](./ai/ai-improvement.md#ai-feedback-with-ratings). See [Data Sharing](./business/admin-controls.md#data-sharing) in Admin Controls.

<!-- TODO: link to telemetry org-wide disable control once it ships (currently planned for a future release) -->

## Concerns and Questions

If you have concerns about telemetry, you can [open an issue](https://github.com/zed-industries/zed/issues/new/choose) or email support@cuecode.dev.
