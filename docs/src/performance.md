...
description: "Performance profiling and optimization for CueCode development."
---

How to use our internal tools to profile and keep CueCode fast.
...
Then either compile CueCode with `ZTRACING=1 cargo r --features tracy --release`. The release build is optional but highly recommended as like every program Zeds performance characteristics change dramatically with optimizations. You do not want to chase slowdowns that do not exist in release.
