---
title: CLI Reference
description: "Reference for CueCode's command-line interface (CLI), including opening files and directories, integrating with tools, and controlling CueCode from scripts."
---

# CLI Reference

Use CueCode's command-line interface (CLI) to open files and directories, integrate with other tools, and control CueCode from scripts.

## Installation

**macOS:** Run the {#action cli::InstallCliBinary} command from the command palette ({#kb command_palette::Toggle}) to install the `cuecode` CLI to `/usr/local/bin/zed`.

**Linux:** The CLI is included with CueCode packages. The binary name may vary by distribution (commonly `zed` or `zeditor`).

**Windows:** The CLI is included with CueCode. Add CueCode's installation directory to your PATH, or use the full path to `zed.exe`.
...
This is useful for integrating CueCode with tools that expect an editor to block until editing is complete (e.g., `git commit`):

```sh
export EDITOR="zed --wait"
git commit  # Opens CueCode and waits for you to close the commit message file
```
...
Run CueCode in the foreground, keeping the terminal attached. Useful for debugging:
...
Print CueCode's version and exit:
...
Uninstall CueCode and remove all related files (macOS and Linux only):
...
Specify a custom path to the CueCode application or binary:

```sh
zed --zed /path/to/CueCode.app myfile.txt
```
...
This creates a temporary file with the stdin content and opens it in CueCode.
...
## Using CueCode as Your Default Editor

Set CueCode as your default editor for Git and other tools:
...
On Windows, the CLI supports opening paths from WSL distributions. This is handled automatically when launching CueCode from within WSL.
