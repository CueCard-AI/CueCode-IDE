---
title: How to Migrate from VS Code to CueCode
description: "Guide for migrating from VS Code to CueCode, including settings and keybindings."
---

# How to Migrate from VS Code to CueCode

This guide explains how to move from VS Code to CueCode without rebuilding your workflow.
...
After installation, you can launch CueCode from your Applications folder (macOS) or directly from the terminal (Linux) using:
`cuecode .`
This opens the current directory in CueCode.

## Import Settings from VS Code

During setup, you have the option to import key settings from VS Code. CueCode imports the following settings:
...
| VS Code Setting                             | CueCode Setting                                    |
| ------------------------------------------- | ---------------------------------------------- |
...
CueCode doesn’t import extensions or keybindings, but this import gets core editor behavior close to your VS Code setup. If you skip that step during setup, you can still import settings manually later via the command palette:

`Cmd+Shift+P → {#action cuecode::ImportVsCodeSettings}`
...
| VS Code | CueCode | Notes |
| --- | --- | --- |
| editor.fontFamily | buffer_font_family | CueCode uses CueCode Mono by default |
...
CueCode also supports per-project settings. You can find these in the Settings Editor as well.
...
### Common Shared Keybindings (CueCode <> VS Code)
...
### Different Keybindings (CueCode <> VS Code)

| Action              | VS Code               | CueCode                    |
| ------------------- | --------------------- | ---------------------- |
...
CueCode also supports chords (multi-key sequences) like `Cmd+K Cmd+C`, like VS Code does.
...
CueCode takes a different approach.
...
- **You can start from a single file or an empty window**: CueCode doesn’t require you to open a folder to begin editing.
...
- Instead of a persistent panel, CueCode encourages you to:
...
CueCode does not offer as many extensions as VS Code. The available extensions are focused on language support, themes, syntax highlighting, and other core editing enhancements.

Several features that typically require extensions in VS Code are built into CueCode:
...
You won’t find one-to-one replacements for every VS Code extension, especially if you rely on tools for DevOps, containers, or test runners. CueCode's extension catalog is still growing and remains smaller.

### Collaboration in CueCode vs. VS Code

Unlike VS Code, CueCode doesn’t require an extension to collaborate. It’s built into the core experience.
...
Learn how [CueCode uses CueCode](https://cuecode.dev/blog/zed-is-our-office) to plan work and collaborate.

### Using AI in CueCode

If you’re used to GitHub Copilot in VS Code, you can do the same in CueCode. You can also explore other agents through CueCode Pro, or bring your own keys and connect without authentication. You can disable AI features entirely if you prefer.
...
**Custom Tasks**: Define build or run commands in your `tasks.json` (accessed via command palette: {#action cuecode::OpenTasks}):
...
Copy your VS Code snippet JSON directly into CueCode's snippets folder ({#action snippets::ConfigureSnippets}).
