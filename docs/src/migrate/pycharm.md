---
title: How to Migrate from PyCharm to CueCode
description: "Guide for migrating from PyCharm to CueCode, including settings and keybindings."
---

# How to Migrate from PyCharm to CueCode

This guide covers how to set up CueCode if you're coming from PyCharm, including keybindings, settings, and the differences you should expect.

## Install CueCode

CueCode is available on macOS, Windows, and Linux.

For macOS and Windows, download CueCode from [cuecode.dev/download](https://cuecode.dev/download).

For Linux, use the installation script:

```sh
curl -f https://cuecode.dev/install.sh | sh
```

After installation, you can launch CueCode from your Applications folder (macOS), Start menu (Windows), or directly from the terminal using:
`cuecode .`
This opens the current directory in CueCode.
...
You can configure most settings in the Settings Editor ({#kb cuecode::OpenSettings}). For advanced settings, run {#action cuecode::OpenSettingsFile} from the Command Palette to edit your settings file directly.

Settings PyCharm users typically configure first:

| CueCode Setting             | What it does                                                                    |
| ----------------------- | ------------------------------------------------------------------------------- |
...
CueCode also supports per-project settings. Create a `.zed/settings.json` file in your project root to override global settings for that project, similar to how you might use `.idea` folders in PyCharm.
...
After setup, press `Cmd+Shift+O` (with JetBrains keymap) to open a folder. This becomes your workspace in CueCode. Unlike PyCharm, there's no project configuration wizard, no interpreter selection dialog, and no project structure setup required.

To start a new project, create a directory using your terminal or file manager, then open it in CueCode. The editor will treat that folder as the root of your project.

You can also launch CueCode from the terminal inside any folder with:
`cuecode .`
...
If you chose the JetBrains keymap during onboarding, most of your shortcuts should already feel familiar. Here's a quick reference for how CueCode compares to PyCharm.
...
### Different Keybindings (PyCharm → CueCode)

| Action                 | PyCharm     | CueCode (JetBrains keymap)   |
| ---------------------- | ----------- | ------------------------ |
...
CueCode doesn't index. You open a folder and start working immediately. File search and navigation stay fast regardless of project size, without waiting through indexing pauses.

PyCharm's index powers features like finding all usages across your entire codebase, understanding class hierarchies, and detecting unused imports project-wide. CueCode delegates this work to language servers, which may not analyze as deeply or as broadly.
...
CueCode uses the Language Server Protocol (LSP) for code intelligence. For Python, CueCode provides several language servers out of the box:
...
CueCode handles virtual environments through its toolchain system:

- CueCode automatically discovers virtual environments in common locations (`.venv`, `venv`, `.env`, `env`)
...
- Open the folder in CueCode—it will detect the environment automatically
- If you need to switch environments, use the toolchain selector
- For conda environments, ensure they're activated in your shell before launching CueCode

> **Tip:** If basedpyright shows import errors for packages you've installed, check that CueCode has selected the correct virtual environment. Use the toolchain selector to verify or change the active environment.
...
CueCode has no project model. A project is a folder. There's no wizard, no interpreter selection screen, no project structure configuration.
...
- Interpreter management is external. CueCode discovers environments but doesn't create them.
...
- Define common commands in `tasks.json` (open via Command Palette: {#action cuecode::OpenTasks}):
...
CueCode has none of this. The language server sees Python code as Python code—it doesn't understand that `@app.route` defines an endpoint or that a Django model class creates database tables.
...
- Consider using framework-specific CLI tools (`python manage.py`, `flask routes`) from CueCode's terminal

> **Tip:** For database work, pick up a dedicated tool like DataGrip, DBeaver, or TablePlus. Many developers who switch to CueCode keep DataGrip around specifically for SQL.

### Tool Windows vs. Docks

PyCharm organizes auxiliary views into numbered tool windows (Project = 1, Python Console = 4, Terminal = Alt+F12, etc.). CueCode uses a similar concept called "docks":

| PyCharm Tool Window | CueCode Equivalent | Shortcut (JetBrains keymap) |
| ------------------- | -------------- | --------------------------- |
...
CueCode has three dock positions: left, bottom, and right. Panels can be moved between docks by dragging or through settings.

### Debugging

Both PyCharm and CueCode offer integrated debugging, but the experience differs:

- CueCode uses `debugpy` (the same debug adapter that VS Code uses)
...
CueCode can automatically detect debuggable entry points. Press `F4` to see available options, including:
...
PyCharm has a dedicated test runner with a visual interface showing pass/fail status for each test. CueCode provides test running through:
...
CueCode's extension catalog is smaller and more focused:

- Language support and syntax highlighting
- Themes
- Context servers

Several features that require plugins in PyCharm are built into CueCode:
...
### What's Not in CueCode

To set expectations clearly, here's what PyCharm offers that CueCode doesn't have:

- **Scientific Mode / Jupyter integration** — For notebooks and data science workflows, use JupyterLab or VS Code with the Jupyter extension alongside CueCode for your Python editing
...
- **Remote interpreters** — CueCode has remote development, but it works differently
- **Profiler integration** — Use cProfile, py-spy, or similar tools externally

## Collaboration in CueCode vs. PyCharm

PyCharm offers Code With Me as a separate plugin for collaboration. CueCode has collaboration built into the core experience.

- Open the Collab Panel in the left dock
- Create a channel and [invite your collaborators](https://cuecode.dev/docs/collaboration#inviting-a-collaborator) to join
- [Share your screen or your codebase](https://cuecode.dev/docs/collaboration#share-a-project) directly
...
If you're used to AI assistants in PyCharm (like GitHub Copilot or JetBrains AI Assistant), CueCode offers similar capabilities with more flexibility.
...
Once signed in, just start typing. CueCode will offer suggestions inline for you to accept.

### Additional AI Options

To use other AI models in CueCode, you have several options:

- Use CueCode's hosted models, with higher rate limits. Requires [authentication](https://cuecode.dev/docs/authentication) and access through [CueCode Pro](https://cuecode.dev/docs/account/zed-hosted-models.html).
- Bring your own [API keys](https://cuecode.dev/docs/ai/use-api-access.html), no authentication needed
- Use [External Agents like Claude Agent](https://cuecode.dev/docs/ai/external-agents.html)

## Advanced Config and Productivity Tweaks

CueCode exposes advanced settings for power users who want to fine-tune their environment.
...
Now that you're set up, here are some resources to help you get the most out of CueCode:
...
- [AI Features](../ai/overview.md) — Explore CueCode's AI capabilities beyond code completion
- [Collaboration](../collaboration/overview.md) — Share your projects and code together in real time
- [Python in CueCode](../languages/python.md) — Python-specific setup and configuration
