---
title: Model Context Protocol (MCP) in CueCode
description: Install and configure MCP servers in CueCode to extend your AI agent with external tools, data sources, and integrations.
---

# Model Context Protocol

CueCode uses the [Model Context Protocol](https://modelcontextprotocol.io/) to interact with context servers.
...
CueCode currently supports MCP's [Tools](https://modelcontextprotocol.io/specification/2025-11-25/server/tools) and [Prompts](https://modelcontextprotocol.io/specification/2025-11-25/server/prompts) features.
We welcome contributions that help advance CueCode's MCP feature coverage (Discovery, Sampling, Elicitation, etc).

CueCode also handles the `notifications/tools/list_changed` notification from MCP servers. When a server adds, removes, or modifies its available tools at runtime, CueCode automatically reloads the tool list without requiring a server restart.

## Agent Path Support {#agent-path-support}

| Agent path                                | MCP behavior                                                                            |
| ----------------------------------------- | --------------------------------------------------------------------------------------- |
| [CueCode Agent](./cuecode-agent.md)               | Uses CueCode-configured MCP servers directly                                                |
| [External Agents](./external-agents.md)   | CueCode can forward configured MCP servers over ACP; agents may also read native MCP config |
...
One of the ways you can use MCP servers in CueCode is by exposing them as an extension.
...
1. [the CueCode website](https://cuecode.dev/extensions?filter=context-servers)
2. in the app, open the Command Palette and run the {#action cuecode::Extensions} action
...
- [Context7](https://cuecode.dev/extensions/mcp-server-context7)
- [GitHub](https://cuecode.dev/extensions/mcp-server-github)
- [Puppeteer](https://cuecode.dev/extensions/mcp-server-puppeteer)
- [Gem](https://cuecode.dev/extensions/gem)
- [Brave Search](https://cuecode.dev/extensions/mcp-server-brave-search)
- [Prisma](https://github.com/aqrln/prisma-mcp-zed)
- [Framelink Figma](https://cuecode.dev/extensions/mcp-server-figma)
- [Resend](https://cuecode.dev/extensions/mcp-server-resend)

### As Custom Servers

Creating an extension is not the only way to use MCP servers in CueCode.
You can connect them by adding their commands directly to your settings file ([how to edit](../configuring-cuecode.md#settings-files)), like so:
...
> Note: When a remote MCP server has no configured `"Authorization"` header, CueCode will prompt you to authenticate yourself against the MCP server using the standard MCP OAuth flow.
...
In the case of extensions, after installing it, CueCode will pop up a modal displaying what is required for you to properly set it up.
...
As an example, [the Dagger team suggests](https://container-use.com/agent-integrations#zed) doing that with their [Container Use MCP server](https://cuecode.dev/extensions/mcp-server-container-use):
...
> **Note:** In CueCode v0.224.0 and above, tool approval is controlled by `agent.tool_permissions.default`.
> In earlier versions, it was controlled by the `agent.always_allow_tool_actions` boolean (default `false`).

CueCode's Agent Panel provides the `agent.tool_permissions.default` setting to control tool approval behavior for the native CueCode agent:
...
MCP servers configured in CueCode are forwarded to [External Agents](./external-agents.md) via the [Agent Client Protocol](https://agentclientprotocol.com/). External Agents can also access MCP servers from their own native configuration files.

For details on what configuration is shared between CueCode and External Agents, see [Configuration Boundaries](./external-agents.md#configuration-boundaries).
