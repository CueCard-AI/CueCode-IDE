---
title: AI in CueCode
description: Understand CueCode's AI features, agent paths, model providers, and setup routes.
---

# AI

CueCode's AI docs are organized around three areas:

| Area         | Use it to choose                    | Examples                                                             |
| ------------ | ----------------------------------- | -------------------------------------------------------------------- |
| Agents       | How agentic work runs in CueCode        | CueCode Agent, External Agents, Terminal Threads                         |
| Model access | How CueCode connects to language models | CueCode-hosted models, API access, subscriptions, gateways, local models |
...
Agent paths decide how agentic work runs in CueCode.

- [CueCode Agent](./cuecode-agent.md): CueCode's native agent. It can use models configured through [LLM Providers](./llm-providers.md), including CueCode-hosted models, provider API keys, supported subscriptions, gateways, and local models. It also uses built-in tools, profiles, skills, instructions, and MCP servers.
- [External Agents](./external-agents.md): ACP-integrated agents that run through their own process and configuration.
- [Terminal Threads](./terminal-threads.md): terminal-backed threads for running an agent CLI or TUI directly in CueCode.
...
Model access controls which models power the CueCode Agent and other model-backed CueCode AI features. CueCode can use hosted models, provider API access, subscription sign-in, gateways, and local models.

See [LLM Providers](./llm-providers.md) to choose a model access path.

## AI Features {#ai-features}

CueCode has several AI-powered workflows:
