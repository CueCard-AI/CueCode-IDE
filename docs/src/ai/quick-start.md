---
title: AI Quick Start - CueCode
description: Choose the right CueCode AI setup path for agents, models, subscriptions, local models, edit prediction, and privacy.
---

# AI Quick Start

Use this page to choose the right AI setup path in CueCode. If you already know the company, subscription, model provider, agent, or CLI you want to use, start with [AI by Company](./by-company.md).

## Use the CueCode Agent {#zed-agent}

Use [CueCode Agent](./cuecode-agent.md) when you want CueCode's native agent to read, edit, search, and run code in your project.

CueCode Agent uses CueCode-configured models from [LLM Providers](./llm-providers.md). It also uses CueCode's built-in tools, [Agent Profiles](./agent-profiles.md), [Skills](./skills.md), [Instructions](./instructions.md), and [MCP servers](./mcp.md).

Start in the [Agent Panel](./agent-panel.md) to prompt the agent, add context, review changes, and manage threads.

### Configure the CueCode Agent {#configure-zed-agent}
...
## Use Another Coding Agent in CueCode {#agent-cli}

Use this path for Claude, Codex, OpenCode, Copilot, Cursor, Pi Coding Agent, Gemini CLI, or another coding agent.

| If the agent...                 | Use                                       |
| ------------------------------- | ----------------------------------------- |
| Integrates with CueCode through ACP | [External Agents](./external-agents.md)   |
...
## Choose Which Models the CueCode Agent Uses {#model-access}

The CueCode Agent and other model-backed CueCode AI features use models configured through [LLM Providers](./llm-providers.md).

| If you want to...                                                             | Use                                                               |
| ----------------------------------------------------------------------------- | ----------------------------------------------------------------- |
| Use models billed through CueCode                                                 | [CueCode-Hosted Models](../account/zed-hosted-models.md)              |
...
Provider keys saved through CueCode are stored in the system keychain, not in `settings.json`.
...
| Disable AI, configure tool permissions, or set up edit prediction providers | Settings Editor with {#action cuecode::OpenSettings}                         |
| Edit advanced JSON-only settings                                            | Settings file with {#action cuecode::OpenSettingsFile}                       |
...
| Understand plans, usage, and billing | [Plans & Pricing](../account/plans-and-pricing.md), [CueCode-Hosted Models](../account/zed-hosted-models.md), and [Billing](../account/billing.md) |

## Turn AI Off {#turn-ai-off}

Open the Settings Editor with {#action cuecode::OpenSettings}, search for `Disable AI`, and enable it.
