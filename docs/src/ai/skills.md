---
title: Agent Skills - CueCode
description: Extend CueCode's AI agent with reusable, on-demand skill files for specialized tasks.
---
...
CueCode includes a built-in `create-skill` skill — invoke it with `/create-skill` and the agent walks you through the process.
...
Lastly, it's also possible to add a skill through importing it from an existing GitHub Markdown file. Open the command palette and look for the {#action agent::CreateSkillFromUrl} action. If your clipboard contains a supported GitHub `.md` URL, CueCode pre-fills and fetches it automatically.
...
- [`find-skills`](https://skills.sh/vercel-sites/labs/skills/find-skills): discover and install skills from the open ecosystem
...
To install a skill, copy the skill's folder into `~/.agents/skills/` for global use, or into your project's `.cursor/skills/` folder for project-local use.
...
When someone opens that link (for example by pasting it into their browser or clicking it in a chat), CueCode opens the "Create Skill" page in the settings window, pre-filled with the shared skill.
...
When the agent invokes a skill you created or installed, CueCode prompts you to allow or deny it, using the same permission flow as other tools. Skills built into CueCode do not prompt. You can set per-skill defaults in [Tool Permissions](./tool-permissions.md) so you're not prompted for skills you always trust.
...
CueCode loads skills from two locations:

| Scope         | Path                         | When it applies          |
| ------------- | ---------------------------- | ------------------------ |
| Global        | `~/.agents/skills/`          | Every project            |
| Project-local | `<worktree>/.cursor/skills/` | Only the current project |
...
CueCode Skills apply to the CueCode Agent. External Agents and Terminal Threads may have their own native skills, prompts, or instruction systems. Configure those in the External Agent or CLI.
...
- **No remote registry.** CueCode does not discover or load skills from remote locations at runtime, and custom search paths are not supported. (You can still import a skill once from a GitHub URL — see [Create your own](#create-your-own).) Skills are loaded from `~/.agents/skills/` and `<worktree>/.cursor/skills/` only. Use a symlink if you need to point at another location.
