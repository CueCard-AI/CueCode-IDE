---
title: AI Code Completion in CueCode - BYOK, Ollama, Copilot, Codestral, Mercury Coder
description: Set up AI code completions in CueCode with your own API provider, Ollama, GitHub Copilot, Codestral, or Mercury Coder. Multi-line predictions as you type.
---

# Edit Prediction

Edit Prediction is how CueCode's AI code completions work: an LLM predicts the code you want to write.
As you type, CueCode requests predictions from the edit prediction provider, which returns individual or multi-line suggestions you accept by pressing `tab`.

CueCode is bring-your-own-key: by default, edit predictions reuse the OpenAI-compatible provider you configured for the agent (`language_models.openai_compatible`), so a single API setup powers both chat and inline predictions. You can also use [other providers](#other-providers) like Ollama, GitHub Copilot, Mercury Coder, and Codestral.
...
If you already configured an OpenAI-compatible provider for the agent, inline predictions use its first available model automatically — no extra setup. The provider's endpoint must support legacy `/completions` (FIM-style) requests; a code-focused model (e.g. a coder/FIM variant) gives the best results.

To use a dedicated endpoint or model for predictions instead, set it explicitly:
...
You can verify the configuration by opening the [Settings Editor](zed://settings/edit_predictions.providers) (`Cmd+,` on macOS or `Ctrl+,` on Linux/Windows) and searching for `edit_predictions`.

The prediction icon in the status bar indicates edit predictions are active.
...
![Edit Prediction status bar menu, with the modes toggle.](https://cuecode.dev/img/edit-prediction/status-bar-menu.webp)
...
Open the keymap editor with {#action cuecode::OpenKeymap} ({#kb cuecode::OpenKeymap}), search for `AcceptEditPrediction`, right click on the binding for `tab` and hit `edit`. Then change the context the binding is active in to just `Editor && edit_prediction` and save it.
...
Open the keymap editor with {#action cuecode::OpenKeymap} ({#kb cuecode::OpenKeymap}), search for `AcceptEditPrediction`, right click on the binding for `tab` and delete it.
...
Open the keymap editor with {#action cuecode::OpenKeymap} ({#kb cuecode::OpenKeymap}), search for `AcceptEditPrediction`, right click on the binding for `tab` and delete it. Then right click on the binding for `alt-tab`, select "Edit", and record your desired keystrokes before saving.
...
If you configured edit prediction keybindings before CueCode `v0.229.0`, your `keymap.json` may have entries that are now redundant.
...
**Renamed context**: The `edit_prediction_conflict` context has been replaced by `edit_prediction && (showing_completions || in_leading_whitespace)`. CueCode automatically migrates any bindings that used `edit_prediction_conflict`, so no changes are required on your end.
...
To not have predictions appear automatically as you type, set this in your settings file ([how to edit](../configuring-cuecode.md#settings-files)):
...
To not have predictions appear automatically as you type when working with a specific language, set this in your settings file ([how to edit](../configuring-cuecode.md#settings-files)):
...
To disable edit predictions for specific directories or files, set this in your settings file ([how to edit](../configuring-cuecode.md#settings-files)):

```json [settings]
{
  "edit_predictions": {
    "disabled_globs": ["~/.config/cuecode/settings.json"]
  }
}
```
...
To use GitHub Copilot as your provider, set this in your settings file ([how to edit](../configuring-cuecode.md#settings-files)):
...
If your organization uses GitHub Copilot Enterprise, you can configure CueCode to use your enterprise instance by specifying the enterprise URI in your settings file ([how to edit](../configuring-cuecode.md#settings-files)):
...
Once set, CueCode routes Copilot requests through your enterprise endpoint.
...
1. Open the Settings Editor ({#kb cuecode::OpenSettings})
...
With `"prompt_format": "infer"`, CueCode automatically uses Zeta 2 format for models named `zeta2` and Zeta 2.1 format for models named `zeta2.1`.
