---
title: AI Privacy - CueCode
description: Understand how CueCode handles AI prompts, code context, hosted model requests, provider data boundaries, feedback, training data, and privacy controls.
---

# AI Privacy

This page explains the privacy and trust boundaries for AI features in CueCode,
including [CueCode Agent](./cuecode-agent.md), [Edit Prediction](./edit-prediction.md),
[Inline Assistant](./inline-assistant.md), and
[Git commit generation](../git.md#ai-support-in-git).

CueCode does not retain your prompts or code context by default. For
[CueCode-hosted models](../account/zed-hosted-models.md), CueCode has no-training
...
CueCode only retains AI data when you explicitly share feedback or opt in to
...
| ------------------------------------------------------------ | ------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- |
| [CueCode-hosted models](../account/zed-hosted-models.md)         | CueCode routes requests to hosted model providers     | Provider agreements prohibit training on your prompts or code context and require zero data retention for inference requests, except for provider-designated models with safety retention, such as Anthropic's Mythos-class models. | [CueCode-hosted model commitments](#data-retention-and-training)                                      |
| [Provider API keys](./use-api-access.md)                     | The configured provider                           | The provider handles requests under its own terms. Provider keys saved through CueCode are stored in the system keychain, not in `settings.json`.                                                                                       | [Use API Access](./use-api-access.md)                                                             |
...
| [Agent tools](./tools.md), [MCP](./mcp.md), and integrations | CueCode, configured MCP servers, and external systems | Tools can read, edit, search, run commands, fetch URLs, or call external systems depending on profile, MCP server, and tool permission settings.                                                                                    | [Agent Profiles](./agent-profiles.md), [Tool Permissions](./tool-permissions.md), [MCP](./mcp.md) |
| Project trust and instructions                               | CueCode and the trusted worktree                      | Project-local instructions and skills are loaded from trusted worktrees. External Agents and Terminal Threads may read their own instruction files.                                                                                 | [Worktree Trust](../worktree-trust.md), [Skills](./skills.md), [Instructions](./instructions.md)  |

## CueCode-Hosted Model Commitments {#data-retention-and-training}

For CueCode-hosted models, CueCode has commitments from model providers that prohibit
...
API terms; CueCode-hosted model requests are governed by CueCode's provider agreements.
...
purposes. CueCode cannot opt out of this retention; it applies wherever these models
are served. See
...
- CueCode does not retain your prompts or outputs. Retention happens at the
  provider, under the provider's documented access controls and deletion
  timelines.
- All other CueCode-hosted models keep zero-data-retention handling.
...
## AI Data Retained by CueCode {#ai-data-retained-by-zed}

CueCode may retain AI data only when you explicitly share it or opt in:

- [Response ratings and feedback](./ai-improvement.md#ai-feedback-with-ratings)
  can send a conversation thread to CueCode for review and improvement.
...
- [Telemetry](../telemetry.md): What telemetry CueCode collects and how to control
  it.
- [Privacy for Business](../business/privacy.md): How CueCode Business enforces
  privacy settings across an organization.
- [Admin Controls](../business/admin-controls.md): How owners and admins control
  CueCode-hosted models, Edit Prediction, and feedback sharing.
- [AI Quick Start](./quick-start.md#turn-ai-off): How to turn AI off.
- [Privacy Policy](https://cuecode.dev/privacy-policy): CueCode's privacy policy.
- [Subprocessors](https://cuecode.dev/subprocessors): CueCode's subprocessors.
- [Terms of Service](https://cuecode.dev/terms): CueCode's terms.
