---
title: Use a Gateway - CueCode
description: Configure OpenRouter, Vercel AI Gateway, Amazon Bedrock, and other gateway or cloud model platforms in CueCode.
---
...
| Gateway                   | CueCode AI features | External Agents | Terminal Threads | Notes                                        |
| ------------------------- | --------------- | --------------- | ---------------- | -------------------------------------------- |
...
Use OpenRouter when you want to route CueCode AI features through OpenRouter.
...
CueCode also reads `OPENROUTER_API_KEY` from the local CueCode process environment.
...
Use Vercel AI Gateway when you want to route CueCode AI features through Vercel.
...
CueCode also reads `VERCEL_AI_GATEWAY_API_KEY` from the local CueCode process environment.
...
Bedrock supports CueCode-prefixed AWS environment variables so CueCode does not override or consume your normal AWS credentials:
...
CueCode uses [Cross-Region inference](https://docs.aws.amazon.com/bedrock/latest/userguide/cross-region-inference.html) for Bedrock on a best-effort basis.

By default, CueCode uses regional inference profiles. To opt into global profiles, add `allow_global`:
