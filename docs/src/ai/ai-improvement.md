---
title: Feedback and Training Data - CueCode
description: Understand opt-in AI feedback ratings, Edit Prediction training data, and when CueCode may retain AI data.
---

# Feedback and Training Data

Normal AI requests are not retained by CueCode. For
[CueCode-hosted models](../account/zed-hosted-models.md), provider agreements
...
This page covers the cases where CueCode may retain AI data because you explicitly
shared it or opted in.

AI features in CueCode include:
...
CueCode-hosted model zero-data-retention and no-training commitments, including the
...
You can rate AI responses or submit feedback to help improve CueCode's system prompt,
...
> If you do not want the thread persisted by CueCode, do not rate the response.

### Data Collected from Feedback {#data-collected-ai-feedback}

For conversation threads you explicitly share through ratings or feedback, CueCode
may store:
...
- metadata about your CueCode installation

If you do not rate responses or submit feedback, CueCode does not store Customer Data
related to your AI feature usage for improvement.

Telemetry related to CueCode's AI features is collected separately. This includes
...
Collected feedback data is stored in Snowflake. CueCode periodically reviews this
...
CueCode does not collect training data for the Edit Prediction model unless all of
...
[`edit_predictions.disabled_globs`](https://cuecode.dev/docs/reference/all-settings#edit-predictions)
in your CueCode settings file ([how to edit](../configuring-cuecode.md#settings-files)):
...
For open source projects where you opted in, CueCode may collect:
...
Collected data is stored in Snowflake. CueCode periodically reviews this data to
...
CueCode uses this training dataset to fine-tune
...
On CueCode Business, data sharing is off by default and controlled by organization
...
See the [CueCode Terms of Service](https://cuecode.dev/terms) for more.
