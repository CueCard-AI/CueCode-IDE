---
title: AI Coding Agent - CueCode Agent Panel
description: Use CueCode's AI coding agent to generate, refactor, and debug code with tool calling, checkpoints, and multi-model support.
---

# Agent Panel

The Agent Panel is where you interact with AI agents that can read, write, and run code in your project.
It's the core of CueCode's AI code editing experience — use it for code generation, refactoring, debugging, documentation, and general questions.
...
If you're using the Agent Panel for the first time, configure either a model for the [CueCode Agent](./cuecode-agent.md) or an [External Agent](./external-agents.md).

- Use [LLM Providers](./llm-providers.md) for CueCode-hosted models, API access, subscriptions, gateways, and local models.
...
By default, the Agent Panel uses CueCode's first-party agent.
...
- Pick **CueCode Agent** or any installed [External Agent](./external-agents.md) to start a new thread with that agent.
- Choose **New From Summary** to start a fresh CueCode Agent thread seeded with a summary of the current conversation — useful for compacting long threads as you approach the context window limit.
...
For the CueCode Agent, queued messages get sent at the next turn boundary, which is usually between a tool call and a response, whereas for External Agents, the message gets sent at the end of the generation.
...
If you send a prompt to the Agent and then put CueCode in the background, you can choose to be notified when its generation wraps up via:
...
When you paste multi-line code selections copied from a buffer, CueCode automatically formats them as @-mentions with the file context.
...
After you've configured your LLM providers—either via [API access](./use-api-access.md) or through [CueCode-hosted models](../account/zed-hosted-models.md)—you can switch between their models by clicking on the model selector on the message editor or by using the {#kb agent::ToggleModelSelector} keybinding.

> The same model can be offered via multiple providers - for example, Claude Sonnet 4.5 is available via CueCode Pro, OpenRouter, Anthropic directly, and more.
...
The Agent Panel supports tool calling, which enables agentic editing. CueCode includes [built-in tools](./tools.md) for searching your codebase, editing files, running terminal commands, and more.

Use [Agent Profiles](./agent-profiles.md) to choose which built-in tools and MCP tools are available in a CueCode Agent thread. Use [Tool Permissions](./tool-permissions.md) to control whether permission-gated tool calls are allowed, denied, or confirmed.
...
All [CueCode-hosted models](../account/zed-hosted-models.md) support tool calling out-of-the-box.

### MCP Servers {#mcp-servers}

Similarly to the built-in tools, some models may not support all tools included in a given MCP Server.
CueCode's UI will inform you about this via a warning icon that appears close to the model selector.
...
You can rate agent responses to help improve CueCode's system prompt and tools.

> **Warning:** Rating an AI response sends the conversation thread to CueCode. The
...
> **_If you don't want data persisted on CueCode's servers, don't rate_**.
> We will not collect data for improving CueCode's agent experience without you
> explicitly rating responses.

To help improve CueCode's system prompt and tools, rate responses with the thumbs up/down controls at the end of each response.
