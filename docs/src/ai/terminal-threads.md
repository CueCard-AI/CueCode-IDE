---
title: Terminal Threads - CueCode
description: Run agent CLIs and TUIs directly in terminal-backed threads in CueCode.
---

# Terminal Threads

Terminal Threads are terminal-backed threads in the [Threads Sidebar](./parallel-agents.md#threads-sidebar). Use them when you want to run an agent CLI or TUI directly in CueCode.

Terminal Threads are different from [External Agents](./external-agents.md). External Agents integrate with CueCode through ACP and render as agent threads. Terminal Threads run the native command-line tool in a terminal that CueCode organizes as a thread.

## What CueCode Owns {#what-zed-owns}

CueCode owns the thread surface:
...
CueCode Agent profiles, CueCode Agent tool permissions, CueCode Skills, and CueCode Agent MCP settings do not automatically apply to Terminal Threads.
...
When a terminal produces a bell character while not in focus, CueCode notifies you the same way it does when an agent finishes: with a visual pop-up and an optional sound. Clicking the notification brings the terminal into focus and clears the indicator.
...
Some agent CLIs and TUIs can send terminal signals, such as bell notifications or title updates, that CueCode uses to show useful context in the sidebar.
...
Amp updates terminal titles automatically and can also notify you when it needs your attention. To enable notifications in CueCode Terminal Threads, add `AMP_FORCE_BEL=1` to your terminal environment settings:
...
OpenCode can update terminal titles automatically. For CueCode notifications, add an OpenCode plugin that emits a terminal bell when OpenCode needs your attention.
...
Codex can update the terminal title as it works, which CueCode uses to show useful context for Codex Terminal Threads in the sidebar, such as the project, current status, branch, model, or task progress.
...
In remote projects, the CLI may read the remote shell environment and remote config files. In local Terminal Threads, it reads the local shell environment and local config files. CueCode does not copy API keys from LLM provider settings into Terminal Threads.
