---
title: Use API Access - CueCode
description: Configure provider API access, API keys, API credits, usage billing, and OpenAI-compatible endpoints for CueCode AI features.
---
...
CueCode supports these first-class API providers for model-backed CueCode AI features:
...
Use API access for the CueCode Agent, Inline Assistant, Git commit generation,
thread summaries, and similar CueCode-owned AI features.
...
Most API-access providers can be configured in CueCode's Agent Settings panel with {#action agent::OpenSettings}. Keys saved through CueCode are stored in the system keychain, not in `settings.json`.

CueCode also reads provider-specific environment variables. Non-empty environment variables take precedence over keychain values. If a key comes from an environment variable, unset the variable and restart CueCode to stop using it.
...
You can attach extra HTTP headers to every request CueCode makes to supported HTTP-based providers. This is useful in corporate environments or for observability tooling.
...
Headers managed by CueCode for each provider, such as `Authorization`, `Content-Type`, `Accept`, and provider-specific authentication headers, are ignored with a warning if you try to override them.

## Remote Projects {#remote-projects}

CueCode LLM providers for CueCode AI features are initialized in the local CueCode app. In SSH, dev container, and other remote projects, API keys saved in CueCode are read from the local system keychain, and provider environment variables are read from the local CueCode process environment.
...
CueCode also reads `ANTHROPIC_API_KEY` from the local CueCode process environment.
...
CueCode also reads `OPENAI_API_KEY` from the local CueCode process environment.
...
CueCode reads `GEMINI_API_KEY`, falling back to `GOOGLE_AI_API_KEY`, from the local CueCode process environment.
...
CueCode also reads `MISTRAL_API_KEY` from the local CueCode process environment.
...
Use DeepSeek API access when you have paid API usage, top-ups, or an API key. In CueCode, DeepSeek is API access, not subscription sign-in.
...
CueCode also reads `DEEPSEEK_API_KEY` from the local CueCode process environment.
...
CueCode also reads `XAI_API_KEY` from the local CueCode process environment.
...
CueCode does not sign in to OpenCode with OAuth or detect your OpenCode subscription; it uses an OpenCode API key saved in the system keychain or `OPENCODE_API_KEY`.
...
CueCode also reads `OPENCODE_API_KEY` from the local CueCode process environment.
...
**Note:** CueCode only bundles configuration for long-term OpenCode Free models. Free models that are available for a limited time are not included in CueCode. To use those models, add a custom OpenCode model with configuration from [the OpenCode website](https://opencode.ai/docs/zen#pricing) and [models.dev](https://github.com/anomalyco/models.dev/tree/dev/providers/opencode/models).

#### Custom OpenCode Models {#opencode-custom-models}

The CueCode Agent comes preconfigured with OpenCode models. Add custom OpenCode models when you need newer models, limited-time Free models, or models with custom endpoints.
...
The optional `custom_headers` map adds extra headers to every request, which some providers require. Headers managed by CueCode (such as `X-Api-Key` and `Anthropic-Version`) cannot be overridden.
...
If a model only works with the Responses API, set `capabilities.chat_completions` to `false`. CueCode will use the Responses endpoint for that model.
