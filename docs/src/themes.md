---
title: Themes - CueCode
description: Browse, install, and create themes for CueCode. Includes built-in themes and community theme extensions.
---

# Themes

CueCode comes with a number of built-in themes, with more themes available as extensions.
...
You can find hundreds of different theme options in CueCode's extensions store, which you can access via the command palette with {#action cuecode::Extensions} or the [CueCode website](https://cuecode.dev/extensions?filter=themes).

Many popular themes have been ported to CueCode, and if you're struggling to choose one, visit [zed-themes.com](https://zed-themes.com), a third-party gallery with visible previews for many of them.

## Build Your Theme

You can use [CueCode's Theme Builder](https://cuecode.dev/theme-builder) to design your own custom theme based on an existing one.

This tool lets you fine-tune and preview how every surface in the CueCode app will look.
You can then export the JSON for [local use](./themes.md#local-themes) or for [publishing in CueCode's extension store](./extensions/themes.md).

## Configuring a Theme

Your selected theme is stored in your settings file.
You can open your settings file from the command palette with {#action cuecode::OpenSettingsFile} (bound to {#kb cuecode::OpenSettingsFile}).

By default, CueCode maintains two themes: one for light mode and one for dark mode.
...
Store new themes locally by placing them in the `~/.config/cuecode/themes` directory (macOS and Linux) or `%USERPROFILE%\AppData\Roaming\CueCode\themes\` (Windows).

For example, to create a new theme called `my-cool-theme`, create a file called `my-cool-theme.json` in that directory.
It will be available in the theme selector the next time CueCode loads.
