---
title: Building CueCode for macOS
description: "Guide to building zed for macos for CueCode development."
---

# Building CueCode for macOS
...
CueCode includes visual regression tests that capture screenshots of real CueCode windows and compare them against baseline images. These tests require macOS with Screen Recording permission.
...
```sh
cargo run -p cuecode --bin zed_visual_test_runner --features visual-tests
```

### Baseline Images

Baseline images are stored in `crates/cuecode/test_fixtures/visual_tests/` but are
...
UPDATE_BASELINE=1 cargo run -p cuecode --bin zed_visual_test_runner --features visual-tests
git checkout -
```
...
If CueCode continually rebuilds root crates, you may be opening the CueCode codebase itself in your development build.
...
build of CueCode. These environment variables are in turn passed to `cargo check`, which
...
If you build CueCode frequently, macOS may keep verifying new builds, which can add a few seconds to each iteration.
