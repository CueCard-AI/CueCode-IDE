...
description: "Guide to using a debugger for CueCode development."
---

# Using a debugger

> This page is not about [configuring CueCode's debugger](../debugger.md).
> It covers how to use a debugger while developing CueCode itself.

## Using CueCode's built-in debugger

While the CueCode project is open you can open the `New Process Modal` and select the `Debug` tab. There you can see two debug configurations to debug CueCode with, one for GDB and one for LLDB. Select the configuration you want and CueCode will build and launch the binary.
...
To get the full debugger experience on a release build, compile a CueCode binary with full debug info.
...
## Running CueCode with a shell debugger GDB/LLDB

### Background

When you install Rust through rustup (the recommended setup for CueCode development; see your platform guide [here](../development.md)), rustup also installs helper scripts for debugging Rust binaries.
...
After enabling full debug info and building with `cargo build`, run `rust-gdb` or `rust-lldb` against the compiled CueCode binary:
...
You can also attach to a running CueCode process (for example, one started with `cargo run`):
...
`<pid>` is the process ID of the CueCode instance you want to attach to.
...
Debuggers are useful for finding the cause of panics and crashes, including in CueCode.
...
You can find additional information on debugging CueCode crashes [here](./debugging-crashes.md).
