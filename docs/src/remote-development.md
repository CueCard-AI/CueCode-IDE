---
title: Remote Development in CueCode - SSH Workflows
description: Use remote development in CueCode to edit code over SSH with local UI performance, remote terminals, language servers, and tasks.
---

# Remote Development

Remote Development lets you edit code on a remote server while running CueCode locally. The UI stays responsive because it runs on your machine, while language servers, tasks, and terminals run on the server.
...
Remote development requires two computers, your local machine that runs the CueCode UI and the remote server which runs a CueCode headless server. The two communicate over SSH, so you will need to be able to SSH from your local machine into the remote server to use this feature.

![Architectural overview of CueCode Remote Development](https://cuecode.dev/img/remote-development/diagram.png)

On your local machine, CueCode runs its UI, talks to language models, uses Tree-sitter to parse and syntax-highlight code, and stores unsaved changes and recent projects. The source code, language servers, tasks, and the terminal all run on the remote server. [AI features](./ai/overview.md) work in remote sessions, including the Agent Panel and Inline Assistant.

> **Note:** The original version of remote development sent traffic via CueCode's servers. As of CueCode v0.157 you can no-longer use that mode.

## Setup

1. Download and install the latest [CueCode](https://cuecode.dev/releases). You need at least CueCode v0.159.
...
1. Your local machine will attempt to connect to the remote server using the `ssh` binary on your path. Assuming the connection is successful, CueCode will download the server on the remote host and start it.
1. Once the CueCode server is running, you will be prompted to choose a path to open on the remote server.
   > **Note:** CueCode does not currently handle opening very large directories (for example, `/` or `~` that may have >100,000 files) very well. We are working on improving this, but suggest in the meantime opening only specific projects, or subfolders of very large mono-repos.
...
The remote machine must be able to run CueCode's server. The following platforms should work, though note that we have not exhaustively tested every Linux distribution:
...
The list of remote servers is stored in your settings file {#kb cuecode::OpenSettings}. You can edit this list using the Remote Projects dialog {#kb projects::OpenRemote}, which provides some robustness - for example it checks that the connection can be established before writing it to the settings file.
...
CueCode shells out to the `ssh` on your path, and so it will inherit any configuration you have in `~/.ssh/config` for the given host. That said, if you need to override anything you can configure the following additional options on each connection:
...
There are two additional CueCode-specific options per connection, `upload_binary_over_ssh` and `nickname`:
  ...
  "ssh_connections": [
    {
      "host": "192.168.1.10",
      "projects": [{ "paths": ["~/code/zed/zed"] }],
      // by default CueCode will download the server binary from the internet on the remote.
      ...
      // Shown in the CueCode UI to help distinguish multiple hosts.
      "nickname": "lil-linux"
    }
  ]
}
...
CueCode on Windows supports SSH remoting and will prompt for credentials when needed.
...
CueCode supports opening folders inside of WSL natively on Windows.
...
- The local CueCode settings (in `~/.config/cuecode/settings.json` on macOS or `~/.config/cuecode/settings.json` on Linux) on your local machine.
- The server CueCode settings (in the same place) on the remote server.
- The project settings (in `.zed/settings.json` or `.editorconfig` of your project)

Both the local CueCode and the server CueCode read the project settings, but they are not aware of the other's main settings file.
...
In most cases, your remote server will already have proxy environment variables configured. CueCode will automatically use them when downloading language servers, communicating with LLM models, etc.
...
Alternatively, you can configure the proxy in the remote machine's `~/.config/cuecode/settings.json` (Linux) or `~/.config/cuecode/settings.json` (macOS):
...
Once you provide the SSH options, CueCode shells out to `ssh` on your local machine to create a ControlMaster connection with the options you provide.

Any prompts that SSH needs will be shown in the UI, so you can verify host keys, type key passwords, etc.

Once the master connection is established, CueCode will check to see if the remote server binary is present in `~/.zed_server` on the remote, and that its version matches the current version of CueCode that you're using.

If it is not there or the version mismatches, CueCode will try to download the latest version. By default, it will download from `https://cuecode.dev` directly, but if you set: `{"upload_binary_over_ssh":true}` in your settings for that server, it will download the binary to your local machine and then upload it to the remote server.

If you'd like to maintain the server binary yourself you can. You can either download our prebuilt versions from [GitHub](https://github.com/zed-industries/zed/releases), or [build your own](https://cuecode.dev/docs/development):
...
Stripping debug symbols matches the packaging step in [`script/bundle-linux`](https://github.com/zed-industries/zed/blob/main/script/bundle-linux) and keeps the binary closer to the size of CueCode's prebuilt downloads. Prefer `llvm-objcopy` because GNU `objcopy` may not understand newer LLVM-emitted CREL sections.
...
If you do this, you must upload it to `~/.zed_server/zed-remote-server-{RELEASE_CHANNEL}-{VERSION}` on the server, for example `~/.zed_server/zed-remote-server-stable-0.217.3+stable.105.80433cb239e868271457ac376673a5f75bc4adb1`. The version must exactly match the version of CueCode itself you are using.

## Maintaining the SSH connection

Once the server is initialized. CueCode will create new SSH connections (reusing the existing ControlMaster) to run the remote development server.
...
In the case that reconnecting fails, the daemon will not be re-used. That said, unsaved changes are by default persisted locally, so that you do not lose work. You can always reconnect to the project at a later date and CueCode will restore unsaved changes.

If you are struggling with connection issues, you should be able to see more information in the CueCode log (`cmd-shift-p` Open Log). If you are seeing things that are unexpected, please file a [GitHub issue](https://github.com/CueCard-AI/CueCode-IDE/issues/new) or reach out in the #support forums on [Discord](https://cuecode.dev/community-links).

## Supported SSH Options

Under the hood, CueCode shells out to the `ssh` binary to connect to the remote server. We create one SSH control master per project, and then use that to multiplex SSH connections for the CueCode protocol itself, any terminals you open and tasks you run. We read settings from your SSH config file, but if you want to specify additional options to the SSH control master you can configure CueCode to set them.
...
Note that we deliberately disallow some options (for example `-t` or `-T`) that CueCode will set for you.
...
- [Git Worktrees](./git.md#git-worktrees): Create and switch between linked
  Git worktrees. CueCode supports the worktree picker in remote projects when the
  remote connection is active.
- [Configuring CueCode](./configuring-cuecode.md): Manage shared and project settings,
  including `.zed/settings.json`.
- [Agent Panel](./ai/agent-panel.md): Use AI workflows in remote projects.
- [Remote Development on cuecode.dev](https://cuecode.dev/remote-development): Product
