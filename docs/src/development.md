---
title: Developing CueCode
description: "Guide to building and developing CueCode from source."
---

# Developing CueCode

See the platform-specific instructions for building CueCode from source:
...
CueCode stores secrets in the system keychain.

However, when running a development build of CueCode on macOS (and perhaps other
...
That is why, by default, when running a development build of CueCode an alternative
...
development build, run CueCode with the following environment variable set:
...
CueCode includes a frame time measurement system that can be used to profile how long it takes to render each frame. This is particularly useful when comparing rendering performance between different versions or when optimizing frame rendering code.
...
When enabled, CueCode will print frame rendering timing information to stderr, showing how long each frame takes to render.
...
2. **Test the first version:**
   ...
   - Run CueCode in release mode and use it for 5-10 seconds: `cargo run --release &> version-a`

3. **Test the second version:**

   - Checkout another commit you want to compare
   - Run CueCode in release mode and use it for 5-10 seconds: `cargo run --release &> version-b`
...
CueCode supports performance profiling with Event Tracing for Windows (ETW) to capture detailed performance data, including CPU, GPU, memory, disk, and file I/O activity. Data is saved to an `.etl` file, which can be opened in standard profiling tools for analysis.
...
- `zed: record etw trace with heap tracing`: includes heap allocation data for the CueCode process

CueCode will prompt you to choose a save location for the `.etl` file, then request administrator permission. Once granted, recording will begin.
...
- [Code of Conduct](https://cuecode.dev/code-of-conduct)
- [CueCode Contributor License](https://cuecode.dev/cla)
