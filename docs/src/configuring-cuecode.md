---
title: Configuring CueCode - Settings and Preferences
description: Configure CueCode with the Settings Editor, JSON files, and project-specific overrides. Covers all settings options.
---

# Configuring CueCode

This guide explains how CueCode's settings system works, including the Settings Editor, JSON configuration files, and project-specific settings.

For visual customization (themes, fonts, icons), see [Appearance](./appearance.md).

## Settings Editor

The **Settings Editor** ({#kb cuecode::OpenSettings}) is the primary way to configure CueCode. It provides a searchable interface where you can browse available settings, see their current values, and make changes.

To open it:

- Press {#kb cuecode::OpenSettings}
- Or run {#action cuecode::OpenSettings} from the command palette
...
Your user settings apply globally across all projects. Open the file with {#kb cuecode::OpenSettingsFile} or run {#action cuecode::OpenSettingsFile} from the command palette.

The file is located at:

- macOS: `~/.config/cuecode/settings.json`
- Linux: `~/.config/cuecode/settings.json` (or `$XDG_CONFIG_HOME/cuecode/settings.json`)
- Windows: `%APPDATA%\CueCode\settings.json`

The syntax is JSON with support for `//` comments.

### Default Settings

To see all available settings with their default values, run {#action cuecode::OpenDefaultSettings} from the command palette. This opens a read-only reference you can use when editing your own settings.

### Project Settings

Override user settings for a specific project by creating a `.zed/settings.json` file in your project root. Run {#action cuecode::OpenProjectSettings} to create this file.
...
1. **Default settings** — CueCode's built-in defaults
...
CueCode has some compatibility support for Emacs and Vim [modelines](./modelines.md), so you can set some settings per-file.
...
CueCode supports deep links that open specific settings directly:
