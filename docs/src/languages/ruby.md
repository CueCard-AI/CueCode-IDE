---
title: Ruby
description: "Configure Ruby language support in CueCode, including language servers, formatting, and debugging."
---
...
There are multiple language servers available for Ruby. CueCode supports the two following:
...
In addition to these two language servers, CueCode also supports:

- [rubocop](https://github.com/rubocop/rubocop) which is a static code analyzer and linter for Ruby. Under the hood, it's also used by CueCode as a language server, but its functionality is complementary to that of solargraph and ruby-lsp.
...
Configure language servers in Settings ({#kb cuecode::OpenSettings}) under Languages > Ruby, or add to your settings file:
...
Solargraph has formatting and diagnostics disabled by default. We can tell CueCode to enable them by adding the following to your `settings.json`:
...
Rubocop has unsafe autocorrection disabled by default. We can tell CueCode to enable it by adding the following to your `settings.json`:
...
The Ruby extension provides a debug adapter for debugging Ruby code. CueCode's name for the adapter (in the UI and `debug.json`) is `rdbg`, and under the hood, it uses the [`debug`](https://github.com/ruby/debug) gem. The extension uses the [same activation logic](#language-server-activation) as the language servers.
...
Configure formatting in Settings ({#kb cuecode::OpenSettings}) under Languages > HTML+ERB, or add to your settings file:
