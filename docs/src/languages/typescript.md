---
title: TypeScript
description: "Configure TypeScript language support in CueCode, including language servers, formatting, and debugging."
---

# TypeScript

TypeScript and TSX support are available natively in CueCode.
...
By default CueCode uses [vtsls](https://github.com/yioneko/vtsls) for TypeScript, TSX, and JavaScript files.
Configure language servers in Settings ({#kb cuecode::OpenSettings}) under Languages > TypeScript/TSX/JavaScript, or add to your settings file:
...
Prettier will also be used for TypeScript files by default. To disable this, configure in Settings ({#kb cuecode::OpenSettings}) under Languages > TypeScript, or add to your settings file:
...
CueCode sets the following initialization options to make the language server send back inlay hints (that is, when CueCode has inlay hints enabled in the settings).

You can override these settings in your CueCode `settings.json` when using `typescript-language-server`:
...
CueCode enables references and implementations code lenses for `vtsls` by default. These show reference counts and implementation counts above functions, classes, and interfaces. To use them, enable the `code_lens` setting:
...
CueCode supports debugging TypeScript code out of the box with `vscode-js-debug`.
...
As for all languages, configurations from `.vscode/launch.json` are also available for debugging in CueCode.
...
- [CueCode Yarn documentation](./yarn.md) for a walkthrough of configuring your project to use Yarn.
- [CueCode Deno documentation](./deno.md)
