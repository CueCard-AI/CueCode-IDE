---
title: Ansible
description: "Configure Ansible language support in CueCode, including language servers, formatting, and debugging."
---

# Ansible

Support for Ansible in CueCode is provided via a community-maintained [Ansible extension](https://github.com/kartikvashistha/zed-ansible).
...
To change this behavior, you can add a `"file_types"` section to CueCode settings inside your project (`.zed/settings.json`) or your CueCode user settings (`~/.config/cuecode/settings.json`) to match your folder/naming conventions. For example:
...
- or, configure the YAML language server settings to set this schema for all your inventory files, that match your inventory pattern, under your CueCode settings ([ref](https://cuecode.dev/docs/languages/yaml)):
...
When desired, any of the above default settings can be overridden under the `"lsp"` section of your CueCode settings file. For example:

```json [settings]
{
  "lsp": {
    // The CueCode Ansible extension prefixes all settings with `ansible`
