---
title: SQL
description: "Configure SQL language support in CueCode, including language servers, formatting, and debugging."
---
...
CueCode supports auto-formatting SQL using external tools like [`sql-formatter`](https://github.com/sql-formatter-org/sql-formatter).
...
3. Configure formatting in Settings ({#kb cuecode::OpenSettings}) under Languages > SQL, or add to your settings file:
...
You can add this to CueCode project settings (`.zed/settings.json`) or via your CueCode user settings (`~/.config/cuecode/settings.json`).
...
When using a `.sql-formatter.json` file you can use a simplified CueCode settings configuration:
