---
title: YAML
description: "Configure YAML language support in CueCode, including language servers, formatting, and debugging."
---

# YAML

YAML support is available natively in CueCode.
...
You can configure various [yaml-language-server settings](https://github.com/redhat-developer/yaml-language-server?tab=readme-ov-file#language-server-settings) by adding them to your CueCode settings.json in a `yaml-language-server` block under the `lsp` key.

You can configure custom YAML schemas using relative paths. CueCode resolves paths relative to your project root:
...
By default, CueCode uses Prettier for formatting YAML files.
...
To use `yaml-language-server` instead of Prettier for YAML formatting, configure in Settings ({#kb cuecode::OpenSettings}) under Languages > YAML, or add to your settings file:
