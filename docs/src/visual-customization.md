...
description: "Various aspects of CueCode's visual layout can be configured in the Settings Editor or settings file."
---

# Visual Customization

Various aspects of CueCode's visual layout can be configured in the Settings Editor ({#kb cuecode::OpenSettings}) or in your settings file ({#kb cuecode::OpenSettingsFile}).

See [All Settings](./reference/all-settings.md) for additional information and other non-visual settings.

## Themes

You can install many [themes](./themes.md) and [icon themes](./icon-themes.md) in form of extensions by running {#action cuecode::Extensions} from the command palette.
...
By default CueCode enables font ligatures which will visually combine certain adjacent characters.
...
```json [settings]
{
  // Force usage of CueCode build in path prompts (file and directory pickers)
  // instead of OS native pickers (false).
  "use_system_path_prompts": true,
  // Force usage of CueCode built in confirmation prompts ("Do you want to save?")
...
See [CueCode AI Documentation](./ai/overview.md) for additional non-visual AI settings.
