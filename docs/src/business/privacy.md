---
title: Privacy for Business - CueCode Business
description: How CueCode Business handles data privacy across your organization, including enforced protections for prompts and training data.
---

# Privacy for Business

CueCode Business removes the per-member data-sharing options that Free and Pro
...
For all members of a CueCode Business organization:

- **No prompt sharing:** Conversations and prompts are never shared with CueCode.
...
- **No training data sharing:** Code context is never shared with CueCode for
...
- Disable CueCode-hosted models entirely via the CueCode Model Provider toggle, so no
  prompts reach CueCode's infrastructure
