# CueCode Documentation Conventions

This document covers structural conventions for CueCode documentation: what to document, how to organize it, and when to create new pages.
...
- **Version-specific behavior changes** — Include version callouts (e.g., "In CueCode v0.224.0 and above...")
...
- Users would search for it **by name** (e.g., "CueCode terminal", "CueCode snippets")
...
```yaml
---
title: Feature Name - CueCode
description: One sentence describing what this page covers. Used in search results.
---
```

- `title`: Feature name, optionally with "- CueCode" suffix for SEO
...
- Commands: `:w`, `:q`
- File paths: `~/.config/cuecode/settings.json`
- Action names: `git::Commit`
- Values: `true`, `false`, `"eager"`

### Action and Keybinding References

Use CueCode's special syntax for dynamic rendering:
...
- Link to `cuecode.dev/docs` pages when appropriate
...
- For main feature pages with a matching marketing page, include a relevant
  `cuecode.dev` docs link in addition to docs links
...
Keep language docs focused on CueCode-specific configuration, not general language tutorials.
...
> Configure inline blame in Settings ({#kb cuecode::OpenSettings}) by searching for "inline blame", or add to your settings file:
>
> ```json [settings]
> {
>   "git": {
...
> Add the following to your settings file ([how to edit](./configuring-cuecode.md#settings-files)):

### Settings File Locations

- **macOS/Linux:** `~/.config/cuecode/settings.json`
...
- [ ] Main feature pages include a relevant `cuecode.dev` docs link
