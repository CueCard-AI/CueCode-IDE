...
```markdown
![Alt text description](https://cuecode.dev/img/path/to/image.webp)
```
...
- **Practical over promotional**: Focus on what users can do, not on selling CueCode. Avoid marketing language like "powerful," "revolutionary," or "best-in-class."
- **Honest about limitations**: When CueCode lacks a feature or doesn't match another tool's depth, say so directly. Pair limitations with workarounds or alternative workflows.
...
- **Present tense**: "CueCode opens the file" not "CueCode will open the file."
...
```
CueCode provides a powerful and seamless formatting experience. Simply navigate to the settings and you'll find the format_on_save option which enables CueCode's incredible auto-formatting capabilities.
```

### Good: Honest about limitations

```
CueCode doesn't index your project like IntelliJ does. You open a folder and start working immediately—no waiting. The trade-off: cross-project analysis relies on language servers, which may not go as deep.
...
```
While some users might miss indexing, CueCode's approach is actually better because it's faster.
```
...
## CueCode-Specific Conventions

### Recognized Rules Files

When documenting rules/instructions for AI, note that CueCode recognizes these files (in priority order):
...
- macOS: `~/.config/cuecode/keymap.json`
- Linux: `~/.config/cuecode/keymap.json`
- Windows: `%AppData%\CueCode\keymap.json`
