...
**Stack context:** Rust, GPUI, native macOS/Linux/Windows. IDE lives at
`apps/CueCode-IDE/`; agent runtime in `apps/CueCode-IDE/crates/{agent,acp_thread,agent_ui}`.
**Production harness:** proprietary cloud ([harness/cloud/](./harness/cloud/README.md)) via
planned `cuecode_cloud` client. Local stubs: `cuecode_specs`, `cuecode_sandbox`
(see [06-system-design](./core/06-system-design#new-crates)).

### Active vs archive {#active-vs-archive}

| Kind | Where | Notes |
|------|-------|-------|
| **Execute next** | [delivery/build-plans/](./delivery/build-plans/README.md) | **Next: Build phase 1.5** (spec roots + pin modes). Say `Build phase X.Y`. |
| **Living product** | [core/](./core/README.md), [agent/](./agent/README.md), [design/](./design/README.md), [harness/](./harness/README.md), [marketing/](./marketing/README.md), `growth/` (private, CueInference workspace only) | Authoritative product / runtime / public-site / CueRaise truth |
| **Living ops** | [ops/13-plan-e2e-fixture](./ops/13-plan-e2e-fixture.md), [RUNBOOK](../../RUNBOOK.md) | QA fixture + day-to-day commands |
| **Archive — do not re-run** | [ops/14](./ops/14-monorepo-layout.md) (H0–H6 **Done**), [ops/15](./ops/15-nest-cuecode-ide.md) (N0–N4 **Done**) | History only. Day-to-day IDE paths: [MONOREPO § nested IDE](../../MONOREPO.md#working-in-the-nested-ide) |

**Number collisions:** `14` and `15` appear in more than one folder (`agent/14`, `harness/14`, `ops/14`; `parity/15`, `ops/15`). Always open by **folder path**, not bare number.

**AI agents (session):** root [`AGENTS.md`](../../AGENTS.md) → [`AGENTS_GUIDE.md`](../../AGENTS_GUIDE.md) → this file → task-specific spec / build phase.
...
├── marketing/                ← Cue Cloud + Cue Labs public marketing pages
├── growth/                   ← PRIVATE CueGrowth / CueRaise product (not GPL-synced)
├── ops/                      ← 10–15 infra, metrics, OQs, fixture, monorepo layout, nest IDE
...
| [marketing/](./marketing/README.md) | Cue Cloud landing / public marketing |
| `growth/` (workspace only) | CueRaise investor OS / CueGrowth-family products — **private**, excluded from GPL sync; open in CueInference checkout only |
...
3. [10-infrastructure](./ops/10-infrastructure) — models, sandbox OS, CI, telemetry
4. [11-metrics-and-success](./ops/11-metrics-and-success) — release gates
5. [14-monorepo-layout](./ops/14-monorepo-layout) — shelf migration history (**Done**; do not re-run)
6. [15-nest-cuecode-ide](./ops/15-nest-cuecode-ide) — nest IDE history (**Done** / archive)
7. [07-implementation-roadmap](./delivery/07-implementation-roadmap#phase-6) — Phase 6 bundles
8. Day-to-day IDE paths → [MONOREPO § nested IDE](../../MONOREPO.md#working-in-the-nested-ide)
...
|---------|------------|-----------|----------------|
| **AI coding agent (session)** | Root [`AGENTS.md`](../../AGENTS.md) → [`AGENTS_GUIDE.md`](../../AGENTS_GUIDE.md) → [§ Active vs archive](#active-vs-archive) | [build-plans](./delivery/build-plans/README.md) next phase; skill `implement-spec` | PR cites `{#anchor-id}`; verify from `apps/CueCode-IDE/` |
...
│ Build phase X.Y│ implement-spec + rust-quality                          │
...
| design/16 | [planning-hub](./design/16-planning-hub) | Agent Linear / Plan hub (draft) | Plan tab + `.cuecode/plans` |
| design/17 | [layout-studio](./design/17-layout-studio) | Layout Studio workspace UX (draft) | Virtualized workspace |
| design/18 | [design-studio](./design/18-design-studio) | Design Studio canvas (stub) | Co-creation surface |
| mkt/01 | [cuecloud-landing](./marketing/01-cuecloud-landing.md) | Cue Cloud marketing home | `{#proof}`, funnel phases |
| mkt/02 | [site-assistant](./marketing/02-site-assistant.md) | On-site chat assistant | `SITE.md` + Gemini |
| mkt/03 | [seo-geo](./marketing/03-seo-geo.md) | SEO + GEO technical plan | S0–S6 |
| mkt/04 | [cuelabs](./marketing/04-cuelabs.md) | Cue Labs company site | L1–L4 |
| mkt/05 | [hiring-challenge](./marketing/05-hiring-challenge.md) | SMBH hiring challenge | M0–M4 |
...
| — | [decisions/](./decisions/README.md) | ADRs closing ops/12 questions | [ADR-041](./decisions/ADR-041-multi-lane-sub-sessions.md) |
| — | [ops/13-plan-e2e-fixture](./ops/13-plan-e2e-fixture) | PulseBoard QA fixture paths | `{#plan-e2e-fixture}` |
| — | [ops/14-monorepo-layout](./ops/14-monorepo-layout) | Root hierarchy migration — **Done** (archive) | `{#monorepo-layout}`, `{#phases}` |
| — | [ops/15-nest-cuecode-ide](./ops/15-nest-cuecode-ide) | Nest GPL client → `apps/CueCode-IDE/` — **Done** (archive) | `{#nest-cuecode-ide}`, `{#verify}` |
| 13 | [ai-maxxing](./agent/13-ai-maxxing) | AI strategy vs sidebar IDEs | `{#design-checklist}`, `{#vs-cursor}` |
| agent/14 | [agent-modes-and-builder](./agent/14-agent-modes-and-builder) | Composer modes, saved agents, Agent Builder | Modes + builder |
| harness/14 | [agent-harness (redirect)](./harness/14-agent-harness.md) | Stub → [harness/](./harness/README.md) | `{#quick-links}` |
...
### Rollup (read this first)

| Bucket | Status | Where |
|--------|--------|-------|
| **Ship work** | Active — next **1.5** | [build-plans/](./delivery/build-plans/README.md) · progress in [07 §progress](./delivery/07-implementation-roadmap.md#progress) |
| **Product / agent / design / harness / marketing** | Living drafts | Folder READMEs; treat as authoritative until a phase marks them done |
| **ops/13 Plan E2E fixture** | Living | [13-plan-e2e-fixture](./ops/13-plan-e2e-fixture.md) |
| **ops/14 monorepo layout** | **Done** (archive) | [14-monorepo-layout](./ops/14-monorepo-layout.md) |
| **ops/15 nest CueCode-IDE** | **Done** (archive) | [15-nest-cuecode-ide](./ops/15-nest-cuecode-ide.md) |
| **00-README** | Living index | This file |

Do **not** treat the old all-Draft table as “everything is equally unfinished.” Execution state lives in build-plans; migration programs above are closed.

### Per-doc detail (product / parity)
...
| 03-zed-reference-cleanup (passes A–E) | Living progress | Platform | [Progress](./core/03-zed-reference-cleanup-phases.md#progress) |
| 04-sandbox-core | Draft | Product + agent | — |
| 05-innovations | Draft — highest priority | Product | — |
| 06-system-design | Draft | Platform | — |
| 07-implementation-roadmap | Living — progress rollup | Product | — |
| 08-agent-tools-and-skills | Draft | Agent inference | — |
| 09-ui-ux-spec | Draft | GPUI | — |
| design/16-planning-hub | Draft | Product + GPUI | — |
| design/17-layout-studio | Draft | GPUI | — |
| design/18-design-studio | Stub | Product + GPUI | — |
| marketing/01–05 | Living drafts | Marketing | 2026-07-20 |
...
| agent/14-agent-modes-and-builder | Draft | Agent + GPUI | — |
| harness/14-agent-harness | Redirect stub | Agent + GPUI | 2026-06-17 |
| ops/14-monorepo-layout | **Done** (archive) | Ops | 2026-07-20 |
| ops/15-nest-cuecode-ide | **Done** (archive) | Ops | 2026-07-20 |
...
| 00-README | Living index | All | 2026-07-20 |
...
- **Living** — actively maintained index / rollup (not a closed program).
- **Done (archive)** — migration finished; do not re-execute phases.
...
| `implement-spec` | Load build phase / `@spec`; plan → implement → verify | [build-plans/](./delivery/build-plans/README.md), [08](./agent/08-agent-tools-and-skills) |
| `rebrand-progress` | Phase 0 rebrand pass tracker | [03](./core/03-fork-and-rebrand) |

**Also planned** (see [08](./agent/08-agent-tools-and-skills)):

- `cuecode-specs` — load and cite spec sections in agent context (rule `cuecode-specs.mdc` covers much of this today)
...
| `.cursor/specs/decisions/` | ADRs closing ops/12 questions ([index](./decisions/README.md)) |
...
| `apps/CueCode-IDE/crates/agent/` | Native agent loop, tools, templates |
| `apps/CueCode-IDE/crates/agent_ui/` | GPUI agent panel, composer, review |
| `apps/CueCode-IDE/crates/acp_thread/` | Session entity, plan, terminals |
| `apps/CueCode-IDE/crates/sandbox/` | OS sandbox (Seatbelt, Bubblewrap) |
...
| **implement-spec** | Shipped skill: load `@spec` / **Build phase X.Y** → plan → implement → verify from `apps/CueCode-IDE/`. |
