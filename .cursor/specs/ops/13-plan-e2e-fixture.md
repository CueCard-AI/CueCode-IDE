...
> **Seed in monorepo:** [`qa-fixture/pulseboard/`](../../../qa-fixture/pulseboard/) — edit here; sync with `./script/sync-qa-fixture`
> **Local clone (open in CueCode):** [`cuecode-testing-repo/`](../../../cuecode-testing-repo/) — `./script/clone-qa-fixture-repo`

CueCode Planning Hub **Implement**, ticket sessions, multi-language agent work, and Plan QA run against a **dedicated polyglot fixture** — not against the CueInference monorepo while building CueCode-IDE.
...
| **Unit tests** | `CueCode-IDE/crates/cuecode_plans/tests/` | CI without cloning GitHub |
...
Canonical seed copy: **`CueInference/qa-fixture/pulseboard/`**.  
...
Creates or updates **`cuecode-testing-repo/`** next to `CueCode-IDE/`.

Open in CueCode:

```bash
cd CueCode-IDE && ./script/run-cuecode-dev ../cuecode-testing-repo
```
...
4. **`cargo run -p cuecode_plans --bin cuecode-plan -- validate`** from fixture root (requires CueCode-IDE on PATH or run from dev checkout with `--project-root` pointing at fixture).
...
Three layers: **seed** (`qa-fixture/pulseboard/`, committed in CueInference) ↔ **local clone** (`cuecode-testing-repo/`) ↔ **GitHub** (`CueCard-AI/cuecode-testing-repo`).

| Direction | Command | Notes |
|-----------|---------|-------|
| Status | `./script/sync-qa-fixture --status` | Diff seed vs clone vs remote |
| Outbound | `./script/sync-qa-fixture --to-remote --push` | Seed → clone → GitHub; CI on `qa-fixture/**` push to `main` |
...
2. Review `qa-fixture/pulseboard/` → commit to CueInference
...
|----------|---------|--------|
| `publish_qa_fixture.yml` | Push to `main` changing `qa-fixture/**` | Publish seed → GitHub |
| `ingest_qa_fixture_manual.yml` | Manual dispatch | PR: GitHub → `qa-fixture/pulseboard/` |
...
Edit the seed under `qa-fixture/pulseboard/`, then:
...
```bash
./script/clone-qa-fixture-repo
rsync -a --delete qa-fixture/pulseboard/ cuecode-testing-repo/
