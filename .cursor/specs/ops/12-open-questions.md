...
Resolve cloud-specific decisions there first; file ADRs in
[`.cursor/specs/decisions/`](../decisions/README.md) when closed.
...
| P1 | Q5, Q6, Q14, Q15 | Phase 2–3 harness (Q13 resolved — ADR-041) |
...
[`.cursor/specs/decisions/`](../decisions/README.md) or link from PR description.
...
Q13 is closed using this pattern (done):

1. ADR filed at [ADR-041](../decisions/ADR-041-multi-lane-sub-sessions.md)
2. Q13 status → Resolved with ADR link (see [§q13-lanes](#q13-lanes))
3. Row added under [§Resolved](#resolved)
4. Implement per [local §lane-panel-ui](../harness/local/01-agent-harness.md#lane-panel-ui)

For new questions: copy the ADR template above into `decisions/ADR-NNN-….md`, then
update the question status + Resolved table the same way.
...
| 2026-06-17 | Q13 Multi-lane threads | One parent AcpThread + sub-sessions — [ADR-041](../decisions/ADR-041-multi-lane-sub-sessions.md) |
