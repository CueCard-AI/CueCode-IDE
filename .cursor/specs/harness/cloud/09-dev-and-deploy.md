# Dev, test, run, and deploy — CueHarness agent E2E runbook {#dev-and-deploy}

> **CueCloud umbrella:** runbook for CueHarness (Model B) — the cloud agent-runtime half of CueCloud. Index: [README](./README.md).
> **Branch:** [harness/cloud/](./README.md) — CueHarness (Model B) local development and M0 implementation.  
> **Status:** Implemented in public repo (M0). Private `cuecode-harness` is a sibling repo at `~/CueInference/cloud/cuecode-harness`.
...
| Private orchestration | `~/CueInference/cloud/cuecode-harness/services/harness-api` | Proprietary |
...
```bash
# From apps/CueCode-IDE root (public GPL repo)
...
├── apps/CueCode-IDE/        # public GPL archive source
│   ├── crates/cuecode_chp/
...
└── cloud/cuecode-harness/         # private harness API
    ├── vendor/cuecode_chp/  # symlink → ../../../apps/CueCode-IDE/crates/cuecode_chp
    ├── services/harness-api/
    └── script/dev-harness
```

If `apps/CueCode-IDE/` is checked out elsewhere, `script/cuecode-local --harness` checks `../../cloud/cuecode-harness` (from the IDE package) and `~/CueInference/cloud/cuecode-harness`.
...
| **2 — Stack** | `script/cuecode-local --harness` or start `~/CueInference/cloud/cuecode-harness` + `script/cuecode-local` | Private `harness-api` |
...
**Path:** `~/CueInference/cloud/cuecode-harness/`

```bash
cd ~/CueInference/cloud/cuecode-harness
cp .env.example .env
script/dev-harness
curl -sf http://127.0.0.1:8787/health
...
CHP types come from `vendor/cuecode_chp` (symlink to the GPL crate). For CI without a sibling checkout, use git + `[patch]` (see `cloud/cuecode-harness/README.md`).
