# CueHarness overview — the cloud agent runtime (Model B) {#cloud-harness-overview}

> **CueCloud umbrella:** CueHarness is the cloud agent-runtime half of CueCloud. These specs cover the runtime, CHP protocol, tool-host, and model-gateway; the inference plane (CueControl → CueScheduler → exo → pods) is the other half. Index: [README](./README.md).
