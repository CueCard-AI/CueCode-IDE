...
| **Fixture** | `qa-fixture/pulseboard` — `cd CueCode-IDE && cargo run -p cuecode` |
...
**Verify 0:** `cd CueCode-IDE && CARGO_TARGET_DIR=$PWD/target cargo check -p workspace -p agent_ui && cargo run -p cuecode`
...
**Setup:** `cd CueCode-IDE && cargo run -p cuecode` → open `qa-fixture/pulseboard`
...
**Automated (macOS):** from `CueCode-IDE/`:
...
4. `cuecode-plan validate --project-root .` from CueCode-IDE dev checkout.
