...
- Verification: `apps/CueCode-IDE/script/rebrand-check.sh`, `apps/CueCode-IDE/script/qa-p0.sh`, `apps/CueCode-IDE/script/rebrand-progress.sh`, `apps/CueCode-IDE/script/network-idle-audit.sh`
...
Rough scale: ~846 files under `apps/CueCode-IDE` still match `Zed` / `zed` / `zed.dev`; most
...
- [x] Agent prompt to local model via `eval-cli` + Ollama — `script/qa-agent-ollama.sh` *(gate locally; CI: `agent-ollama-linux` is workflow_dispatch-only)*
...
| Scope | Action |
|-------|--------|
| `apps/CueCode-IDE/docs/` (~196 files) | Replace product copy; keep historical/upstream citations where needed |
