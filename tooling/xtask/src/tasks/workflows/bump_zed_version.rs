...
fn resolve_versions() -> (steps::NamedJob, ResolvedOutputs) {
    fn extract_versions() -> Step<Run> {
        named::bash(indoc::indoc! {r#"
            ...
            channel=$(cat crates/cuecode/RELEASE_CHANNEL)
...
fn bump_main(
    ...
    fn bump_version() -> Step<Run> {
        named::bash("cargo set-version -p cuecode --bump minor")
    }
...
fn create_preview_branch(
    ...
    fn promote_to_preview() -> Step<Run> {
        named::bash("echo -n preview > crates/cuecode/RELEASE_CHANNEL")
    }
    ...
    .with_files("crates/cuecode/RELEASE_CHANNEL")
    .into();
...
fn promote_to_stable(
    ...
    let write_channel = named::bash("echo -n stable > crates/cuecode/RELEASE_CHANNEL");
